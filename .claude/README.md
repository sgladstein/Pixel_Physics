...
**Always-loaded floor: ~22,843 tokens** — `CLAUDE.md` at 91,372 B / 1,381 lines, bytes/4.0. Ceiling 28,000 (5,157 under). Plus ~430 for the hook, and the harness system prompt and tool schemas on top; this is a floor.

Paid by **every session, agent and subagent** — ten heads is ~228,430 tokens before any of them reads source.
...
Cache-prefix churn, distinct versions per day (newest first): 2026-09-14 x1, 2026-09-13 x5, 2026-09-12 x4. Each one is a prefix no later session can share. A running session keeps the version it started with, so the remedy is batching edits into one commit near session end, not editing less.
