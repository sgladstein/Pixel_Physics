...
**Always-loaded floor: ~25,290 tokens** — `CLAUDE.md` at 101,161 B / 1,525 lines, bytes/4.0. Ceiling 28,000 (2,710 under). Plus ~430 for the hook, and the harness system prompt and tool schemas on top; this is a floor.

Paid by **every session, agent and subagent** — ten heads is ~252,900 tokens before any of them reads source.
...
Cache-prefix churn, distinct versions per day (newest first): 2026-09-20 x2, 2026-09-19 x2. Each one is a prefix no later session can share. A running session keeps the version it started with, so the remedy is batching edits into one commit near session end, not editing less.
