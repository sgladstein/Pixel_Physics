...
**Always-loaded floor: ~22,256 tokens** — `CLAUDE.md` at 89,025 B / 1,344 lines, bytes/4.0. Ceiling 28,000 (5,744 under). Plus ~430 for the hook, and the harness system prompt and tool schemas on top; this is a floor.

Paid by **every session, agent and subagent** — ten heads is ~222,560 tokens before any of them reads source.

Consulted by lookup, paid unconditionally: 62% (~13,863 tokens) across Method, Gotchas, Conventions. On demand instead, the floor would be ~8,800. That gap is the work; the ceiling only holds the line.

Cache-prefix churn, distinct versions per day (newest first): 2026-09-13 x1, 2026-09-12 x1, 2026-09-09 x1. Each one is a prefix no later session can share. A running session keeps the version it started with, so the remedy is batching edits into one commit near session end, not editing less.
