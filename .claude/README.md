...
**Always-loaded floor: ~23,213 tokens** — `CLAUDE.md` at 92,853 B / 1,397 lines, bytes/4.0. Ceiling 28,000 (4,787 under). Plus ~430 for the hook, and the harness system prompt and tool schemas on top; this is a floor.

Paid by **every session, agent and subagent** — ten heads is ~232,130 tokens before any of them reads source.

Consulted by lookup, paid unconditionally: 61% (~14,144 tokens) across Method, Gotchas, Conventions. On demand instead, the floor would be ~8,800. That gap is the work; the ceiling only holds the line.

Cache-prefix churn, distinct versions per day (newest first): 2026-09-14 x2, 2026-09-13 x5, 2026-09-12 x3, 2026-09-08 x1, 2026-09-05 x5. Each one is a prefix no later session can share. A running session keeps the version it started with, so the remedy is batching edits into one commit near session end, not editing less.
