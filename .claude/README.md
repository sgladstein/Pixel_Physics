...
**Always-loaded floor: ~21,502 tokens** — `CLAUDE.md` at 86,009 B / 1,310 lines, bytes/4.0. Ceiling 28,000 (6,498 under). Plus ~430 for the hook, and the harness system prompt and tool schemas on top; this is a floor.

Paid by **every session, agent and subagent** — ten heads is ~215,020 tokens before any of them reads source.
...
Cache-prefix churn, distinct versions per day (newest first): 2026-09-05 x5, 2026-09-03 x1. Each one is a prefix no later session can share. A running session keeps the version it started with, so the remedy is batching edits into one commit near session end, not editing less.
