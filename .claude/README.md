...
**Always-loaded floor: ~23,884 tokens** — `CLAUDE.md` at 95,534 B / 1,443 lines, bytes/4.0. Ceiling 28,000 (4,116 under). Plus ~430 for the hook, and the harness system prompt and tool schemas on top; this is a floor.

Paid by **every session, agent and subagent** — ten heads is ~238,840 tokens before any of them reads source.

Consulted by lookup, paid unconditionally: 63% (~15,049 tokens) across Method, Gotchas, Conventions. On demand instead, the floor would be ~8,800. That gap is the work; the ceiling only holds the line.
