...
| `G` | **Prototype.** Cycle how a liquid's brightness grain is generated: position (current) → cell → muted → animated → motion → animated-muted → animated-smooth. The animated variants defeat the dirty-rect render skip: measured on a *settled* world, the stepped ones cost ~2.0 ms mean (they only redraw on the frames their step changes) and animated-smooth ~10.4 ms, against 0.000 ms for every other mode. Exists so the variants can be judged on real moving water rather than argued about; the active one is shown in the title bar. See `render::GrainMode`, and expect this key and the enum to disappear together once one is chosen. |
