...
Generated by `scripts/readmetoc.py` from the `##` headings — 74 of
...
|---|---|
| [Running](#running) | 174 |
| [Finding things](#finding-things) | 190 |
| [Controls](#controls) | 226 |
| [Materials](#materials) | 297 |
| [Architecture](#architecture) | 371 |
| [The coarse field grid](#the-coarse-field-grid) | 573 |
| [M12/M13 status](#m12m13-status) | 835 |
| [Canopy throughfall status — rain drips through a wood](#canopy-throughfall-status--rain-drips-through-a-wood) | 862 |
| [Liquid physics: compressible volume, not discrete occupied cells](#liquid-physics-compressible-volume-not-discrete-occupied-cells) | 991 |
| [M14 status](#m14-status) | 1017 |
| [M7 status](#m7-status) | 1165 |
| [M15 status](#m15-status) | 1194 |
| [M6 deferral](#m6-deferral) | 1325 |
| [M5 status](#m5-status) | 1335 |
| [Plant lines merged: the genome, and the ecology](#plant-lines-merged-the-genome-and-the-ecology) | 1430 |
| [The generation loop: plants die, seeds expire, slots come back](#the-generation-loop-plants-die-seeds-expire-slots-come-back) | 1500 |
| [The economy re-derived: standing tissue costs something](#the-economy-re-derived-standing-tissue-costs-something) | 1581 |
| [Stems draw a line: the growth walk renders its heading](#stems-draw-a-line-the-growth-walk-renders-its-heading) | 1789 |
| [Plants that stop: organs, determinacy, and a price on both](#plants-that-stop-organs-determinacy-and-a-price-on-both) | 1871 |
| [M16 status](#m16-status) | 1939 |
| [M17 status](#m17-status) | 2125 |
| [M18 status](#m18-status) | 2352 |
| [UI improvements — overnight run, section 9](#ui-improvements--overnight-run-section-9) | 2759 |
| [Live tunables panel — overnight run, section 10](#live-tunables-panel--overnight-run-section-10) | 2804 |
| [Rendering performance — overnight run, section 11](#rendering-performance--overnight-run-section-11) | 2872 |
| [Zoom-out resolution status — more pixels instead of fewer cells](#zoom-out-resolution-status--more-pixels-instead-of-fewer-cells) | 2892 |
| [Zoom-in styles status — what a cell's 64 pixels can say](#zoom-in-styles-status--what-a-cells-64-pixels-can-say) | 3061 |
| [M8 status — started, not complete](#m8-status--started-not-complete) | 3120 |
| [M9 status — the gnome](#m9-status--the-gnome) | 3233 |
| [M10 status — the worldgen half](#m10-status--the-worldgen-half) | 3739 |
| [Weather status](#weather-status) | 3909 |
| [The ant colony — status](#the-ant-colony--status) | 3926 |
| [Inheritance status — the growth program has no fallback under it](#inheritance-status--the-growth-program-has-no-fallback-under-it) | 4718 |
| [Parameter-genome status — a species file is a starting point, and it ships inert](#parameter-genome-status--a-species-file-is-a-starting-point-and-it-ships-inert) | 4821 |
| [M19 status — started](#m19-status--started) | 4929 |
| [Felling status — the verb works, and what it produces is pieces](#felling-status--the-verb-works-and-what-it-produces-is-pieces) | 5001 |
| [Bending status — soft tissue lies over, and the wind is what pushes it](#bending-status--soft-tissue-lies-over-and-the-wind-is-what-pushes-it) | 5238 |
| [Breaking status — a badly grown tree comes down on its own](#breaking-status--a-badly-grown-tree-comes-down-on-its-own) | 5315 |
| [Lab lighting status — the fixtures are what light the crop](#lab-lighting-status--the-fixtures-are-what-light-the-crop) | 5508 |
| [Cell page status — the specimen readout is in three groups, and folds](#cell-page-status--the-specimen-readout-is-in-three-groups-and-folds) | 5635 |
| [Roster status — every plant and every animal, as a list you click through](#roster-status--every-plant-and-every-animal-as-a-list-you-click-through) | 5683 |
| [Plain-speech status — the genome read back as sentences](#plain-speech-status--the-genome-read-back-as-sentences) | 5914 |
| [Watch status — where one individual has been, and how its numbers moved](#watch-status--where-one-individual-has-been-and-how-its-numbers-moved) | 6039 |
| [Side-by-side status — two individuals, with what differs marked](#side-by-side-status--two-individuals-with-what-differs-marked) | 6104 |
| [Lineage overlay status — which founding line is taking the bed](#lineage-overlay-status--which-founding-line-is-taking-the-bed) | 6166 |
| [Creature groups status — who is who in the box, and who is family](#creature-groups-status--who-is-who-in-the-box-and-who-is-family) | 6246 |
| [Life record status — what an individual has done, and what killed it](#life-record-status--what-an-individual-has-done-and-what-killed-it) | 6668 |
| [Specimen shelf status — an individual's genetics outlive the box](#specimen-shelf-status--an-individuals-genetics-outlive-the-box) | 6776 |
| [Lab hand-verbs status — what a click puts in the box, and what the view shows](#lab-hand-verbs-status--what-a-click-puts-in-the-box-and-what-the-view-shows) | 6838 |
| [Lab parameters status — a save that reaches the founders, not just the file](#lab-parameters-status--a-save-that-reaches-the-founders-not-just-the-file) | 6926 |
| [Performance](#performance) | 6976 |
| [World speed — five independent time axes](#world-speed--five-independent-time-axes) | 7173 |
| [Status](#status) | 7256 |
| [License](#license) | 7367 |
| [Lab speed-dial status — what the dial is actually short of](#lab-speed-dial-status--what-the-dial-is-actually-short-of) | 7391 |
| [Lab scenarios status — a saved starting box with a question written on it](#lab-scenarios-status--a-saved-starting-box-with-a-question-written-on-it) | 7548 |
| [Soil nutrient status — ground is worth something water is not](#soil-nutrient-status--ground-is-worth-something-water-is-not) | 7651 |
| [Lab soil status — a hole that stays a hole anywhere in the bed, and a bed that stops running out](#lab-soil-status--a-hole-that-stays-a-hole-anywhere-in-the-bed-and-a-bed-that-stops-running-out) | 7694 |
| [Chronicle status — the box tells you who did what, at any population](#chronicle-status--the-box-tells-you-who-did-what-at-any-population) | 7851 |
| [Event reaction status — the box calls you back](#event-reaction-status--the-box-calls-you-back) | 7919 |
| [Life marks status — a pause-time mark, and what the owner rejected](#life-marks-status--a-pause-time-mark-and-what-the-owner-rejected) | 7959 |
| [Hunger status — a fed ant rests, a hungry one walks, and the founding cliff is walking cost](#hunger-status--a-fed-ant-rests-a-hungry-one-walks-and-the-founding-cliff-is-walking-cost) | 7994 |
| [Hopper status — the jump has a species, and it hops itself to death](#hopper-status--the-jump-has-a-species-and-it-hops-itself-to-death) | 8074 |
| [Rest status — a rest that ends, so a resting animal is not a stopped one](#rest-status--a-rest-that-ends-so-a-resting-animal-is-not-a-stopped-one) | 8105 |
| [Creature tools status — a hand in the box: scent, alarm, fling, lamp](#creature-tools-status--a-hand-in-the-box-scent-alarm-fling-lamp) | 8179 |
| [Trophallaxis status — the colony's stomach, shipped on, and a first measurement that says wait](#trophallaxis-status--the-colonys-stomach-shipped-on-and-a-first-measurement-that-says-wait) | 8236 |
| [Stacking status — many ants of one colony in one cell, off by default](#stacking-status--many-ants-of-one-colony-in-one-cell-off-by-default) | 8294 |
| [Trail status — the ant reads its way home, and the food trail is the larder's problem](#trail-status--the-ant-reads-its-way-home-and-the-food-trail-is-the-larders-problem) | 8348 |
| [Trail lifetime status — the way home keeps, the food trail still fades](#trail-lifetime-status--the-way-home-keeps-the-food-trail-still-fades) | 8385 |
| [Lifespan status — an ant gets old, and the colony's fall becomes a slope](#lifespan-status--an-ant-gets-old-and-the-colonys-fall-becomes-a-slope) | 8460 |
| [Held world status — the third game, and time you carry in a circle](#held-world-status--the-third-game-and-time-you-carry-in-a-circle) | 8606 |
| [Colony books status — what each colony is living on, in joules](#colony-books-status--what-each-colony-is-living-on-in-joules) | 9514 |
| [Food road status — the trail the colony actually walks, and the box that was not there](#food-road-status--the-trail-the-colony-actually-walks-and-the-box-that-was-not-there) | 9645 |
| [Field wake status — the field solves for the ants, and only one half of the fix is free](#field-wake-status--the-field-solves-for-the-ants-and-only-one-half-of-the-fix-is-free) | 9700 |
...
| M | Section | Line |
|---|---|---|
| 5 | [M5 status](#m5-status) | 1335 |
| 6 | [M6 deferral](#m6-deferral) | 1325 |
| 7 | [M7 status](#m7-status) | 1165 |
| 8 | [M8 status — started, not complete](#m8-status--started-not-complete) | 3120 |
| 9 | [M9 status — the gnome](#m9-status--the-gnome) | 3233 |
| 10 | [M10 status — the worldgen half](#m10-status--the-worldgen-half) | 3739 |
| 12 | [M12/M13 status](#m12m13-status) | 835 |
| 14 | [M14 status](#m14-status) | 1017 |
| 15 | [M15 status](#m15-status) | 1194 |
| 16 | [M16 status](#m16-status) | 1939 |
| 17 | [M17 status](#m17-status) | 2125 |
| 18 | [M18 status](#m18-status) | 2352 |
| 19 | [M19 status — started](#m19-status--started) | 4929 |
...
[Status](#status), line 7256 — the *last* section in the
...
|---|---|---|
| **plants, trees and moss** | engine | [M16 status](#m16-status) 1939, [Soil nutrient status](#soil-nutrient-status--ground-is-worth-something-water-is-not) 7651, [Plant lines merged](#plant-lines-merged-the-genome-and-the-ecology) 1430, [Inheritance status](#inheritance-status--the-growth-program-has-no-fallback-under-it) 4718, [Parameter-genome status](#parameter-genome-status--a-species-file-is-a-starting-point-and-it-ships-inert) 4821, [The economy re-derived](#the-economy-re-derived-standing-tissue-costs-something) 1581, [Plants that stop](#plants-that-stop-organs-determinacy-and-a-price-on-both) 1871, [The generation loop](#the-generation-loop-plants-die-seeds-expire-slots-come-back) 1500, [Stems draw a line](#stems-draw-a-line-the-growth-walk-renders-its-heading) 1789, [Felling status](#felling-status--the-verb-works-and-what-it-produces-is-pieces) 5001, [Bending status](#bending-status--soft-tissue-lies-over-and-the-wind-is-what-pushes-it) 5238, [Breaking status](#breaking-status--a-badly-grown-tree-comes-down-on-its-own) 5315, [Specimen shelf status](#specimen-shelf-status--an-individuals-genetics-outlive-the-box) 6776, [Cell page status](#cell-page-status--the-specimen-readout-is-in-three-groups-and-folds) 5635, [Roster status](#roster-status--every-plant-and-every-animal-as-a-list-you-click-through) 5683, [Plain-speech status](#plain-speech-status--the-genome-read-back-as-sentences) 5914, [Life record status](#life-record-status--what-an-individual-has-done-and-what-killed-it) 6668 |
| **creatures — worms and the ant colony** | engine | [M18 status](#m18-status) 2352, [The ant colony](#the-ant-colony--status) 3926, [Specimen shelf status](#specimen-shelf-status--an-individuals-genetics-outlive-the-box) 6776, [Roster status](#roster-status--every-plant-and-every-animal-as-a-list-you-click-through) 5683, [Plain-speech status](#plain-speech-status--the-genome-read-back-as-sentences) 5914, [Lab hand-verbs status](#lab-hand-verbs-status--what-a-click-puts-in-the-box-and-what-the-view-shows) 6838, [Life record status](#life-record-status--what-an-individual-has-done-and-what-killed-it) 6668, [Lab soil status](#lab-soil-status--a-hole-that-stays-a-hole-anywhere-in-the-bed-and-a-bed-that-stops-running-out) 7694, [Hunger status](#hunger-status--a-fed-ant-rests-a-hungry-one-walks-and-the-founding-cliff-is-walking-cost) 7994, [Rest status](#rest-status--a-rest-that-ends-so-a-resting-animal-is-not-a-stopped-one) 8105, [Hopper status](#hopper-status--the-jump-has-a-species-and-it-hops-itself-to-death) 8074, [Trophallaxis status](#trophallaxis-status--the-colonys-stomach-shipped-on-and-a-first-measurement-that-says-wait) 8236, [Stacking status](#stacking-status--many-ants-of-one-colony-in-one-cell-off-by-default) 8294, [Trail status](#trail-status--the-ant-reads-its-way-home-and-the-food-trail-is-the-larders-problem) 8348, [Trail lifetime status](#trail-lifetime-status--the-way-home-keeps-the-food-trail-still-fades) 8385, [Lifespan status](#lifespan-status--an-ant-gets-old-and-the-colonys-fall-becomes-a-slope) 8460, [Colony books status](#colony-books-status--what-each-colony-is-living-on-in-joules) 9514, [Food road status](#food-road-status--the-trail-the-colony-actually-walks-and-the-box-that-was-not-there) 9645 |
| **structural collapse, felling and rigid bodies** | outdoor | [M17 status](#m17-status) 2125, [Felling status](#felling-status--the-verb-works-and-what-it-produces-is-pieces) 5001, [Bending status](#bending-status--soft-tissue-lies-over-and-the-wind-is-what-pushes-it) 5238, [Breaking status](#breaking-status--a-badly-grown-tree-comes-down-on-its-own) 5315, [M8 status](#m8-status--started-not-complete) 3120 |
| **fire, heat and phase change** | engine | [M14 status](#m14-status) 1017, [Materials](#materials) 297 |
| **explosions, particles and debris** | outdoor | [M15 status](#m15-status) 1194, [M7 status](#m7-status) 1165 |
| **liquids and gases** | engine | [Liquid physics](#liquid-physics-compressible-volume-not-discrete-occupied-cells) 991, [Canopy throughfall status](#canopy-throughfall-status--rain-drips-through-a-wood) 862, [The coarse field grid](#the-coarse-field-grid) 573 |
| **powders and granular flow** | engine | [Materials](#materials) 297, [Architecture](#architecture) 371, [Lab soil status](#lab-soil-status--a-hole-that-stays-a-hole-anywhere-in-the-bed-and-a-bed-that-stops-running-out) 7694 |
| **the coarse field grid — pressure, heat, light** | engine | [The coarse field grid](#the-coarse-field-grid) 573, [M12/M13 status](#m12m13-status) 835, [Lab lighting status](#lab-lighting-status--the-fixtures-are-what-light-the-crop) 5508, [Field wake status](#field-wake-status--the-field-solves-for-the-ants-and-only-one-half-of-the-fix-is-free) 9700 |
| **worldgen and world structure** | outdoor | [M10 status](#m10-status--the-worldgen-half) 3739, [Architecture](#architecture) 371 |
| **the gnome (player character)** | outdoor | [M9 status](#m9-status--the-gnome) 3233, [Controls](#controls) 226 |
| **weather, sky and the clock** | engine | [Weather status](#weather-status) 3909, [M19 status](#m19-status--started) 4929, [World speed](#world-speed--five-independent-time-axes) 7173 |
| **rendering, UI and tunables** | engine | [Zoom-out resolution status](#zoom-out-resolution-status--more-pixels-instead-of-fewer-cells) 2892, [Zoom-in styles status](#zoom-in-styles-status--what-a-cells-64-pixels-can-say) 3061, [UI improvements](#ui-improvements--overnight-run-section-9) 2759, [Live tunables panel](#live-tunables-panel--overnight-run-section-10) 2804, [Rendering performance](#rendering-performance--overnight-run-section-11) 2872, [M6 deferral](#m6-deferral) 1325 |
| **performance and the parallel sweep** | engine | [Performance](#performance) 6976, [M5 status](#m5-status) 1335, [Architecture](#architecture) 371, [Rendering performance](#rendering-performance--overnight-run-section-11) 2872, [Lab speed-dial status](#lab-speed-dial-status--what-the-dial-is-actually-short-of) 7391 |
| **materials and the data schema** | engine | [Materials](#materials) 297, [M12/M13 status](#m12m13-status) 835 |
| **the evolution lab — the box and its lights** | lab | [Lab lighting status](#lab-lighting-status--the-fixtures-are-what-light-the-crop) 5508, [Lab soil status](#lab-soil-status--a-hole-that-stays-a-hole-anywhere-in-the-bed-and-a-bed-that-stops-running-out) 7694 |
| **reading one specimen off the screen** | lab | [Cell page status](#cell-page-status--the-specimen-readout-is-in-three-groups-and-folds) 5635 |
| **keeping, cloning and mutating an individual** | lab | [Specimen shelf status](#specimen-shelf-status--an-individuals-genetics-outlive-the-box) 6776 |
| **finding one individual among all of them** | lab | [Roster status](#roster-status--every-plant-and-every-animal-as-a-list-you-click-through) 5683 |
| **the genome in plain words** | lab | [Plain-speech status](#plain-speech-status--the-genome-read-back-as-sentences) 5914 |
| **what an individual has done, and what killed it** | lab | [Life record status](#life-record-status--what-an-individual-has-done-and-what-killed-it) 6668 |
| **watching one individual over time** | lab | [Watch status](#watch-status--where-one-individual-has-been-and-how-its-numbers-moved) 6039 |
| **comparing two individuals** | lab | [Side-by-side status](#side-by-side-status--two-individuals-with-what-differs-marked) 6104 |
| **which founding line is winning** | lab | [Lineage overlay status](#lineage-overlay-status--which-founding-line-is-taking-the-bed) 6166 |
| **who is who in the box, colonies and the rivalry rule** | lab | [Creature groups status](#creature-groups-status--who-is-who-in-the-box-and-who-is-family) 6246 |
| **putting things in the box, and what the view shows** | lab | [Lab hand-verbs status](#lab-hand-verbs-status--what-a-click-puts-in-the-box-and-what-the-view-shows) 6838 |
| **the chronicle: names, line events, and a clock that reacts** | lab | [Chronicle status](#chronicle-status--the-box-tells-you-who-did-what-at-any-population) 7851, [Event reaction status](#event-reaction-status--the-box-calls-you-back) 7919, [Life marks status](#life-marks-status--a-pause-time-mark-and-what-the-owner-rejected) 7959 |
| **a hand in the box: scent, alarm, fling, lamp** | lab | [Creature tools status](#creature-tools-status--a-hand-in-the-box-scent-alarm-fling-lamp) 8179 |
| **the speed dial, and what a tick costs** | lab | [Lab speed-dial status](#lab-speed-dial-status--what-the-dial-is-actually-short-of) 7391 |
| **tuning and saving the box's own numbers** | lab | [Lab parameters status](#lab-parameters-status--a-save-that-reaches-the-founders-not-just-the-file) 6926 |
| **a saved starting box, and replicating it in a rack** | lab | [Lab scenarios status](#lab-scenarios-status--a-saved-starting-box-with-a-question-written-on-it) 7548 |
| **the held world — the third game, and time you carry** | held | [Held world status](#held-world-status--the-third-game-and-time-you-carry-in-a-circle) 8606 |
...
cargo run --release --bin lab      # the evolution lab
cargo run --release --bin druid    # the held world
cargo run --example ascii          # headless terminal view, no GPU needed
cargo test
```

Three games share this engine, and which one a README topic belongs to is the
`Game` column in [By topic](#by-topic) above. `engine` is most of them and is
shared by all three; `outdoor`, `lab` and `held` mark what the other two never
reach. Why one repository holds them all:
[`Reports/two-games-one-repo-2026-08-30.md`](Reports/two-games-one-repo-2026-08-30.md).
...
| `+` (shift `=`), zoomed **out** | **How many of the cells in view get a pixel of their own** — cycles `App::pixel_budget` x1 / x2 / x4. At the widest zoom-out the view spans 2048x1280 cells through a 512x320 back-buffer, so 94% of them cannot reach a pixel at all and `ZoomOutFilter` above only decides which of each sixteen wins. This grows the buffer instead — x4 carries every cell at one pixel each — bounded by what the window can actually display, because pixels past that are paid for and then discarded by the GPU. **x1 is the default and is bit-identical to every build before 2026-09-13.** Measured on the shipped path (`App::update` + `App::draw`) at **1.29x** the whole frame for x2 and **1.66x** for x4 (`Reports/zoom-out-resolution-2026-09-13.md`). Same key as the zoom-in styles below because the two are never live at once |
| `+` (shift `=`), zoomed **in** | **What the game looks like when the view is zoomed in** — cycles `render::MagnifyStyle`. Past 1x a cell owns a `zoom`x`zoom` block of pixels, and this decides what it spends them on: `CELL-ART` (the default, and byte-identical to the build before this existed) a flat square; `PAINTED` bilinear between cell centres with a grain over the masses, the look the owner picked on card `20260912T044316700Z-4abaa9`; `PAINTED+INK` the same softness with each sub-pixel assigned to a *class* before it is coloured and an ink line where a mass meets air, which is the *"combine C and D"* he asked for and the only one of the five that keeps **more** of a one-cell twig than today; `ILLUSTRATED` flat fills with the same ink; `CHAMFER` the pixel look with its staircases cut. Named on the status line whenever the view is zoomed in, silent otherwise. Shift on the zoom-*in* key, mirroring `_` above |
| `{` (shift `[`) | The chamfer's concave-corner rule — `render::NotchRule`, `FILL` / `DEEP` / `CUT`. Does nothing outside `CHAMFER` |
| `}` (shift `]`) | How heavy the ink line is. **One of its stops turns the ink off entirely**, which is the control that says how much of the look is the line and how much the class field under it |
| `Space` | Pause |
...
| `F7` | Cycle the worldgen preset, keeping the seed — arid → canyon → druid → flat → rolling → terraced → wetland → legacy. Alphabetical with `legacy` appended, because `WorldgenPresets::presets` is a `BTreeMap` and `cycle_order` does not re-sort; this row named a different order and omitted `druid` (the held world's country, `src/druid`) until 2026-09-13. `flat` is the structural test bed |
...
src/sim/     the simulation — knows nothing about windows or GPUs
  ...
  fxhash.rs    a fixed-seed FxHash and the ChunkMap/ChunkSet/PosMap aliases
               the hot maps are keyed through -- deterministic across runs,
               a few ns against SipHash's tens on a chunk key
  ...
  writewatch.rs what has been written since the creature pass's speculation
               window opened -- the validity test that lets a creature's
               read phase be computed off one shared &World ahead of its
               turn while every write stays serial. Off by default; see
               Reports/evolution-lab-creature-parallelism-2026-09-13.md
...
src/food_road.rs
               where a colony's food comes from and how it moves: a
               decaying per-cell record of where animals walked *carrying*,
               and a coarse per-colony map of the regions food has been
               taken out of. Observed from the tick loop, drawn by
               `render.rs`, cycled by `F7` in the lab. Off by default and
               free then
...
**Water that lands on living tissue works its way down through it, a drop at
a time, instead of standing on the crown for ever.**
`update::drip_through_organism` is the liquid half of the powder rule
`fall_through_organism` has had since litter shipped, and rests on the same
argument that function already states: *the world is one vertical slice of a
3D wood, a branch one cell wide is not a shelf spanning the tree's whole
depth.* `update_liquid` had no equivalent — it tries straight down, a lateral
descent, both diagonals and fill transfer, and every one of those needs a
target it can enter, so water on a crown simply sat.

**How bad it was**, `scene=canopyrain` (new) under pinned rain: **89–96% of
every liquid cell in the world stood on living tissue**, mean 55–70 rows above
the floor and as high as 107. Almost no rain reached the ground in a wood.

**A rate, not a teleport.** `DRIP_PERIOD_FRAMES = 8`, staggered by column so
a wood drips rather than shedding in flat curtains, and driven off the frame
number rather than an RNG draw so it costs nothing and perturbs no stream.
Passing water straight through would empty a crown the instant the rain
stopped, which is the binary outcome the ethos rules out; the middle is a
wood that keeps dripping after the sky clears.

Rate sweep, all arms from one binary via `CANOPY_DRIP`, monotonic in both
columns — share of liquid on tissue 90–96% → **61–69%** at the shipped
period, mean height 55–70 rows → **28–51**. After the rain stops, standing
water falls to 9 cells against 20 with no drip, and the no-drip arm plateaus
where this one keeps falling.

**It never displaces tissue** — the landing cell must be genuinely empty, so
this can no more overwrite a leaf than a falling leaf can. That is what makes
it far simpler than the creature work it follows, where a body has to *be*
somewhere and displacing a plant cell cost the plant its anchor.

**Cost is below the noise floor**: three alternating paired `ascii` runs give
off 118.0 / 120.4 / 121.9 s against on 120.1 / 118.6 / 122.0 s, interleaved.
The gate is one branch on a `Cell` the caller already holds.

### …and the far side of a leaf is ground as often as it is air

**The rule above was written for a tree and was only ever correct for one.**
A tree has open air under its crown; a grass tussock, a herb, a shrub or a
scrambler lying along the floor has the *bed* under its tissue, so the scan
found soil, refused, and the drop stayed on the leaf for ever. The owner
reported it a second time, in the evolution lab and in those words: *water
pools on top of the plants instead of going through and soaking into the
soil.*

**How bad it was**, replaying the scan over every standing drop on
`played_bed_scrambler` at 24,000 frames (`examples/waterstand`, new): **876
liquid cells resting on living tissue and 8 of them able to drip.** Of the
868 refused, 308 were stopped by the soil under the mat — **276 of those
over ground with room to spare** — and 500 by water already trapped in the
mat. It is not a rain-rate artifact: at the mister OFF, on condensation
alone, it is 408 resting with 3 able to move.

**Ground that can hold water is a landing.** `update::soak_into_ground` runs
the identical arithmetic `update_soil_water`'s own infiltration uses —
`min(fill, room)`, remainder written back as liquid rather than the cell
consumed whole — so the drop enters the ground the one way water ever
enters ground. **Soil at capacity still refuses**, which is what keeps this a
graded outcome rather than a hole in the bottom of the world: rain soaks in
until the bed has had enough and then it stands.

Paired, same seed, same bed, 24,000 frames at the mister's shipped LIGHT:
standing water above the soil line **1,222 cells → 401**, fill above the
soil line **932,589 → 159,148**, drops held up over ground that had room
**242 → 11**. Cost measured paired and alternating on `ascii` with
`RAYON_NUM_THREADS` pinned, three runs each: mean **0.695 → 0.689 ms**, and
the worst frame (7.24 / 6.71 / 8.56 before, 6.43 / 6.50 / 6.42 after) sits
inside the baseline's own spread and is not quotable either way. The hot
path is unchanged — a drop not sitting on tissue still returns on the first
branch.

The guard is `a_puddle_on_a_mat_soaks_into_the_ground_but_not_into_stone_or_a_full_bed`,
four arms, three of them controls, **every one confirmed red for its own
fault**: the mechanism removed, stone counted as ground, saturated ground
drinking anyway, the opt-in flag ignored, and the cell consumed whole. Two
of those controls were written as *"some of it is still up there"* first and
were **blind** — 79% of the puddle went into stone and 11% into a full bed
with all three green. They assert the placed volume exactly now.

### …and it took three rounds, the last of them the owner's

**"This is not fully fixed. water is still pooling on top of plants."** Right,
and the fall above had the residue in it. Three more things were holding
water, measured at the shipped rain rate on 213 drops standing on tissue:

- **41 refused by *dead* grass.** The entry gate read `organism_id`, so a drop
  on a dead `grassblade` — which a grown bed is full of — never entered the
  rule at all. It had **two** gates, and teaching the scan to walk through
  plant matter moved nothing until the gate was fixed too.
- **66 refused by a trapped drop.** Leaving it to drain on its own beat works
  only while it has somewhere to go; over full ground it has none and the
  column stacks behind it for ever. `soak_into_ground` merges now.
- **Soil over an ant-dug tunnel, the owner's own diagnosis:** *"water doesn't
  drop down out of soil into air, so a single pixel line of soil gets
  saturated and then water can pool on top of it."* One clause —
  `update_soil_water`'s drainage required the cell below to *hold* water. A
  wet roof drips into the void now, at the same rate and off the same surplus
  as the soil-to-soil branch; **damp ground still does not leak**, which is
  what keeps it *a wet roof drips* rather than *soil leaks*. Perched cells on
  `played_bed_scrambler`: 1 of 1 saturated → **0 of 7**.

`deadwood`, `log` and `nest` still dam a drop on purpose — a snapped branch is
chunky enough to hang up in a crown, which is `fall_through_organism`'s own
existing call for the falling side.

**What is left is not this rule.** Of 158 drops still standing, **72 are over
genuinely saturated ground** and 53 over water that is itself full. The bed
under a plant has three doors and all are shut: no evaporation
(`is_damp_soil_surface` refuses a covered column), no sideways spread (the
capillary rest threshold), and drainage only downward into a sealed box.
`World::soil_capillary_levels` is a measurable lever on the second — standing
water **397 → 220** — and is off by owner ruling; evaporation under cover is
untouched and is the larger suspect.
`Reports/canopy-throughfall-2026-09-07.md` §8.

**Known limitations.** Interception is not modelled — a real crown holds a
film and loses some to evaporation; here every drop eventually goes through.
A drop still gives up at the first cell that is neither air, nor organism,
nor ground that can drink it, so dead tissue standing in a mat (`grassblade`,
`grassroot`, a fallen `log`) and water already trapped below still dam a
column — 47 and 500 cells respectively on the bed above. Stemflow is now
*partly* modelled by accident and only where it helps: water on a trunk with
soil within the 16-cell reach below goes into the ground, which is what
stemflow does, but nothing runs down a trunk longer than that.
...
`organism_id` back instead of consuming one of the world's slots forever.
...
across a full 4,095-organism world and moves nothing — draws are keyed
...
**The organism-slot ceiling is a real check, and it is no longer 4,095.**
`Cell::organism_id` gave 12 bits to the slot index and the encoder does not
mask, so a 4,096th organism silently became a live one in release builds;
the only guard was a `debug_assert`. `World::push_organism` returns `Option`
— refusing the birth, counting it in `organisms_refused`, and letting the
compiler make every caller decide — which is
`population-dynamics-research.md` 9g's ask in its own words.

**The ceiling itself moved to 1,048,575 on 2026-09-14**, because 4,095 was
being hit in play: the held world's grown and dead starts produced 4,093
organisms in their grow phase and `C` — the only verb in that game that
makes an animal — founded nothing, while the bar blamed the ground
(`Reports/open-bugs-handoff.md` §Z21). `Cell` widened 12 → 16 bytes and
`organism_id` became a `u32` split **20 bits index / 12 bits generation**,
so the generation wrap that bounds stale-handle aliasing went from 16
reuses to 4,096 — the widening makes that safer rather than trading it
away, which the alternative (re-splitting the old `u16` 13/3 for 8,191
slots) would not have.

**What it cost, measured paired and alternating rather than argued.**
Memory: the cell grid at the shipped 8192x2560 world goes **240 → 320 MiB**,
+33% on the program's largest allocation — that is the number to weigh.
Frame cost: `frame_profile` on the shipped world, settled block, three
alternating rounds of two fixed binaries — `ca_sweep` p50 median 5.51 →
5.71 ms (+4%), whole-**frame mean 19.05 → 18.96 ms, i.e. unmoved**, since
the frame is 47% draw and 22% fields. `examples/ascii`'s worst-frame could
not separate the arms at all (the 16-byte build won 3 of 6 pairs), the
order-statistic-is-noise case `CLAUDE.md` warns about.

**Every readout that spelled 4,095 was a latent bug and they are fixed
together**: the lab's slot row coloured amber at a hardcoded `>= 4000`
(0.4% of the new ceiling), its help string named the old number, and two
censuses scanned `1..4096` by hand — which was doubly wrong, since a bare
slot index carries generation 0, so those scans had only ever resolved
organisms in never-reused slots. All four now read
...
**And the store no longer grows without bound with that contact, because a
plant that can grow its own bucket cannot die of thirst.** Capacity is
`WATER_SCALE x contact_root_cells` and nothing limited it, so at
`WATER_SCALE` 4.0 the tank matched a mature tree's ~29-per-tick demand at
about **7** contact cells while the tree grows 250–320 — thirty-five ticks
of buffer where a real plant stores well under a day's transpiration.
`WATER_TANK_CONTACT_CAP` bounds the storage term at 32 contact cells and
touches nothing about what roots buy in *uptake*, which is the channel that
should matter. It is a band rather than a threshold: swept against
`a_tree_denied_water_dies_and_a_watered_one_does_not`, 8 and 16 stunt the
tree so the *watered* one dies, unbounded lets the *droughted* one live, and
32 and 64 both pass. Water is stored in sapwood, not in fine roots, so root
count was never the quantity storage should have scaled with.

**A root tip asks about the ground it is standing in, not about the tank.**
`break_root_tips` refused **95–99.6%** of root-tip initiations at
`water_status >= 0.95` while the carbon exit refused **none** — measured
across four beds and two genotype draws, root extension was bounded by
neither the tip cap, nor a shortage of sites, nor the economy, but by a
plant reading "demand met" while the soil its roots touched sat at **0.016**
available. `water_status` is `min(stock/demand, openness)`, so it is
ceiling-clipped and flat over most of the tank's range, and the tank is the
one the roots themselves inflate. The gate now reads
`OrganismState::root_zone_water` — the mean plant-available fraction over
the drinkable faces `contact_root_cells` already counts, accumulated inside
the four-neighbour look that was already running, so it adds no traversal.
**The threshold is not retuned**: 0.95 means the same thing on both
fraction-of-full scales.

Over 12 paired seeds on a 34-row bed, with the tank capped: root cells
**4.74x** and contact roots **4.13x**, both 12/12 seeds; **income +30%**
(9/12); total cells **+41%** (12/12); uptake +57%; the worst plant in each
bed 0.792 → **1.000**; water in the root zone 0.016 → **0.839**; and
**root:shoot 7.5% → 22.4%** against a real tree's 20–25%. The two switches
had to land together: alone, the gate is income-neutral and *fails*
`a_tree_denied_water_dies_and_a_watered_one_does_not`, because a thirsty
plant answers by rooting, which inflates the tank, which raises
`water_status`, which props up income — a droughted plant roots its way to a
bigger bucket and reads as well-watered.

**A ground collapse *is* real, and the mechanism first written here for it
was wrong. Both halves are kept, because the error is the instructive
part.** `filmstrip scene=grove` at 6,000 / 12,000 / 18,000 / 24,000 frames
showed the soil bed *and its stone floor* intact with the switches on and
gone by 18k with them off. That was written up as root-bound ground holding
a slope bare soil loses -- reasoned from a picture to a mechanism, which is
the failure `CLAUDE.md` names and §A records costing two wrong answers.

**What the scene is actually standing on.** `PlantScene::build` puts its bed
on a stone slab 512 wide and `STONE_DEPTH` = 6 thick, anchored only at the
two world edges -- `load.rs` reads `Cell::OUT_OF_BOUNDS` as bedrock -- **over
an 80-row void**, since `ground_y` 200 plus 34 rows of soil plus 6 of floor
leaves y 240..319 empty. Stone's `max_unsupported_span` is 16, so
`capacity_within`'s base is 16²/2 = **128** against a 3,072-cell section
spanning 512 cells. That slab cannot pass the load model. It is the
3,072-cell entry in every one of these runs' `hanging census`, and the
arithmetic is exact: 512 x 6.

**It survives by never being asked.** The control is a bare bed --
`filmstrip scene=grove plants=0`, which needed one change to `filmstrip` to
be expressible at all -- and over 24,000 frames it reports **zero**
`overloaded` and **zero** `unsupported` failures, `awake 0/40`, and the
3,072-cell cluster intact at every stop. Nothing disturbs the slab, so
`structural::tick` never evaluates it, and a latently impossible structure
stands indefinitely. **A falling tree is what wakes it**: `settled log
pieces` is zero in both arms until the first big fall, and the collapse
follows it -- the roots-off arm's first fall is at frame ~14,450 (10 pieces
holding 928 cells, largest a 619-cell log 45x74 standing upright),
`overloaded` first appears at 17k (19 failures, 1,002 cells), and by 18k it
is 553 failures over 6,032 cells with the floor cluster gone.

**And the roots-on arm is genuinely more robust, not merely later -- which
is a narrower claim than the original and survives the control.** Run out to
36,000 frames it carries **33 settled log pieces holding 1,568 cells** and
reports `overloaded 0` at 18k, 24k, 30k and 36k, with the 3,072-cell floor
intact throughout. It absorbs *more* fallen mass over *twice* the time than
the arm that collapsed. **What is not yet separated is why**: the roots-off
arm's largest piece was 619 cells and 74 rows tall against the roots-on
arm's 526 and 44, so a bigger single impact and a root-bound bed spreading
the load are both live, and one seed cannot tell them apart.

**Fixed the same day**, on the owner's go-ahead: `PlantScene::build` now
fills y `floor_bottom`..`height` with `BEDROCK`, so the slab has ground
under it. Deliberately *under* the floor rather than making the floor itself
bedrock -- `relax_one` roots a cell's distance at 0 beside bedrock, so a
bedrock floor would have silently anchored every root that reached it, a
plant behaviour change disguised as a scene repair. The arm that reliably
collapsed now reports `overloaded 0` and its floor intact at 6k / 12k / 18k
/ 24k *with* trees falling on it. **It is not inert** -- predicted
bit-identical, measured -0.4% at frame 6,000 on the default seed -- **but it
is unbiased**: six paired seeds at 12,000 frames give a median ratio of
**1.002**, larger on 4 of 6, range 0.982-1.175, one seed bit-identical.
`PIXEL_PHYSICS_SCENE_FLOOR=void` restores the old bed so any old number can
be re-taken paired; never compare across that switch unpaired.

**Known limitation: it is a depth-dependent win.****Known limitation: it is a depth-dependent win.****Known limitation: it is a depth-dependent win.** The gate only pays where
roots can reach soil no root has drunk. Measured over 12 paired seeds at
four bed depths, income off → on: **0.12x at 6 rows**, 0.92x at 12, **1.34x
at 20**, 1.00x at 34, ordered by the root-zone recovery it achieves (0.000 /
0.016 / 0.280 / 0.570). At six rows there is no "elsewhere" and the rule is
pure cost — but that bed is already marginal before any change, and both
shipped worlds are far deeper: `assets/worldgen.ron` gives 48–135 rows
outdoors and `lab::scene`'s `DEFAULT_SOIL_DEPTH` is 96. Fine-root turnover
was built as the counterweight for shallow beds and measured **worse at
every rate** — a shed-and-rebuild treadmill, since a bed with no
unexhausted soil has nowhere better to put a root. `Reports/dead-ends.md`
carries it with the condition its rejection depends on.
...
### M18: plants are not walls (2026-09-06)

**A body can push through living foliage, and the foliage closes behind it
undamaged.** Until now `landing_is_placeable` required `World::is_empty`, so
every cell of living tissue was a wall to every creature — while the *gnome*
has walked through and climbed living plants since M16 by owner ruling
(`wood.ron`: *"a growing tree is scenery you move past and go up, not a wall
you stop against"*). `Material::climbable` is read only by `player.rs`, so
that rule had simply never been extended to anything with legs.

**The mechanism is `organism::Parted`.** The grid holds one thing per cell,
so a body entering foliage must displace it, and the three options are
destroy it (an ant that eats a tunnel through every bush), move it (which
breaks the plant's own geometry) or hold it. `relocate_chain` holds it: it
lifts the tissue the new body position covers — the `Cell` **and** its
`OrganismCell` scalars, because `World::set`'s seam zeroes those whenever a
cell changes hands — and restores whatever the body no longer covers.
`creature_dies` returns the lot before the corpse is written, so a bush
closes over an animal that dies inside it.

**What counts as soft is data, never a material name**: `MaterialKind::Plant`,
`organism_id() != 0` (the same "is it alive" gate `climbable` is read behind,
so a painted `wood` wall is as solid as it ever was), and
`penetration_resistance <= dig_force` — the pattern roots and `act`'s dig gate
already use. `leaf`/`flower`/`moss` are 0.1, `fruit` 0.2, and `grassblade` is
now 0.1: it authored no resistance at all and so sat at the 100.0 default,
which priced a meadow like heartwood. `wood` and `rootwood` author none and
stay solid — an ant walks *on* a trunk, not through it. The force is the dig
allele, so a lineage that evolves a stronger bite gets through thicker growth
with no new gene.

**And a trunk is got round rather than entered.** `organism::Crossing`: a
body refused by living *woody* tissue looks along its heading for the first
place it could stand, waits `thickness x tick_interval` frames, and appears
there, charged the energy of the walk it replaced. It never occupies the
wood. Owner's design — *"they basically just teleport to the other side with
a delay long enough for however thick the trunk is, so they don't actually
overlap with the cells ever"* — and the justification is that a side-view
grid has no depth axis, so a trunk a real animal steps around is an unbroken
wall here and the time it takes to go round *is* that axis.

**Never occupying is the whole value, and that is measured.** Letting a body
occupy wood the way it occupies foliage works for movement and kills plants:
`plant::is_structural_anchor` resolves ownership through the grid, so an
animal standing in a stem stops that cell counting as an anchor and a
seedling with one base cell becomes an unanchored plant.
`lab::tests::copies_carry_what_was_planted_and_still_diverge` finishes at
`plant_cells 0` in all three copies with that on. Crossing has nothing to
displace, so the class of failure does not arise.

`scene=colony` seed 1, both arms from one binary via `CROSS_TRUNK=0`: blocked
steps **15.7% → 7.3%**, tissue blocking **656 → 216**, **243 crossings begun,
243 completed, 0 abandoned**, and limb severing 3,987 → 3,770 — plants
slightly better off. What still blocks is trunk encounters with no far side
to come out on, which is the rule declining rather than a gap.
`grassblade`/`grassroot` gained `climbable` here too; they authored none, so
a meadow was priced as a wall and was 35% of the residue.

**Coverage.** Both mechanisms sit in the shared `step_chain`/`relocate_chain`
path and gate on material data rather than species names, so every walking
species has them — Chain (`ant`, `ant_long`, `ancestor`) and Rigid
(`beetle`, `ant_block`, `ant_block_shaded`, `ant_wide`, `chitin_pale`).
**The worm has neither**: it runs on `worm_tick`/`move_cost`, a separate path
where `MaterialKind::Plant` returns `None`, and it moves by *overwriting* the
cell it enters, so making tissue enterable there would have it eat roots
rather than pass them. That is the open item.

**`examples/ascii` is byte-identical across this** — its creature scenes are
built on stone and sand with no living tissue beside the animals, so the gate
is blind to it. The evidence is `scene=hedge` and
`an_ant_walking_through_foliage_leaves_it_intact`, which censuses leaf count
*and* carbon and was watched failing for both of its faults.
...
**Shipped 2026-09-13 as a selector, default off.** The owner, from play:
*"why my screen resolution can solve all of the pixels, why cannot there just
be more pixels when you zoom out?"* He was right, and the constraint was never
the monitor. The renderer draws into a fixed 512x320 back-buffer (`WIDTH`/
`HEIGHT`) which `Pixels::new` then upscales to the window, so the screen's real
pixels **magnify** that image rather than carrying more of it. At
`MAX_ZOOM_OUT_STRIDE` the view spans 2048x1280 cells through those 512x320
pixels — sixteen cells per pixel, **fifteen of which have to be discarded**.

That discarding is the whole of §Z11. `ZoomOutFilter::Coverage` (2026-09-12)
answered *which of the sixteen wins*, and it works — nothing disappears any
more. What it cannot do is stop a one-cell stem being **drawn four cells wide**,
because the pixel it wins is four cells across. The dropout became an
exaggeration.

`App::pixel_budget` (`+` while zoomed out, x1 / x2 / x4) grows the buffer
instead, and `Renderer::pixel_scale` spends it. Three properties are worth
stating because they are what make it safe to carry through the renderer:

- **It buys resolution, never reach.** The scale is only ever a power of two
  that *divides* the zoom rung, so `sampling_stride = rung / scale` is exact and
  the span in cells is untouched. The stride cap carries three separate owner
  rulings and none of them is reopened. At rung 3 no power-of-two budget can be
  spent at all, and it buys nothing rather than widening the view by a third.
  **Since 2026-09-14 rung 3 is no longer a stop a player walks through** —
  `render::ZOOM_OUT_RUNGS` is `1, 2, 4` and `adjust_zoom` steps along it in
  both directions, on the owner's ruling *"get rid of stop 3"* (card
  `20260914T084458895Z-ae1b01`), because the blocky rung was the one no budget
  could sharpen. It survives as a **cap** rather than a stop — `zoom_within`
  clamps to `max_zoom_out_stride`, which can still land on 3 for a box needing
  exactly three screens to cover, and that is the case this bullet is about.
- **Every ordinary frame is unchanged.** At `zoom > 1`, at rung 1, and at
  budget 1 the scale is 1 and every derived quantity is what it was.
- **The window is the ceiling.** A buffer larger than the surface is paid for in
  full and then thrown away by the GPU on the way down, which would put the
  dropout back *below* `ZoomOutFilter` where the salience rule cannot reach it.
  `App::pixel_scale_cap` is set from the surface each frame; the player's
  choice is remembered rather than clobbered, so it comes back when the window
  grows.

**The HUD is the half of this that is not in the renderer.** Everything it
draws is positioned in fixed pixel coordinates, so in a 2048x1280 buffer all of
it would land in the top-left eighth at a quarter size. `render::Hud` keeps the
HUD drawing in logical pixels and turns each into a `scale`-square block; at
scale 1 it is the free functions it wraps, byte for byte.

**What it costs, measured before it was built**
(`Reports/zoom-out-resolution-2026-09-13.md`, `examples/zoomout_pixels.rs`):
**1.31-1.37x the whole frame at x2** and **1.79-2.07x at x4**, where x4 is
sixteen times the pixels. It is sub-linear because `pixels x stride²` is
constant, so cell reads do not move and only per-pixel colour work does.
Re-measured *after* building, on the real `App::update` + `App::draw` path with
the HUD in it, it is **1.29x and 1.66x** — a little cheaper, because the app's
frame carries fixed cost the subsystem harness does not and the growing part is
a smaller share of it.

**The lab has it too, and defaults to x4** (`lab::DEFAULT_PIXEL_BUDGET`) — the
same default as the sandbox, because **consistency between the two games is a
requirement**. It is the setting the owner picked when shown the full range.
**Both games *request* x4; what either displays depends on the window**, since a
buffer larger than the surface would be paid for and then discarded. At the
1024x640 default window that resolves to x2. **A refused request says so on
screen** — a line under the lab's clock naming the request, what it resolved to,
and whether the window or the zoom rung refused it, drawn only when the request
is not met. The
lab re-exports these same `WIDTH`/`HEIGHT` and drives the same `Renderer`, so it
had the identical 15-in-16 discard; `Lab::pixel_budget` is its copy of the
selector, on the same `+` key. Three things are specific to it:

- **The shipped 512x320 bed cannot zoom out at all**, so the budget buys nothing
  and costs nothing there — measured 1.00x the achieved tick rate at every
  budget. A box no bigger than the viewport has nothing to pull back from. Beds
  up to 2048x1280 show *whole* at one cell per pixel; `MAX_BOX` is 4096, so the
  top of the range does not.
- **The bar is interactive, which the sandbox's HUD is not.** The cursor arrives
  in buffer pixels and the bar is laid out in logical ones, so the conversion
  happens once at the window boundary (`Lab::to_logical`) and everything
  downstream — hit tests, hover, `press`/`drag`, and world lookups through
  `Renderer::logical_to_world` — is logical.
- **The cost is not amortised by the speed dial, and this reverses the obvious
  reasoning.** `TimeControl` gives each pass one wall-clock budget and the
  render comes out of it, so at dial 1 the bigger buffer is **free** and at
  fast-forward it costs **0.45-0.65x** of the achieved rate on a 1024x640 bed —
  most, exactly where it was expected to cost least. On a 2048x1280 bed x2 reads
  0.73-0.78x and x4 reads 0.40-0.44x. `examples/labzoom_cost.rs` is the
  instrument and `achieved` — simulated seconds per real second — is the unit,
  because frame milliseconds answer the wrong question about a box you run fast.

**Why does making off-screen cells visible cost anything, when they were being
simulated all along?** Asked directly, twice, and it is a fair question with a
short answer: **the simulation half does not move at all.** Those cells were
always being stepped, on or off screen, and nothing here changes that. What
grows is the *drawing*. The renderer does its per-**output-pixel** work — the
colour, the shade, the grain, the lighting — once per buffer pixel, so 512x320
is 164k of that work and 2048x1280 is 2.6M, plus a larger buffer handed to the
GPU each frame. **That is also why it is 1.66x and not 16x**: the simulation is
untouched, and *cell reads* are constant by construction (`pixels x stride²` is
the same product at every budget), so the only thing that grows is per-pixel
colour work — a minority of the frame.

**The one thing that was nearly got wrong, recorded because the mistake is
instructive.** The two review cards offered **different menus** — the lab card
was x1/x2/x4 and the real-game card was two panes, x1 against x2, so x4 was
never on offer in the game. Reading *"x2 in the game, x4 in the lab"* off that
pair reads the construction of the cards and calls it a preference. Given the
full range he picked the finest, and said so: *"I am not sure what questions
that I answered that suggests zoom should be different between the games, but
that doesn't seem like what I want."* **Both games default to x4, and
consistency between them is a stated requirement rather than a nice-to-have.**

**The held world has it too, from 2026-09-15, and it was the game this was
written for that never got it.** The druid (`--bin druid`) was being built by
another lane while this landed, so for two days it was the one game of the three
still drawing every frame into a fixed 512x320 buffer: at the widest rung it
alone took 2048x1280 cells through 164k pixels and discarded fifteen in sixteen,
which is exactly the complaint this whole section exists to answer. Reported
from play as *"it works well in the lab and looks really crisp when we zoom out,
but it doesn't look as good in the druid game"* — the same renderer, the same
`Coverage` filter, the same ladder, and only the buffer different.

`Druid::pixel_budget` is its copy of the selector, on the same `Shift`+`=` key
and at the same x4 default, and `bin/druid.rs` sizes the buffer and the cap the
way `main.rs` and `bin/lab.rs` already did. Three notes specific to it:

- **The interface was already written in logical pixels**, which made this much
  cheaper here than in the sandbox: every panel, the button bar and the
  biosphere page draw through `render::Hud` rather than at raw coordinates, so
  handing that canvas the scale was the whole of the HUD half. What did need
  changing is the **cursor** — `window_pos_to_pixel` lands in buffer pixels and
  every hit test is logical, so `Druid::to_logical` is the second half of the
  conversion, the same pair the lab runs. It was genuinely 1:1 before this.
- **`Druid::draw` stopped taking a viewport.** There are now two — the camera
  and the interface work in logical pixels, the renderer draws into buffer ones
  — and a caller handing one number to both is precisely the two-sources-of-
  truth shape that panicked the sandbox and silently halved the lab's zoom reach
  on the same afternoon. It derives both itself, so there is no ordering to get
  right.
- **A refused request says so here as well**, under the corner readout rather
  than on the bar: at the 1024x640 default window a request of x4 resolves to
  x2, and a player who cannot see which of the two refused it is being told
  nothing. `PIXEL_PHYSICS_DRUID_ZOOM_OUT=<rung>,<budget>` is the headless hook,
  the mirror of the sandbox's, since a captured frame at zoom-out is otherwise
  unreachable without a keyboard.

**So all three games now request x4 and spend it the same way**, which is what
the consistency requirement above actually asked for — it was written when there
were two.

**And the held world's default now has a verdict of its own, rather than an
inherited one.** The paragraphs above are careful that x4 arrived here by the
consistency ruling — the owner picked the finest buffer on a *lab* bed, and this
game took the same number without his eye ever having been on it. Blind A/B card
`20260915T214604023Z-16cab5` (board `druid`, the two panes normalised to one
on-screen size) put the widest zoom-out of the held world in front of him at x1
against the shipped x2-at-that-window, and he **chose the finer buffer**. So the
default is now his about this game as well, which closes the qualification the
lab's own section had to carry for two days.

**Known limitation.** On outdoor rock the gain is not obvious: the per-cell
shade jitter that gives stone its grain is drawn at 4x4 blocks at x1, and at one
cell per pixel that grain reads as **smooth** rather than as texture — more
correct, arguably less characterful, against a house style that is chunky on
purpose. On the lab bed the finer buffer plainly wins. **In the held world the
same caveat applies to its bare ground**, which is most of what a new world is —
the gain there is clearest on the skyline, the rock strata and the druid
herself, and it will grow as the player plants.

## Zoom-in styles status — what a cell's 64 pixels can say

**Shipped 2026-09-12, as a mode rather than as the new default.** Past 1x a
world cell owns a `zoom`x`zoom` block of screen pixels, and until now it spent
all of them on one colour: at 8x every cell is a flat square, which is the
*"big sharp squares"* complaint at eight times the size. `render::MagnifyStyle`
is the selector (`Shift`+`=`), and `Reports/evolution-lab-zoom-in-design-2026-09-12.md`
is the design of record behind it.

**The owner picked the direction by eye and it is quoted rather than
paraphrased** (card `20260912T044316700Z-4abaa9`): *"Could we maybe combine C
and D? I like D the best (although D and B are almost interchangeable) but D/B
look blurry. Maybe adding C or something else could crisp it?"* — D the painted
look, C the illustrated one. `PaintedInk` is that combination; `Painted` and
`Illustrated` ship beside it as its two ends, and `Chamfer` because the
companion card called it *"most interesting direction, i would have to
playtest"*. The sub-cell material texture (*"C-bad"*) and plain smoothing as a
mode of its own (*"probably not worth extra cost"*) are not here.

**How a style decides a pixel.** `Painted` is bilinear between cell centres
over the shipped colours, plus a position-keyed grain on the masses. The two
*class* styles do a pass first: each sub-pixel is assigned to the class (air /
liquid / powder / solid / plant / creature) whose bilinear occupancy clears
`MAGNIFY_LEVEL`, and only then coloured — `Illustrated` from the winning
class's nearest cell whole, `PaintedInk` from a blend **restricted to that
class**, so a twig's pixels are made of twig and never of the sky behind it.
Both then ink the pixels where a mass meets air. Class, never colour: the
deliberate per-cell shade jitter is exactly what defeats a colour-matching
upscaler, which is why hqx/xBR are in `dead-ends.md`.

**The ink is load-bearing, not decoration**, and the number says so. A
one-cell *diagonal* twig reaching the screen, as a fraction of today's look,
worst of 8 (`examples/magnify.rs mode=twigs`): painted **0.62–0.90** depending
on zoom, painted+ink **1.27–1.58**. A soft look alone reintroduces at play
scale the same *"pixels of plants disappearing"* the zoom-out coverage filter
had just removed at the other end of the scale; the class field's bias and the
ink line are what give it back.

**Cost.** Nine alternating reps inside one run with threads pinned, full
redraw at 3x: today 2.77 ms, painted 5.44, painted+ink 6.90, illustrated 5.79,
chamfer 3.39 — +25.2 ns per output pixel for painted+ink, well under the 48
ns/px the design lane's prototype measured for painted alone, because the 3x3
neighbourhood is hoisted per *cell* and nine `cell_colour` calls amortise to
one per pixel at 3x. A settled world recomputes **0** pixels under every
style, so the dirty-rect skip is untouched, and `magnifying()` is hoisted out
of the pixel loop so zoom 1 pays nothing.

**Known limitations.** The crack strip is the one rule inside `cell_colour`
that reads the sub-cell offset, and it does not draw under a style — a fissure
along one edge of a *block* is a per-block rule a curved boundary has to
restate. A one-cell hole inside a mass comes out as a diamond, because the
occupancy bias that fattens a thin mass thins a thin gap; `magnify_level` is
the dial and 0.5 is the unbiased setting. A neighbour across a chunk border
can change without dirtying this chunk, so a silhouette on a seam can hold a
stale ink pixel until that chunk redraws — real at zoom 2, moot at zoom 8
where a chunk is most of the screen. And the lab has no key for any of it:
`src/bin/lab.rs` owns the lab's input and `examples/labshot.rs style=` is the
way to see a style on that bed for now.
...
### Roots are ground, not scenery — 2026-09-13

**The forest floor holds him up.** Owner's report: *"roots are considered
plant instead of ground, so the gnome falls into the root mass instead of
walking over it like the ground."* A plant has been scenery he walks through
since M16, and roots are the half of a plant that is inside the bank he is
standing on — so every root cell in a forest floor was a hole. Measured on
`scene=wood`, the shipped grown stand: he walks **126 cells** into it, drops
through the floor into the root plate of the first big tree and does not move
for the remaining 2,000 frames of the run — five rows under the soil line
with **60 of his 98 cells inside living tissue**. With the rule: **496
cells**, the length of the world, and 0 under the soil line at rest.

**`Material::underfoot`**, authored on `rootwood` and `grassroot`, ANDed with
`Cell::organism_id() != 0` and read in `player::footing` — the same shape and
the same organism gate as `climbable`, a `Vec` index on a `Cell` the predicate
already holds. Tested *before* the climb arm, because `grip` compares
`Footing::Climb` by equality and reaching that arm first is a rope down into
the bank.

**`Footing::Soft`, not `Hard`, and that is the design call.** The bank is
powder: he sinks to the knee and stops, so a root answering the same way is
what makes the surface continuous. Standing him *on top* of a root would jump
the surface by the wade depth wherever one broke it — the same discontinuity
as falling in, pointing the other way — and it would put a wall across every
bore that clips a tree, which `wiki/the-gnome.md` names as something he walks
past. It is also the graded reading of three: `Hard` is on top of, `Climb` is
straight through, and only `Soft` has a middle.

**Nothing else changed, and that is the point of a flag over a `kind`.**
`MaterialKind::Plant` is read in 25 places — structural, fire, weather, the
light field, `world.rs`'s placement rules, `rigid::is_tool_target`, the colony
— and none of them sees this. The root materials also keep `climbable: true`,
which looks redundant and is not: it is still read by the shake's plant flood
(`shake_target`), `creature::crossable` and `render.rs`'s tree-depth
occlusion, and dropping it to mean "not a ladder" would silently retune all
three.

**And the acceptance case that exists for this question moves with it.**
`scripts/acceptance.sh`'s `wood` arm sweeps four grown stands and gates the
total. Paired off one binary through the ablation, on one machine:
...
**Read the shape rather than the total**: two windows barely move and one is
14 cells *worse*, because the rule only fires where a root plate is in his
path — the two that move are the two where he was being swallowed. A clean
uniform win across all four would have been the tell that something else was
being measured.

`PIXEL_PHYSICS_ROOT_FOOTING=off` is the paired arm, on the
`plant::roots_need_substrate` pattern: the question is judged by eye and the
comparison is paired, so both arms have to come out of one binary. Six guards
in `player.rs`, each watched going red for a named fault — the bug itself, the
`Hard` reading, "all living tissue is ground" (a living branch is still not a
floor), and the missing organism gate (painted root is masonry). The
painted-root guard was a *wall* first and was blind: five cells in a row stop
him at `Soft` as well as at `Hard`, so it passed with the gate deliberately
removed.
...
**How fast an animal lives is heritable now, and it is the first of these
genes a person can watch** (`organism::TRAIT_PACE`, slot 4 of
`CREATURE_TRAITS`, read through `creature::tick_interval_of`). `-1` is an
animal that takes its turn half as often as its kind, `+1` twice as often, on
a reciprocal axis so the two ends are the same factor in opposite directions.
A quick ant scurries and a slow one plods, at up to a 4x spread across the
population's extremes, with no rendering work at all — which is the owner's
stated target (*"clear variety in behavior, different methods of movement"*)
reached by a gene rather than by a shader.

**It needed no price authored, which is true of no other locked field.**
Every levy an animal pays — `idle`, `synapse_tax`, `sight_tax`, exposure — is
charged once per decision, so halving the interval exactly doubles the cost of
living per unit of world time; `CreatureDef::scaled`'s doc had already written
the identity down for a different purpose. Measured, one ant, no
reproduction, 600 frames: **50 / 100 / 199 turns and 5.0 / 10.0 / 19.9 J** at
alleles −1 / 0 / +1. Turns and joules move together to three figures, which
is the claim that this is not a free turn of speed.

**And it is very nearly neutral at first order, which is the argument for it.**
A creature steps one cell per decision, so a quick animal takes twice the
steps and pays twice the bill: joules *per step* do not move. What does not
cancel is everything measured against world time — food regrowing, a predator
closing, a rival reaching the same leaf first, a famine to be outlasted. The
gradient is supplied by the bed and not by the arithmetic, which is the shape
that gives different answers in different beds instead of one answer
everywhere.

**Two of the four trait slots were unreachable from the lab, and nobody had
noticed.** `reproduce_at` and `sight_range` each shipped with the parameters
page registering the older slots beside them by hand and no row of their own,
and the inspector's `GENOME` block listed the same two — so the owner could
neither set them nor see what an animal had inherited on them. Both now read
one `params::TRAIT_ROWS` table, guarded by `every_trait_slot_has_a_row`
against `CREATURE_TRAITS`. Worth stating as its own finding: **a field can be
unlocked in the engine and still locked to the person playing the game**, and
the second kind is invisible from the source because everything about it looks
finished.

The audit behind all of this — every `CreatureDef` field classified as
*locked* and *priced*, with the queue those two axes imply — is
[`Reports/creature-locked-fields-2026-09-05.md`](Reports/creature-locked-fields-2026-09-05.md).
Its governing rule: **unlocking an unpriced field is worse than leaving it
locked**, because an unpriced lever ratchets to its maximum on the first
generation and expresses nothing.

**An eye is heritable now, for every creature — including one that has
none** (`organism::TRAIT_SIGHT_RANGE`, slot 3 of `CREATURE_TRAITS`, read
through `creature::sight_range_of`). `sight_range` was a plain `i32` species
field: whether an animal could see, and how far, was fixed for its whole kind
for ever, and no lineage could trade sharper eyes against their cost or
cheaper eyes against their loss. That is the defect
[`Reports/selective-environments-2026-09-05.md`](Reports/selective-environments-2026-09-05.md)
names for the verbs — *a quantity outside the economy is a quantity selection
cannot reach* — arriving on the sensory axis.

**The price was authored a week before the gene.** `sight_fraction` charges
per cell the eye actually reads, and its own doc says it landed "to stop
`sight_range` being a ratchet" so that the day the reach became heritable
"the gene arrives into a world that already charges for it". Nothing new had
to be priced.

**Additive rather than multiplicative, and that is the whole design.** The
first version of this gene scaled the authored reach and therefore gated on
it: `sight_range: 0` stayed blind at every allele, following
`reproduce_at_of`'s precedent that a mutable slot must not be a back door
through which an eyeless species quietly grows an organ. **The owner
overturned it — "anything should be able to evolve, don't lock" — and the
ruling is right**: a back door is what an open-ended evolutionary system is
*for*, and a species that cannot cross a line drawn by its author is not
evolving, it is being permitted. So the allele *shifts* the reach by up to
`SIGHT_SPAN` (64 cells, the largest eye any shipped species authors), bounded
at `SIGHT_MAX` (128) because a cast is already 328–1,186 `World::get` at
reach 64 and an unbounded allele is an unbounded per-tick cost.

| allele | ant (authored 0) | beetle (authored 64) |
|---|---|---|
| −1 | 0, blind | 0, blind |
| **0** | **0, as authored** | **64, as authored** |
| +1 | **64 — a blind lineage with eyes** | 128 |

**Generation zero is unchanged for every species, blind or sighted**, because
the axis is measured from what the author wrote and only drift moves it. The
accepted cost is stated rather than hidden: every eyeless species in the game
can now, given enough generations, start paying for eyes.

Guarded twice, because the two claims are different.
`a_blind_lineage_can_evolve_an_eye` checks the arithmetic, the neutral
allele, and — the assertion the first version would have failed — that a
species authored blind reaches a positive range at a positive allele.
Fault-injected by putting the gate back; confirmed red, naming it.
`a_sharper_eye_reads_more_of_the_world` checks the arithmetic **reaches a
running world** — and that one was briefly false: a harness that set the
allele on the species *after* the animals were placed gave **byte-identical
counters at three alleles** (9,502 casts, 9,711,169 cells read at every
setting), because a live animal carries its own `OrganismState::traits`,
copied at founding. The identical-output tell caught it; without a counter
beside the number it would have read as a working gene.

`CREATURE_TRAITS` 3 → 4 → 5 cost eight species files a tuple element each
time and nothing else: traits are not in the genome, so `genome_manifest`
does not move, and `specimen.rs` already pads a short trait list — the jar
format was built to survive this growth.
...
**Gate 2 is settled, 2026-09-08.** `arm=lethal` re-run at six seeds on the
harness default bed **and** a fed one (`founders=48`) puts the zeroed brain at
**0.0% of animals on 12 of 12 seed-runs**, and the harness prints its own
verdict — *the bed has teeth*. The coordinator note's standing caveat, that
every evolution result measured in this bed is unvalidated, is **discharged
for the creature side**. Read what that licenses narrowly: it is a
maximal-effect test, so it says the bed turns a *large* fitness difference
into a population difference and says nothing about a small one — the arena's
own seed noise is **2.42–3.12x with no true effect present**, which is why the
small-effect races (flight) null in it. That is a statistical-power problem,
not an ecology one. **Gate 2 for plants is a different harness on a different
kingdom and is still open.**
...
**The box could say what it held and it could say what one individual was. It
could not get you from one to the other.** There was no way to enumerate the
population, so the only route to an individual was spotting it on screen — and
the design guide's own measurement is that *an ant is two dark cells at play
zoom, findable only because it moves*. A dead one has stopped moving, so the
thing most worth looking at was the thing you could not find.

**Two tables now, one per kingdom**, each a row per living thing with eight
columns and a click that holds on to one:

| | columns |
|---|---|
| plants | species, cells, seeds set, water, age, generation, line, state |
| animals | species, bank, crop, body, age, generation, line, state |

Sort on any column (click again to reverse), filter to `IN TROUBLE` or to one
founding line, and page through a list longer than the screen. `FOLLOW` keeps
the camera on the one you picked; `LINE` narrows the list to its founders'
...
one caller drawing a `Panel` through `paint_page` passed `&mut Vec::new()` —
so that heading would have shipped as a row that looks like a button and does
nothing. Nothing had noticed because no generic page had a clickable heading
before.

**What a click stores is an identity, and both of the obvious alternatives are
wrong.** A row index names a different animal the moment anything is born,
dies or the sort changes; a cell — which is what the hover inspector holds,
deliberately — names whatever is standing there now, so a walking ant leaves
its own page behind. The pin is `(organism handle, born frame)`, checked both
halves. `OrganismState::born_frame` is new and exists for this: the handle is a
12-bit slot plus a 4-bit generation and **is reused**, so after sixteen turns
of one slot a handle-only pin follows a stranger. It doubles as the `AGE`
column. A pin that stops resolving is reported as a **death** rather than
quietly becoming another animal.

While a pin holds, the cell page is re-pointed at the individual's own cell
every frame, so the readout that already existed follows the animal instead of
the ground. The marker is a bracket round the whole body for anything four
cells across or more, and the existing crosshair for anything smaller — static,
because a marker that needed motion would fail on exactly the individual you
most want to find.

**Two columns were saying the same word to every row, and only the rendered
table showed it.** `STATE` read `HUNGRY` for all 52 ants, because the floor was
scaled off `body_energy` (480, what an ant's corpse is worth as meat) rather
than `start_energy` (200, what it is given to live on); and it read `HOME` for
all 52, because `nest_memory` is a 3,000-tick sense-decay window and not a
place. A state that never varies is a constant with a column to itself.

**The second one is now `FAR`, and the rewrite is not a rename.** It shipped as
`LOST`, on `since_nest >= nest_memory`: past that window the `recency`
multiplier scaling channel-A deposit was exactly zero, so the animal had no
gradient home and *lost* was a fact about the engine. The 2026-09-02 brain
deleted both halves — `nest_memory` is gone, and homing became three authored
weights on a self-recurrent hidden unit whose curve is hyperbolic and **never
reaches zero**, at a decay rate that is each individual's own genome rather
than a species constant. There is therefore no threshold in Rust at which an
animal's way home has gone, and a column claiming otherwise would be
re-inventing the constant that was just deleted.

What survives is honest and is a different question: **how deep is the
excursion in progress.** `OrganismState::forage_max` answers it in cells and
re-anchors at every nest contact, so — unlike `since_nest`, whose resets were
136-of-142 loitering — standing on the nest cannot manufacture one. The
threshold is 30 cells, set from the measured distribution (p90 of the settled
early bed is 26) rather than picked, and it reads neither zero rows nor all of
them at any tile of a 9,000-frame run. It is gated on the species declaring a
nest, since `CreatureDef::nest` became optional in the same change and an
animal with no home cannot be far from it. **`FAR` is deliberately not in the
`IN TROUBLE` filter**, where `LOST` was: a deep excursion is a forager doing
its job.

**Guards, all seven watched going red.** The list holds every live individual
of its kingdom and no other; an identity survives its slot being handed to
somebody else (the loop turns one slot over until the handle genuinely comes
back); a pin survives a sort and a filter; each table keeps its own sort,
scroll and filter (shared, one *column index* means SEED on one table and BANK
on the other — the harness caught that pinning ant 41 where 11 was expected);
a roster taller than the page can be scrolled, asserted as two separate claims
because the rack shipped a scroll field with no writer; the page and every
widget on it stay inside the panel and on the screen; and the roster can be
left by a button on it, which the harness found twice by getting stuck.

**One guard had to be replaced rather than kept.** *"Sort the same list eight
times and check the order does not move"* stayed green with the tie-break
deleted **and** the sort switched to `sort_unstable_by` — it is blind, because
a sort is deterministic within one build whatever its comparator says, and the
hazard `CLAUDE.md` records lives in the element type and the toolchain. What
replaced it asserts the property that makes the hazard harmless: `roster::
compare` returns `Equal` only for a row against itself, so no implementation
ever has a tie to break. That one goes red.

**Cost: none measurable.** The row list is rebuilt every frame the page is
open and never retained, over tens of organisms. `born_frame` is one `u64`
written at the single allocation seam. Measured on `ascii`'s colony scene at
`RAYON_NUM_THREADS=4`: every counter **byte-identical** to the baseline (moves
21,284, eats 1,142, pickups 1,017, digs 118, deaths 16), and the mean frame
3.98 ms against a 4.30 ms baseline — inside the noise, which was the prediction
stated before measuring. The worst frame is not quoted: `mean x frames` is
51,636 ms against a 51.6 ms worst, so the aggregate does not pin it and it is
an order statistic over many similar frames.

**The graveyard: the dead stay listed.** The list used to cover the living
only, and that was the wrong way round for what this box is for. A selection
experiment is mostly a record of what did *not* work, and the design guide's
own measurement is that an ant is two dark cells at play zoom, findable
because it moves — so a dead one has stopped being findable by the only
channel that ever found it. The individual most worth looking at was the one
you could not look at.

`World::graveyard` keeps a bounded ring of `Grave` records — identity, line,
generation, born and died frames, cause, the whole `LifeCounters`, and where
it died — taken in `free_organism`, which is the one place that decides a
release really happened. It is a **flat record and not a handle**: the
`OrganismState` is already gone and the slot is back on the free list, so
anything not copied out there is lost. `GRAVE_CAP` is 2,048, the same bound as
the run log and for the same reason.

It is a **filter, not a third table**: `ALL → IN TROUBLE → DEAD → (LINE n) →
ALL`. A long run buries fifty living rows under two thousand dead ones, so the
dead are one click away rather than in the way, and they stay in their own
kingdom's table because a dead individual is still a plant or an animal.
`STATE` carries the cause — `STARVED`, `CULLED`, `KILLED`, `LOST ITS TISSUE` —
since for something that has stopped doing anything, what stopped it *is* how
it is doing.

**The columns that need a living body read zero and are not faked.** `CELLS`
and `BANK` are 0 because there is no body and the bank went with it; carrying
the last-known values was considered and rejected, because a corpse showing
95.4 energy reads as an animal that is fine. What a grave is *for* is the
columns that outlive the body — who it was, what line, how long it lasted,
what it produced, what killed it — and those are all real.

**Guards.** The dead are listed and the living table does not carry them,
driven by culling named individuals rather than by waiting for a bed to kill
something (a guard that waits passes for the wrong reason on a bed where
nothing dies); watched going red with the grave writer removed. A cull takes
**3,403 ticks for an animal and 3,632 for a plant** to reach the graveyard on
the shipped bed — `roster::how_long_a_cull_takes` — because a cull marks
senescent and `rot_remains` carries the corpse out at the species half-life.
That delay is the mechanism, not a lag: the owner's own ruling that a death is
...
**A column of `GUT BIAS +0.000` and twelve thousand synapse weights is a
description only if you already know the answer.** Owner's brief: *"instead of
just a bunch of numbers it could say this creature has this movement pattern
and strong dig strength, and no vision."* The cell page now leads with a
`WORDS` group that does exactly that, and the shelf uses the same function on a
jar, which is where *"what did I keep?"* is actually asked.

The shipped ant reads:
...
species may have `sight_range == 0` — every shipped species but the beetle —
or an individual may have eyes and carry no live weight from either prey
input. Those are different animals: *"ANTS HAVE NO EYES"* against *"HAS EYES,
IGNORES THEM"*, the second being a sense that is paid for and wired to
nothing. `W_EPS` is the engine's own definition of "no connection", so the
threshold is not invented.

**A species' capability is never claimed as an individual's.** Only three body
traits and the brain's weights are heritable per individual; `dig_force`,
`sight_range` and `crop_capacity` belong to every member alike. Every sentence
from that half opens with the species' name, and a guard holds it to that — on
a page whose whole purpose is telling two individuals apart, claiming a gene
that does not exist would be the worst thing it could say.

### Two things it got wrong first, both found by asking whether it was accurate

**The hidden layer was not read at all, and that is where the conditionals
live.** The ant's twelve *direct* weights are all unconditional; its entire
foraging loop is twelve *hidden* weights the page said nothing about. Each
hidden unit pairs a large `Bias` offset against a large `Carrying` weight, so
the unit switches on or off with the load, and a pheromone gradient then pushes
`Move`. Decoded, that is *"laden, follow the home scent; empty, follow the food
route"* — the single most important thing the animal does. A summary silent on
that is worse than a short one. `gated()` now reports a unit as a condition
**only when the gate can actually switch it** (`bias + gate` and `bias` must
differ in sign), and collapses the push-pull pairs — the ant wires each
condition twice with opposite signs on sensor and output, which is one gradient
follower built from two half-wave units, not two behaviours.

**The two scent channels were labelled from an assumption, and it was
backwards.** `brain.rs` gives channels A and B **no meaning whatever** — they
are two anonymous planes, and what one means is decided entirely by which
weights emit onto it. The first version hard-coded *"A is the food trail"*; on
the shipped ant every ant lays A all the time (`Bias -> EmitA`), which pools it
wherever the colony is, while only a laden ant lays B (`Carrying -> EmitB`),
which marks the way back from food. So the page described the foraging loop the
wrong way round while reading perfectly. `scent_label` now derives the name
from the individual's own emissions: laid **only while carrying** → `FOOD
ROUTE`; laid **all the time** → `HOME SCENT`; laid **at the nest** → `NEST
MARK`; laid by nobody → `A SCENT`. The guard swaps which channel is laid how
and asserts the names swap with it, which a constant cannot do.

### Also fixed

- **`TRAIT_REPRODUCE_AT` was missing from the numeric `GENOME` group.** Two of
  three body traits were printed from the day that group landed, and nothing
  said so — a page listing two numbers looks exactly like a page whose subject
  has two. It is the one life-history axis the brain cannot express, so without
  it the page cannot tell a fast breeder from a hoarder.
- **A group larger than the whole fold budget could never be opened.** The rule
  was `cost <= room` for every group *including the chosen one*, which reads as
  "serve it first" and is not: clicking such a heading did nothing, for ever.
  Nothing had hit it while the largest group was eleven rows against fourteen.
  The chosen group now opens regardless and `fit_rows` trims it with a count.
- **`plainspeak` no longer panics on a half-built organism.**
  `wiring_from_genome` asserts its genome length by design, and
  `push_organism` allocates a slot before `place_creature` fills the genome in
  — a window a page must be able to describe rather than crash on.

### The column budget is a hard constraint, and it is guarded

`page_rect` sizes the cell page to its widest row and then clamps it onto the
screen, so a long sentence does not wrap — it widens the whole page and slides
it left over whatever it was opened from. Measured: a thirty-character phrase
took the page to 250 px and hid three of the roster's eight columns.
`every_phrase_fits_the_column` sweeps every shipped species, twenty-four random
genomes and every allele combination against `PHRASE_COLUMNS = 26`, so a new
sentence that does not fit fails in the suite rather than on a contact sheet.
That is why the generic fallback is `MOISTURELATERAL > IMPULSE` rather than
*"more impulse when moisture-lateral is high"* — the English form is
thirty-nine characters.

**Known limitations.** Recurrent hidden units are not described. A hidden unit
gated on a *gradient* rather than on a state is reported not at all rather than
as an invented condition. At most five direct drives are listed, strongest
first — the rest are still in the numeric `GENOME` group, and each row's hover
carries its share of the strongest drive, which is what says whether the list
was cut anywhere interesting.

## Watch status — where one individual has been, and how its numbers moved

**The rosters get you to an individual and the life record says what it has
done; this is the part that shows it doing it.** Tier 3 of the 2026-09-01
brief. Pin anything and it grows a trail: 128 samples at one every 12
simulated frames, so 1,536 frames of path, drawn in world space under the
marker and fading from a third of full at the oldest end to full at the
newest. The direction of travel is legible without an arrowhead, which at this
scale would be three pixels of noise.

Two sparklines go in `STATE` — the group defined as *what moves while the box
runs*, which is exactly what a series is. Not a fifth group: that was measured
and does not fit (giving the run-log timeline its own heading cost ~15px and
put the page over the screen).

**The series are scaled to their own range, and the population strip's are
not.** `draw_spark` normalises against the peak, which is right for a
population — a count of zero is extinction and belongs on the floor — and
wrong for one animal's bank. Measured: the shipped ant runs 264 to 349 across
a full ring, so every bar lands between 76% and 100% of peak and the strip
draws as a solid block. The sawtooth of an animal spending and refilling, the
one thing the row exists to show, is invisible. Range-scaled it reads as the
declining ramp it is. **The caption carries the absolute range** (`BANK
264-349`), so the floor is never implied to be zero — without that this would
be the more dangerous error, with a bank hovering near full drawing
identically to one swinging from empty.

**The ring is per-chamber**, swapped with the box like `History`, and for a
sharper reason: it is a list of world coordinates, so one left shared draws
chamber A's path across chamber B's bed. It is keyed on the individual and
clears when the pin moves, so it can never show one animal's path under
another's name. The cell page's sparklines are gated on the pin matching the
organism being inspected — the page follows the cursor and the pin does not,
so an ungated page draws the pinned one's series under the clicked one's
numbers.

**It outlives its subject.** A dead individual stops adding to its trail and
loses none of it. That is the whole reason the path lives on the ring rather
than being recomputed from the world each frame: where an ant got to before it
starved is exactly what its graveyard row cannot tell you.

**Three things the harness had to find before the tile showed anything**, all
of them the byte-identical-output tell:

- `Lab::follow_pin` runs in `draw`, not `tick`, and `Renderer::follow` is a
  dead-zone follow (deliberately — a strictly-centred camera repaints the
  whole screen every frame an animal walks). A loop that ticks 1,600 times and
  draws once gets one nudge. The tile now draws every 16 ticks, which is also
  what a player watching actually sees.
- **At 1:1 the camera cannot move at all.** `world_to_screen` measured as the
  identity: the bed is 512 wide and so is the screen, so an individual at
  x=404 stays at x=404 — under the cell page, which a pin *forces* open. The
  tile zooms in so the view is smaller than the world and FOLLOW has somewhere
  to pan.
- **Row 0 of the roster travelled five cells in 1,524 frames.** A trail is only
  a trail if its subject moved, so the tile picks the individual with the
  greatest `forage_max` — the same number `FAR` reads.

**Guards.** The ring samples on simulated frames at exactly `WATCH_EVERY` (the
same class of claim the population strip got wrong for months); it stays empty
with nothing pinned, so it is not quietly walking every organism every tick;
it clears when the pin moves; and it keeps its trail across the death of its
subject — watched going red by making `observe` clear the ring when the
subject stops resolving.

## Side-by-side status — two individuals, with what differs marked

**The question a selection experiment is actually asking is why one did
better than another, and nothing on this interface could answer it.** Every
other page is about one individual or about the whole box; a difference
between two is neither, and reading it by flipping between two cell pages
means holding thirty numbers in your head.

`HOLD` on a pinned row, pin a second one, press `VS`. One chip with two
meanings rather than two chips — the bar has been measured full twice, and
the state it is in is written on its own face (`HOLD` becomes `VS`) and in
the notice it leaves. The held individual is deliberately **not** a second
pin: only one thing is marked in the box and followed at a time, because two
markers on a 512-wide bed is two things to find rather than one.

The page is built out of `params::specimen_rows`, not a second copy of that
arithmetic, so a number cannot read one way here and another on the cell page
— which would be the worst possible defect on a page whose whole job is
comparison. Rows are paired by label and the **union** is shown, not the
intersection: a plant against an animal produces rows that exist for only one
of them, and those are exactly the ones worth seeing. A missing side reads
`--`.

**What differs comes first, bright; what matches follows, dimmed.** That
ordering is the page's argument, and it was wrong in the first build: in
specimen order the two shipped ants put fourteen identical plain-speech lines
above the nine rows that actually differed, so a page opened to ask why one
was doing better answered below the fold. `fit_rows` also trims from the
bottom, so the ordering decides which rows survive an overrun — and the
identical ones are the right ones to lose.

**The difference is marked, not computed.** A numeric delta was the obvious
build and is wrong: half these rows are not numbers (`ORIGIN FOUNDER`, `CROP
EMPTY`, `STARVING NO`), and a page that subtracts where it can and does not
where it cannot has blank cells meaning two different things. Same-or-different
is defined for every row. A `DIFFER n OF m` count sits at the top because a
page of forty dimmed rows reads identically to a page that failed to load —
two clones of one jar legitimately differ nowhere, and the number is what says
so out loud.

**The cell page is suppressed while this one is open**, alone among the pages.
Every other answers a different question and sits beside it; this one's left
column *is* the pinned individual's cell page, row for row, and a pin forces
`inspect` open — so both up draws the same numbers twice and takes the width
that made the comparison legible.

**Rows whose label carries a frame number never pair, by design.**
`specimen_rows` stamps a few life events into the label itself (`F636 FIRST
FED`), so two individuals that both fed produce two rows rather than one, each
with `--` opposite. That reads correctly — it says when *each* first fed — and
it inflates the differing count by one per such event.

**Guards.** An individual compared against itself differs in zero of its rows
(the positive control: a non-zero there is a pairing bug, which no comparison
of two *different* animals could distinguish); two different animals differ
somewhere; a half-set comparison draws its own empty state rather than a blank
page; and every differing row sorts above every matching one — watched going
red by swapping the two.

The lineage overlay that was the other half of this work now exists — see
below.

## Lineage overlay status — which founding line is taking the bed

**Every other channel in the lab asks about a cell or about one individual.
This is the only one that asks about the population**, and it is the question
a selection box exists to ask: *is one line winning?*

The roster could half-answer it already — `LINE` narrows the table to one
founding line — but a list of forty rows cannot say where those forty are or
what share of the ground they hold. One line spread thin everywhere and one
line owning the left third produce the same table. `OrganismState::lineage`'s
own doc makes the case: *"is this population diverging" cannot be answered
from a genome scatter — only who descends from whom separates them.*

**Two modes, on the `L` cycle beside the other organism channels**, because
they answer different questions and an overlay that silently changed meaning
when a pin appeared would be one you cannot learn:

| | shows |
|---|---|
| `FOUNDING LINES` | the heaviest six lines, a colour each; everything else grey |
| `ONE LINE` | the pinned individual's line; everything else grey |

**Ranked by cells, not by head-count.** A line of forty seedlings and a line
of four mature trees are not the same claim on the bed, and head-count ranks
the stand of failures above the tree shading them out. `World::lineage_mass`
is the census, and it is a **total order** — ties break on the line number,
because two equal lines swapping places between frames would swap *colours*
on screen with nothing in the world having changed. That is `CLAUDE.md`'s
tie-order hazard in the one form a player can actually see.

**Six colours, and six is a decision rather than a limit hit by accident.**
`lineage` is an unbounded counter and a long run has hundreds of founders, so
"colour by line" is not a design — hashing an id into a wheel produces a
picture that *looks* like information and carries none. Six is about the
limit of a categorical set read at a glance, and the tail draws in one flat
grey, which is the honest reading: "not one of the ones winning" is the fact
worth carrying.

**With nothing pinned, `ONE LINE` greys the whole bed** rather than drawing
nothing. A channel that paints normally when it has no subject is
indistinguishable from the overlay being off — the confusion this file
records for a canopy-density sheet that read as blank and was misdiagnosed as
dead code. And the focus is **kept through a death**: the individual is gone
and its line may not be, and *"did this one leave anything behind"* is
precisely what is being asked, so the overlay goes on showing the line while
the roster shows the death.

**Cost: nothing on a settled bed, measured as a counter rather than a clock.**
The channel repaints only on the frames its ranking actually moves — the same
amortisation the stepped grain modes use, and against the animated grain's
measured ~10 ms every frame it matters. On the shipped bed the top-six mapping
moved on **0 of 600 ticks**. That is a tidy null, so it carries its positive
control: culling the heaviest line outright, the same mapping moves **18 times
in 6,000 ticks**. The counter can move; the bed simply does not reshuffle.

**Guards, all watched going red.** The census ranks by cells and not
head-count (an arm the head-count reading would rank last takes the top); its
order is total, checked over **seven tied lines** rather than one, because a
single tie catches a missing tie-break only half the time and six put that at
one in 720; a line drops out of the census once its last body is freed, since
`free_organism` leaves the handle written in the cells and a census walking
cells would credit a line with ground it no longer holds. On the render side,
asserted on **rendered pixels** rather than on the overlay enum — the pinned
line takes the first colour and another line takes the grey, the tail past the
palette takes the grey, and with nothing pinned every animal does. Faults put
back both ways each time: a channel that paints everything and one that paints
nothing each satisfy half of it.

**Two things the harness got wrong before the channel did**, both worth
keeping: the first render sampled a pixel the cell page was standing over and
asserted `PANEL_BG` against a lineage colour, and the second sampled the ant
next door and read the pin's own reticle. Neither was the overlay; both look
exactly like it failing.

**Known limitations.** The palette is fixed, so which line gets red is
"heaviest right now" and changes when the ranking does — a line does not keep
a colour across a long run. `FOUNDING LINES` cannot say *why* a line is
winning, only that it is. And there is no lineage tree: the overlay shows
who holds the ground, not who descends from whom.

## Creature groups status — who is who in the box, and who is family

**The first session in which ants and beetles reached a fluctuating
equilibrium was played with a debug overlay on**, because both species are
dark browns on dark soil and the one thing that reads at 1-2 px — contrast
against the ground — is what neither material palette gives. The owner's
three asks from that session: see who is who, graph the groups apart, and
say what a group *is*. `Reports/creature-groups-and-combat-design-2026-09-06.md`
is the design; this is what shipped.

**A group is a placement gesture.** Every click of the colony tool, every
single animal placed and every jar released founds one
`OrganismState::colony`, claimed by the **first animal that fits** and joined
by every later station of the same gesture — so a founding that tries fifty
sites claims one label and `ANT 3` is the third thing the player put down. A
child is born into its parent's colony beside its `lineage`; a plant is
colony 0. `World::live_creature_groups` is the census, `(species, colony,
alive)` in placement order, and `Grave` carries the colony so the graveyard
can be read per group too.

**What that used to be: nothing.** Two clicks were one population — the only
identities an animal carried were its species and a per-founder lineage
(fifty-two of them in one colony), and nothing in the engine knew a click
had happened.

**Animals wear their group's colour, and the graph line under them is the
same colour.** `render::CreatureColour { Off, Species, Colony }` on the
renderer, `Colony` in the lab and `Off` in the outdoor game (byte-identical
to the shipped draw), cycled from the ANTS page. `render::group_colour` is
the one definition read by the animal on screen and by the ANTS page's line
and legend swatch, so the line you watch fall and the animals you watch die
cannot disagree. The colour is a **replace, not a tint** — `GUT_TINT_*`
records that a 45% pull toward green lost a blind A/B and every subtle
recolour here has read as blank — with the body's own countershading kept as
a luminance ratio floored at 0.55, so a paler head over a darker belly
survives in the group's hue. Eight authored colours, absent from the bed's browns and
greens, ordered so the first two groups placed are far apart on the wheel;
past eight `render::group_palette` generates a fresh hue per group — the
owner's ruling on the lineage card the same day, *"every line should have
its own colour even when there are twenty"* — each candidate rotated on
until it is 56 RGB clear of every colour already handed out and bright
enough to stand off soil. The guard caught both defects the first draft had:
a generated amber 24.6 from the authored one, and a saturated blue at luma
86. Twenty groups sit at least 40 apart, checked.

**The ANTS page graphs every group on one axis.** One shared peak rather
than a strip per group, because two groups fighting is the question and a
per-strip scale would draw both as full. A legend row per group in its
colour with the standing count, the change since the last sample and
`K<n>` for how many of its dead were killed by another animal (the hover
note names the killers and the starved), in placement order; grouped by
colony or by species as the colour mode says; twelve rows, then a `+N MORE`
row. **A wiped-out group stays listed at 0** for as long as the sample ring
remembers it — the first draft dropped the beetles from the legend the
moment they starved, which is the one line a player most wants to watch
reach the floor. The `ANIMALS WEAR` row on the page and the `H` key cycle
the colour mode; the renderer and the page are moved by the same action and
a guard holds them together.

**Deaths are tallied per group, and a kill names the killer.**
`World::group_deaths` carries each animal group's deaths by cause and its
kills *by attacking group*, booked at the bite where both parties are in
scope (`World::tally_kill`) — the corpse alone cannot say who bit it.
`labstats rivalry=1` prints it as a `--- groups ---` block. Measured with it
on the two-colony bed, three seeds x 24,000 frames: rivalry off, **zero**
ant-on-ant kills anywhere; on, 3–9 kills per side per run, both ways,
against 35–42 starvations per side. Predation, not war — the design
report's §2 table.

**Colony rivalry was a rule, and it is retired.** For one evening
`World::colony_rivalry` sat on the ANTS page: on, `creature::is_living_kin`
required the same colony as well as the same species, so an ant from another
colony became somebody else's flesh — prey to any gut whose diet axis reaches
it, by exactly the arithmetic that makes an ant prey to a beetle (a
generalist's gut takes `480 × 0.25 = 120 J` from a stranger's cell against a
`12 J` threshold). **It was superseded the same week by the heritable scent
signature below**, which does the same job as a *distance* rather than a
switch and is on by default, so the field is gone from `World`; all that
remains is `lab/params.rs`'s migration, which drops the stray key from a
dials file saved while it existed rather than refusing to load it. Its three
stated gaps have since closed in order: the `Attack` verb, a per-colony
scent, and the alarm the bitten raise — all below, all shipped on.

**The hunted side of the eye.** `BrainInput::ThreatNear` and
`ThreatBearing`, slots 22 and 23, appended on the same day: the nearest
animal whose gut would take this one's flesh, as nearness and signed
bearing, on the same rays as prey and kin and never breaking one, so the
eye's bill is unchanged. `creature::is_visible_threat` is `is_visible_prey`
with the roles swapped — one definition of who eats whom, read from both
ends — so threat is mutual wherever diets overlap (a generalist ant takes 50
J from beetle flesh against a 12 J bar, so it is a threat to the beetle
too). **Nothing shipped is wired to it and the ant is blind**, so it changes
no picture today; what it changes is that flight, hiding and alarm are
*reachable* by a lineage that grows eyes, where before *a bed could not
select for what the animal could not perceive*. `live_slots` 544 -> 584 and
every species' `mutation_rate` re-derived to `3.18 / 584`. Guarded by
`a_hunted_animal_sees_its_hunter_and_only_a_hunter` (three arms: carnivore
beetle seen, herbivore beetle not, and the beetle seeing the ant back).
`CreatureStats::threat_sightings` is the counter; `labstats` prints it.

**How far a lineage may evolve, as a dial — the arms-race reach.** Every one
of the ten heritable trait slots shared one `[-1, 1]` axis, and for two of
them that was a ruling nobody had made. Armour and the jaw are the only pair
read against *each other* — `damage = clamp(bite/armour, 0, 1)^2` — and their
authored numbers sit four times apart: ant flesh resists **0.25** against an
ant's bite of **1.0**, so the best plate an ant lineage could reach was 0.50
and **a maximally armoured ant was one-shot by any other ant at every point
on the axis**. A 2x range cannot close a 4x gap, so the fights the rivalry
dial starts were binary by arithmetic and no tuning over them meant anything.

`World::trait_reach` is how far those two slots may travel, as a multiple of
the shared axis; `creature::ARMS_RACE_SLOTS` is the pair and
`creature::allele_bound` is the one function both the resolvers and the birth
read. **It ships at 1.0 and at 1.0 every clamp is the `clamp(-1.0, 1.0)` that
was written there**, so the bed is byte-identical and only a lineage that
*evolves* past the old ceiling reads differently. `assets/materials/ant.ron`'s
0.25 and `beetle.ron`'s 0.8 are untouched: the shipped animals are the shipped
animals, and what changed is how far their descendants may get from them.

**Both slots move together, on purpose.** Widening the plate alone would hand
every fight to the defender and the jaw alone to the attacker; widened
together the ratio is scale-invariant, so a max-jaw ant still one-shots a
max-plate one and what the reach buys is the *spread between lineages*. Both
ends are priced every turn already (`armour_fraction`, `force_fraction` on
the COSTS page), and at the top of the dial an ant burns 1.9x its idle rate
carrying them.

Measured — one maximally armoured ant against three ordinary ones, six seeds,
2,000 frames:

| reach | plate vs a 1.0 bite | frames to first breach | defender alive |
|---|---|---|---|
...
Reach 2 reproducing reach 1 exactly is the arithmetic and not a dead dial:
the plate only passes the bite above `reach = 3`, and below that the fight is
one bite at every setting. Guarded by
`a_maximally_armoured_ant_is_graded_only_when_the_reach_allows_it` (on
time-to-first-breach, never a bite count — a finite body takes a finite
amount of chewing, so a bite count reads as a dead heat) and by
`a_lineage_cannot_evolve_past_a_reach_it_was_not_given`, which is the
positive control that the dial reaches the *birth*: the alleles are clamped
where they are drawn, so widening only the resolvers would have been vacuous.
The row is on the GENOME page under the ten slots it governs, and the two
rows it widens widen with it. **Zero is the other useful end** — it pins both
slots where the species authored them while every other slot goes on
mutating, which is the clonal control arm no `Setting` could build before.

**A colony can hear a fight, and can start one.** Two changes, landed
together because the second is what makes the first worth having.

**The alarm plane** (`pheromone::Channel::Alarm`). A third world-sized plane,
written by an animal *being bitten* at the cell that was bitten, decaying at
`ALARM_RHO` 0.25 a pass against the trail planes' 0.03 — measured, one bite
is at a tenth of a trail's height after eight passes and is **gone by twelve**,
about a second and a half of play. That fast decay is the design and not a
tuning: a plane that remembered every bite of the last ten minutes would be a
map of where fights have happened, which is a trail with extra steps, and
`pheromone.rs`'s module doc had resisted a third plane until a signal existed
that was not one. Read as `BrainInput::Alarm` (slot 24), one input and not the
front/lateral pair the trails get: an alarm is an event, and the direction a
hunter is in is already `ThreatBearing`'s job.

**It costs nothing until something is bitten.** The plane is `Option` and is
allocated on the first deposit, because the module doc prices a plane at
**~40 MB** at the shipped 8192x2560 world and allocates the two trail planes
eagerly whether or not a creature exists. `Pheromones::alarm_is_live` is the
counter half of that: a plane that was never made and one that has decayed to
zero sample identically, and only one of them is a statement about the world.
`CreatureStats::deposits_alarm` counts the bites. The decay is the
`alarm_fades` row on the ANTS page.

Verified in a real bed rather than only in a chamber (`labstats founders=48
predators=4`, 9,000 frames, the new `--- fighting ---` line): **2,643 alarm
deposits with the rivalry dial off** and **4,702 with two rival colonies**,
plane live in both — beetles bite ants and ants bite back, so any bed holding
a predator writes the plane early. **`attacks 0` in both**, which is the
opt-in control holding outside the test suite.

**The `Attack` verb** (`BrainOutput::Attack`, slot 12 — the first append on
the *output* axis since the genome layout was reserved, and lawful for exactly
the reason the reserve was bought). Every bite in the engine until now was the
`Feed` path, so a colony could only fight *by being hungry beside* a rival and
territorial defence was unexpressible. `Attack` is the same graded bite at
`(bite/armour)^2` banked on the victim, billed the same jaw work per closure —
and it **yields no food**: it does not fill the crop and sates nothing, so an
animal that spends its day fighting starves. That price is what makes a
soldier a trade against a forager rather than a free upgrade.

Its target is the **nearest non-kin animal in reach**, not the best mouthful:
`adjacent_food_counted` ranks by what the gut would get, which is the wrong
question for a fight — a rival ant and a fallen leaf are not competing offers,
and an animal defending its nest against something it cannot digest would
score every candidate at zero. `creature::nearest_foe` walks the same
deduplicated whole-body ring and asks only *alive, somebody else, not kin* —
so the colony rivalry dial reaches the verb without the verb knowing the dial
exists, and the heritable signature will too. A killed cell is destroyed
rather than eaten, which is the accounted path every other non-eating removal
(a fire, a blast, the brush) already takes.

**Nothing that ships is wired to either.** `squash(0)` is exactly 0, so an
unwired `Attack` never fires and an ant pays one float comparison per tick for
it; the scan and the `Gut` are built only when the verb rolls. What changed is
what a lineage can reach. `CreatureStats::attacks`, `attack_cells` and
`attack_kills` are the swing counter and its two effect counters, paired
because a count of swings is not a count of hits.

**The genome grew on both axes at once, on purpose.** `live_slots` 584 -> 637
(an input column is 20 slots, an **output row is 33** — the asymmetry worth
knowing before appending a verb), every species' `mutation_rate` re-derived to
`3.18 / 637 = 0.0049922`, `GENOME_LEN` unchanged at 12,352 and not one
existing weight moved. Two appends in one change is one re-derivation and one
break in birth-draw comparability instead of two. `ascii`'s
deposition-follows-moisture gate is unmoved at **1.36x on 237 drops from 2,962
laden ants** — that scene has no births in it, so no birth draw shifts — and
the ablated control still reads 0.69x.

Guarded by `being_bitten_calls_out_and_nothing_else_does` (a peaceful colony
writes zero deposits and allocates no plane; a fight writes both),
`the_alarm_forgets_faster_than_a_trail` (paired on one deposit each, read at
eight passes where the mechanism is legible rather than at twenty where both
arms tie at zero), and `attacking_costs_the_jaw_and_yields_no_food` (fires,
takes cells, spends energy, leaves the crop empty, and does nothing at all to
a nestmate).

**And the colony fights, by default.** Owner's ruling, 2026-09-06: *ship new
behaviours as default*. Two things moved off their compatibility settings.

**`TRAIT_REACH_DEFAULT` is now `TRAIT_REACH_MAX`** — 8 rather than 1.
Understand before reading a bed at either setting that **this changes nothing
on day one**: the reach is a ceiling on where a lineage may get, not a value
anything starts at, and every animal is born at allele 0 with `trait_variance`
moving a slot 0.15 a birth against generations of ~8,600 frames. What it
removes is a wall, at the value that made ant-against-ant one bite whoever bit
first.

**The ant is authored to stand and fight** — two weights on a brain of six:

```
(Alarm, Move,   -1.0)    stop where the fight is, instead of hurrying through
(Alarm, Attack,  2.0)    and bite what is there
```

**The alarm is read at the cell the animal is standing on, not the cell
ahead** — the one pheromone input that breaks the trail planes' convention,
and it shipped the other way for half a day. Measured on the standard bed
(`labstats founders=48 predators=4`, 9,000 frames, three seeds): reading ahead
gave **8, 38 and 28 attacks**; reading here gave **296, 258 and 266**, with
`eats` going *up*. The difference is facing — an animal standing in a fight
with its back turned was reading zero, and being in a fight is not a fact
about which way you are looking. What it costs is direction: a here-read
carries none, so recruitment *toward* a distant fight is not expressible with
one slot. The alarm makes an animal act where it stands; ants recruit over
distance on the trail planes, which have a gradient.

**The sign of `(Alarm, Move)` was measured, and the reasoning would have got
it backwards.** Read here, a positive weight says "move faster while you are
in a fight", which is leaving it:

| `(Alarm, Move)` | attacks | cells off a beetle | eats | ants eaten by beetles |
|---|---|---|---|---|
...
Standing to fight takes about **40% more off a beetle** and roughly **halves
what the beetles take back**, and it costs the colony **15–22% of its
foraging**. Populations sit inside the bed's own spread on three seeds
(25/37/28 against 25/32/36 unwired), so the trade is visible in the ledger
rather than in the census — which is what a priced behaviour is supposed to
look like.

**What it costs the outdoor game, where it never fires — measured, not
assumed.** Nothing bites anything in a one-colony ant scene, so the alarm
plane is never written and both weights contribute exactly zero. They are
still **billed**: `synapse_fraction` charges per active synapse per tick and
`eval_brain` counts a synapse active on its weight's magnitude, not on what
flows through it. `ascii`'s deposition gate moves **1.36x → 1.34x** (drops
237 → 233, pickups 332 → 309), still far above its 0.9 bar — and the same
build with both weights set to `0.0` returns **1.36x on 237 drops from 2,962
laden ants, digit for digit**, which is the control that says the drift is the
tax and nothing else. That is `synapse_fraction` working as authored: a
connection that does not earn its keep is one selection can prune, and in a
world with no fights it should.

**Known limitations.**

- **The alarm cannot recruit at a distance.** One input slot read at the
  animal's own cell carries no direction, so a colony converges on a fight
  only insofar as its members are already near it. Making that a taxis rather
  than a kinesis needs a second slot, and `BrainInput::Alarm` argues against
  one.
- **`Persist` and `Tumble` are deliberately not wired.** They are the obvious
  other half of "commit to a heading", and the only measurement this project
  has says the opposite of the obvious: a flight arm carrying them came in
  behind the same arm without them. That comparison was confounded by the
  synapse count it also changed — `creature_arena`'s `padarm=on` now holds
  the count equal with inert-but-taxed weights, so the race is re-runnable
  and has not been re-run. Until it is, nothing has earned two more weights.
- A lone placed animal is a colony of one. Stocking beetles by single clicks
  makes as many groups as clicks; whether a single animal should instead join
  the nearest colony of its kind is the owner's call (report §7).
- **`World::plant_ant` claims a fresh colony per call**, so any *scene* that
  places ants in a loop builds N colonies wearing one colony's clothes. This
  used to be inert while `colony_rivalry` was off; **since the scent
  signature it is never inert**, because a per-colony offset is drawn at
  founding and those N colonies are strangers to each other by default.
  Measured on the rivalry dial before it was retired,
  on `examples/ascii.rs`'s deposition bed with the dial on, attributed drops
  fell **237 -> 22** and the moisture ratio 1.36x -> 1.14x, because the
  "colony" stopped foraging and ate itself. That scene now founds one colony
  and both arms of the dial come back digit for digit; **the other ant scenes
  in `ascii` have not been converted**, and any harness that stocks a bed this
  way is in the same position.
- An ant is still always an *ant* by `state.species`, which is written once —
  but this bullet used to add "and however far a line drifts it stays kin to
  every ant", and **that half is no longer true**: the heritable signature
  below makes kinship a distance, so a line that drifts far enough is a
  stranger to its own species. What has not changed is that nothing *names*
  the new species; there is no speciation event, only a gradient.
- Births are not yet per group; deaths and kills are (the legend's `K`
  and hover note), and the run log does not yet name a killer.
- The graph samples every 120 frames like the strip it replaced; at 1024x
  that is a point every eighth of a second of wall time and a fight can
  happen between two samples.

**Kin is a distance now, not a bit — and an ant is no longer always an
ant.** `creature::is_living_kin` was *same species* (and, behind the
one-evening `colony_rivalry` switch, *same colony label*), so a lineage
could drift as far as it liked and still be nobody's stranger. Since
2026-09-06 it is **the other animal's scent within this animal's
tolerance**: four `CREATURE_TRAITS` slots, `TRAIT_SCENT_A/B/C` (10–12, three
numbers on `-1..=1`) and `TRAIT_TOLERANCE` (13, read as a radius of
`tolerance + 1`), every one heritable, mutated per birth at the species'
`scent_drift`. A founder starts at its species' authored point plus a
per-colony offset drawn at founding (`CreatureDef::scent_spread`,
`creature::colony_scent_offset`, keyed on the seed and the label so every
station of a gesture shares it); species is still consulted unless the
kind's `kin_crosses_kinds` is on — the design report's §7 call, shipped as
a dial defaulting to no. The predicate is judged from the judge's side and
is not symmetric (a tolerant ant beside an intolerant one sees family and
is bitten), and one predicate still feeds the mouth, the eye and the kin
sense. **The colony label follows the scent**: `World::regroup_by_scent`,
at the ANTS page's sample cadence, splits any `(species, label)` group whose
animals are no longer mutually family into clusters; the cluster holding
the oldest founding line keeps the label and every other cluster of at
least `MIN_SPLIT_GROUP` animals is minted a fresh label as its child
(`World::colony_parents`), which `World::group_label` names `ANT 1b`,
`ANT 1c`, `ANT 1bb` on the legend, the killer notes and `labstats`. Splits
only, never merges — two clicks that smell alike stay two groups. **The
shipped bed is exactly what it was**: `scent_spread` and `scent_drift`
default to zero, so every ant sits on one point, the four slots consume no
birth draw, and the two-colony census is byte-identical to the previous
build (§1e of the report). `World::colony_rivalry` retired into the
tolerance dial's narrow end (`spread 1`, `tolerance -1`); `labstats
rivalry=1` keeps the name as an alias so the groups report's §2 table
re-runs as the positive control. All of it on the parameters page — the
four ancestral rows on GENOME beside `scent_drift`, `scent_spread` and
...
something a lineage finds rather than something authored.

**Built, the same evening: a child can be made differently from its
genes, and nothing in the box says what a caste is.** A parent's brain has
one more output, `Provision`, read once at budding; the child keeps that
number as `OrganismState::made` (a phenotype — never inherited, never
jarred, zero for every founder) and reads it back through one more input,
`Made`. The genome carries a *developmental block* after the last brain
block (`brain::TRAIT_SLOTS`, the first `CREATURE_TRAITS` of them live,
mutated with the wiring, kept by a jar as `plastic: [(slot, weight)]`),
and the body an animal actually has is `creature::expressed_traits`:
genotype plus `plasticity × made × block`, each slot held inside its own
allele bound. Every phenotype reader goes through it — the trait
resolvers, the bite's `armour_at`, the tick interval, the eye, the crop,
the kin predicate's scent and the regroup pass; the genotype is what a
child inherits. `World::plasticity` is the one dial, on the GENOME page
beside the arms-race reach, and **it ships on** (1.0, the owner's ruling
of 2026-09-06 evening; it first landed at zero): a line can find a caste
in the shipped box, and until one wires `Provision` every animal is made
of exactly nothing and the reader is one float compare per trait read.
Zero is the clonal control, in which every animal expresses its genotype
whatever its block drifts to. Measured on the merged head, the two-colony
bed at the shipped dial is **byte-identical** to the same binary at zero
on three seeds, and after 48,000 frames and 84 births on the seed that
breeds no line has wired `Provision` yet. The live mutable surface grew
637 → 706 slots and every species' `mutation_rate` is re-derived
(`3.18 / 706`) so the expected point mutations per child stay put.
**Measured**: the shipped two-colony bed is the same distribution as the
head before it on three seeds (alive at 24,000 frames 2/44/14 → 6/41/12,
births 3/32/11 → 5/35/10), with the block reading zero on every slot; on
the one seed that breeds, `plasticity=1` with `Bias → Provision` and a
0.8 developmental weight on armour makes **20 of 36** living ants at
`made = 0.5` and spreads expressed armour **0.000 .. 0.647** inside one
founding line against a mutational floor of 0.08–0.32, and the identical
wiring at dial zero still hands the number (28 of 44) and moves no body.
Raced in the arena with the same wiring under two dials on round
fifteen's bed with teeth (`founders=48 predators=4`, 24,000 frames, six
seeds), dial 1 reads a median **50.9%** against **51.6%** for dial 0 on the
identical genome, 3 seeds below half in each: the developmental block is
not selected for in this bed, and the reason is the bed's — the living
are mostly founders at that horizon, and a +0.40 plate at the shipped
reach is still under the bite. *Reachable, connected, gated; not yet
found.*

## Life record status — what an individual has done, and what killed it

**The roster could say what each individual *is*. It could not say what any of
them had *done*.** Every count in the engine was world-wide: the box knew it
had taken 21,284 steps and 118 digs and could not attribute one of them. So a
colony where two ants do all the foraging and fifty mill about read exactly
like a colony where all fifty forage.

`OrganismState` now carries a `LifeCounters` block — steps, blocked steps,
mouthfuls, digs, deliveries, young, seeds — each **mirrored from a site that
already increments the world-wide counter beside it**. The cell page shows
them under `STATE`, and the animals roster trades its `BODY` column (the same
two cells on every ant in the box, so it carried nothing) for `YOUNG`, which
is the figure you sort a colony on to find what is actually breeding.

### The books close, and the guard says so

```text
sum over the living  +  World::dead_life  ==  the world-wide total
```

A freed organism takes its counts with it, so without the dead-side
accumulator the live sum can only fall and is comparable to nothing. The
roll-up happens in `World::free_organism` — the one function that decides a
release really happened, and therefore the only place that cannot
double-count. Both halves are provable red: delete a mirror and the live sum
undershoots; delete the roll-up and it undershoots by exactly the dead
animals' totals.

**The vacuity checks come first and are not decoration.** On a bed where
nothing dies the dead-side term is zero, and the identity then passes *with
the roll-up deleted* — `CLAUDE.md`'s *green is the default state* blindness
exactly. The guard asserts that something died, that it died carrying a
non-empty life, and that each world counter actually moved, before it asserts
any sum. The first run of it failed on that check: 4,000 frames of the colony
bed produce no death at all, so the test now kills animals that have already
walked, dug and fed.

**`eats` is deliberately not mirrored, and the guard asserts why.** It is
incremented at two sites for one food cell — into the crop in `act`, and again
when a whole cell is absorbed in digestion — and the two comments disagree
about what it means. Measured on `ascii`'s colony it runs `eats 283 /
pickups 265` at 2,000 frames and `1142 / 1017` at 12,000, so it cannot close
against anything. `bites` mirrors `pickups`, which has exactly one site, and
the guard fails if `eats` ever stops exceeding it — because that would mean
the double count had been fixed and a life counter could mirror it.

### `World::seeds_set` did not exist, and the code said so

`lab::ui`'s seeds figure *"walks the live organism list rather than reading a
world-level counter, because there is not one"*, and `lab::stats`' `seeds_borne`
is a **proxy** — `fate_mutation_rolls`, which only moves when the fate roll
fires. So the only cumulative seed count in the engine was an estimate that
fell whenever a bearer died. There is now a real one, incremented at
`bear_seed_at`'s single site.

### Cause of death, and the counter open bug §B2 has never had

`creature_dies` takes a cause; its four production callers each already knew
which death they were — a head gone (`KILLED`), a bank at zero (`STARVED`),
the same while airborne (`STARVED ALOFT`, told apart because the flight verb
charging an animal to death is its own design question), and the cull
(`CULLED`). Plants set theirs beside the one-way `senescent` flag:
`LOST ITS TISSUE` and `STARVED`, with `CULLED` at `mark_organism_senescent`.

**And the one nothing recorded at all.** §B2 — the support check severing a
*living* plant whole — leaves the world with `senescent == false` and no cause,
because `plant.rs`'s senescence rule is guarded on `!cells.is_empty()` and a
whole-plant felling empties the list. Such a plant fell through to slot
reclamation indistinguishable from one that was allocated and never given a
cell. §B2 has only ever had **cell-level** numbers — 654 living cells severed
per run — and could not say **how many plants** died that way. Classifying
"no cells, no cause" at the closing seam as `FELLED` is that count, for one
boolean.

### A guard that was written and then deleted rather than shipped

The dig mirror must sit **outside** the `spoil_kept()` block, because that
block holds the only nearby `organism_mut` and folding into it would gate the
dig count on a knob with nothing to do with digging. A test named for that
trap was written — and it **could not go red**: `spoil_kept` caches in a
`OnceLock` and defaults to *true*, so under `cargo test` the gated mirror
fires anyway and both placements pass. A test that cannot fail for the fault
it is named after is blind, not weak, so it was deleted and the reasoning
moved to a comment at the call site.

### Cost

Every mirror is per *event*, not per cell, beside a counter that was already
being incremented on the same object graph. Three sites need their own
lookup — `moves_blocked` (the hot one, 29,344 blocked against 41,843 moves),
births (which increments the **parent**, not the child), and digs (see above);
the rest fold into a borrow that was already open.

Measured on `ascii`'s colony at `RAYON_NUM_THREADS=4`: **every counter
byte-identical** to baseline — moves 21,284, blocked 1,642, eats 1,142,
pickups 1,017, digs 118, drops 857, deliveries 437, nest-visits 1,455,
deaths 16 — so the simulation is bit-identical and `tests/determinism.rs`
passes unchanged. Mean frame **4.05 ms against a 3.98–4.30 ms baseline
range**: inside noise, which was the prediction stated before measuring.

**Known limitations.** A plant's clock starts when its parent set the seed,
not when it germinated, because that is when `bear_seed_at` allocates its
organism — so a long-dormant seed reads old on its first day above ground, and
the `AGE` row says so. The run log and the watched-individual trail are not
built yet; nor is the graveyard, so a death is a counted cause and not yet a
row you can read.
...
**2026-09-06 — the tick is 2-5x faster on every bed, and not one cell moved.**
The profile that set the order was the owner's own curve stated as a
mechanism: an empty box at 1024x, one small plant at 20x, a full box under 1x
is *per-awake-chunk overhead*, not biomass — a moisture pass paying a
`HashMap` probe per cell read, SipHash over every chunk key, rayon woken for
jobs of one chunk or sixteen field cells, and three field passes solving wind
in a sealed box with no wind. Six commits, each bit-identical on the world
hash *and* the field hash: a fixed-seed hasher on the hot maps, a dense
`ChunkGrid` behind `World::get`, inline `FieldTile` arrays, serial fallbacks
below a size threshold for the sweep and the field, the momentum skip freed
from its never-met "no chunk awake" condition, and `rebuild_blocked`
rescanning only the 16x16 blocks that were written. Paired and alternating,
whole-frame mean, this box: **full box (256 founders, 3 colonies) 6.6 →
2.7 ms, 128 founders + colony 7.0 → 2.8, 16 trees 4.1 → 1.1, one
small plant 2.8 → 0.5** — dials 2.6x → 6.0x, 4.0x → 15x
and 6.0x → 34x. Rayon measured a *net cost* at lab scale (the tick
10% faster on one thread than four), which is what the owner's 40% CPU
reading was. Report §16,
[`evolution-lab-frame-cost-2026-09-01.md`](Reports/evolution-lab-frame-cost-2026-09-01.md).

**The moisture pass and the pheromone plane, 2026-09-07 (report §17).** Two
more pure changes: the pheromone plane's 3x3 mean as an **exact** integer
sliding window (nine `u8`s sum below 2^24, so the old `f32` running sum was
already an exact integer — 2.6x on the phase), and the moisture pass reading
**its own chunk** rather than the chunk map, which is what §8 named and what
was the largest single function in the profile. Then a **behaviour change
behind a switch, default off**: `PIXEL_PHYSICS_MOISTURE_MARKS=cells` replaces
the pass's per-row mark hull with a per-cell bitmap dilated by the
4-neighbourhood — the exact set a write can change. It visits **1.9x to 2.9x
fewer cells and moves the same water** (`sw chgd` within 0.3%), and it is not
...
**`Reports/lab-behaviour-scenarios-2026-09-06.md` §4 item 1: the one piece of
engine work the report's whole approach needed.** `LabBox` (the bed) and
`params::Dials` (the rule and heredity switches) already persisted; nothing
the player painted by hand after the box was built did, so a bed like *"a
soil bank with a colony at its foot and beetles on top"* could be played once
and never replicated. A `scenario::Scenario` is a `LabBox` plus what gets
placed once at build time, what gets placed on a running schedule, and which
parameters-page knobs it turns first — a saved starting box the rack can run
a hundred copies of.

**Three ways in.** The BOX page's new `SCENARIOS` row, right under `WHAT
HAPPENED`, lists every file in `assets/lab_scenarios/` (tracked, shipped
content — not the gitignored `assets/lab_bed.ron` a player's own box saves
to) and loads one on a click. `PIXEL_PHYSICS_LAB_SCENARIO=<name>` or a
`scenario=<name>` argument does the same at startup. And `labbatch
scenario=<name>` / `labshot scenario=<name>` build the whole run from one,
so a scenario is a rack experiment and a single look at the same file.
Loading a scenario replaces the bed, stops the clock so the placements can
be looked at before anything runs, and every later `REBUILD` — by hand or
...
predator-prey persistence — no hand placements, built to be swept:
`labbatch scenario=gauses_jar sweep=compartments values=1,4,16`; its
colony and its beetles arrive on the timeline instead, at frame 3,000 and
6,000, so the herb founders get a head start before anything can eat them),
and `hunting_ground` (S6, flight from a predator, with the ant's
...
**A sixth landed 2026-09-18 with Phase 1: `the_pond_reeds`** -- `the_pond_sediment` cell for cell, with `reed` founders instead of `herb`, and it is the first bed in this repo in which a plant is alive under water. Its control is a switch rather than a second file (`PIXEL_PHYSICS_SUBMERGED_SHOOT=off`), and that control reproduces the old result exactly: four plants frozen at one cell each for 20,000 frames, against 281 cells standing in the water and a stem fourteen rows above the waterline with the switch on. **It also corrected `the_pond_sediment`'s own header.** That file describes two herb founders as placed "in the sediment"; stepped rather than read at frame 0, they rest on the **water surface** for the whole run, 23 rows above it, because `seed.ron` is density 0.6 against water's 1.0 and a plant seed floats. So the measurement Phase 1 was scoped from was a seed that never landed, and the phase needed a second change nobody had specified -- `SpeciesDef::seed_material`, so a reed's seed can sink. See the plan's §2a.
...
scenario=gauses_jar frames=0,100,1200` first exposed this: four colonies of
26 ants landed beside eight herb founders on frame 0 and the founders were
gone by frame 100. `Placement::Colonies` and `Placement::Predators` are the
fix — they spread across a bed's compartments the same way
`LabBox::colony_columns`/`predator_columns` do for `build_counted` itself,
so they can live on the *timeline* instead of at build time and a
`compartments` sweep still lands one colony per room rather than a fixed
column that only ever falls in one. `gauses_jar` and `two_larders` both
delay their colonies this way now; every other shipped scenario has no
plant in the room the colony threatens, so build-time placement was never
the problem there.

**Timing.** A scenario's build-time placements land inside `build()`, before
the box's first tick — frame 0. Its running timeline is read once per tick,
immediately after `frame::step` returns: `frame::begin_step` increments
`World::frame` first thing, so an `Event { at: N, .. }` lands in the world
whose frame reads `N`, the earliest frame it could possibly appear in. That
is also why `at: 0` refuses to load — nothing could ever reach it.

**What it costs the frame: unmeasurable, and here is how that was checked
rather than assumed.** `Scenario::due` returns before touching its own
timeline at all when the timeline is empty, so the steady-state cost of a
scenario with no schedule is one `Option` branch. For a scenario with an
active schedule
(`hunting_ground`, two entries), the first attempt at this measurement built
its control from a bare bed and its treatment from the same bed plus the
scenario's own placements, and read back a ~44x per-tick cost — which was
110 ants and beetles actually being simulated against a control that had
none, not `tick_timeline`. `World` is `Clone`, so the fix is the paired
control `CLAUDE.md` asks for: clone one already-populated 97-organism world
into both arms and vary only whether `tick_timeline` runs. Alternated over
2,500 ticks on one process, held clear of the timeline's own first firing
(frame 3,000) so this reads the "nothing is due" cost and not one
placement's own: **1,116,556.2 ns/tick plain against 1,115,500.0 ns/tick
with `tick_timeline` — a −1,056.2 ns/tick difference, 0.09% of the tick and
the wrong sign for a cost.** The mechanism is a `Vec` scan gated on
...
**Shipped ON 2026-09-06.** `PIXEL_PHYSICS_NUTRIENT=0` is the ablation.

**The report it closes**, from play in the evolution lab: a plant sprawling
in mid-air with water dripping on it, reading ROOT IN SOIL 0%, and not
dying. Light and water were the whole economy and free water supplied the
water, so a lit, drip-fed plant had everything the engine priced and could
not die. **Soil now carries a slow-renewing richness that free water does
not**, so a plant with no ground at all earns nothing and starves.

**Its shape is the finding, not its constants.** The term was a linear
weight first, and no setting of it worked: at status 0 the multiplier is
`1 - w`, so one number set both how hard a healthy stand is taxed and
whether a rootless plant can die. Measured over 12 paired seeds, weight 1.0
killed the rootless plant and left **0.171** of the stand; weight 0.25 kept
0.507 of the stand and let a rootless plant keep 75% of its income. It is
Michaelis-Menten now — flat where soil is fine, **exactly zero at status 0
for any `Km`** — so "no soil is fatal" is structural and `Km` says only what
*poor* soil costs.

**Both prices read that one curve, and the first attempt proved why.**
Shipped with the income term saturating and the construction price still
linear, soil at half stock was worth 0.955 to income and cost **x4.5** to
build in. That disagreement alone was **a third of the stand** (plants
0.672, down on 11 of 12 seeds). Joined, the same soil prices at x1.32.

**What it costs, measured against the ablation over 12 paired seeds:** plant
cells **0.843**, plants **0.842**, seeds borne 0.858, biggest 0.903 — about
a sixth of the stand, and **not unanimous: 4 of 12 seeds improve**. The
recovery period is 45 frames, which matches exactly one root's draw, so a
soil cell supports one root indefinitely and runs down only where roots
crowd.

**Known limitations.** Root *depth* is unmoved — 0.989 here and flat across
seven independent arms; this mechanism does not reach it. The self-thinning
seen at harsher settings (fewer, larger plants) does **not** appear at the
shipped ones, and whether a middle setting produces it without the collapse
is unswept. And `nutrient_status` is a whole-organism channel with no
connectivity check, so in the lab — where COLLAPSE UNDER LOAD is off — a
severed crown reads the nutrient standing of roots it is not attached to:
`Reports/open-bugs-handoff.md` §W7.
...
`corpse` was the one dead-organic material in the tree with no `decays_into` —
`deadleaf`, `deadwood`, `log`, `windfall`, `litter` and `ash` all have one. A
corpse is a `Powder`, so an animal that starved deep in a nest settled into the
gallery below it and stayed. `labnest` with an unfed colony: **37 of 178
refilled void cells are corpse**, unchanged from frame 6,000 to 18,000.
`labmass control=empty colonies=1`: **104 of 104 pool cells locked, 100%**.

It rots to `soil` now at `litter`'s 0.05 yield, fast in damp ground and slowly
in dry. **The silence in that file used to be deliberate and the condition it
depended on is now met**: `dead-ends.md` records it as *"Decay needed no hook:
`corpse.ron` declares no `decays_into`"*, which is a statement about the
**energy ledger** and never a decision about carrion. `decay::tick` is the
fourth `meat_lost` site now, through the same shared
`EnergyLedger::meat_worth_of` predicate the other three use, so
`max_standing_meat` is still a bound.

### A dying root gave back a twentieth of the cell it displaced

A root reaches its cell by *displacing* one — `growable` lets a `RootTip`
enter a penetrable `Powder`, and `displace_soil_water` runs ahead of the
overwrite to save the water. When the plant died, `shed_to_litter` put
`litter` in the hole, and litter returns 5%. So the round trip
soil → root → litter → soil destroyed about nineteen bed cells in twenty, once
per root cell per generation.

Turnover is what makes that visible, and it is why the default `labmass` arm
missed it: a bed measured on one cohort reads the loss as soil *borrowed*.
Culling half the stand every 4,000 frames over 40,000: mineral
**40,267 → 39,317** with only **282 cells standing as roots** — 842 cells of
bed destroyed. After: the bed falls 1,039 against **1,276 standing roots**, so
it is fully lent out and **8 cells are gone**.

Buried tissue rots into the ground it displaced. **This is conservation and
not a yield change**, and the distinction is enforced rather than asserted:
`World::bed_cells_on_loan` books a loan wherever a water-holding `Powder` is
taken over and the repayment is refused when the ledger is empty, so a stand
can never hand back more ground than it took. Two further conditions stop it
minting — no air among the four neighbours, and at least one mineral one — so
a leaf directly above a trunk does not turn to soil in mid-air (its downward
probe passes through organism cells and never sees air) and a root that grew
through an ant's gallery still falls as litter rather than plugging it. The
returned cell draws its water by **levelling its own nine-cell
neighbourhood**, which conserves exactly; `litter.ron`'s 5% is untouched, so
the forest floor argument in
[`soil-accumulation-and-the-carbon-cycle.md`](Reports/soil-accumulation-and-the-carbon-cycle.md)
still holds — every cell that reaches a *surface* still lands as litter and
still pays.

### What it costs, filed rather than absorbed

A rooted bank now sheds **more** of its own soil than a bare one — 0.383
against 0.252 — because it still *has* the soil, where the old rule let its
grass permanently eat 38% of it. The mechanic itself is unmoved: crest
occupancy is +30% for the sod arm under both rules, so a live root still holds
what it threads; what changed is the bank's afterlife, since a dead root now
leaves a loose grain where it used to leave nothing. Three repairs were
measured and none recovers it, and the only one that does mints water. Open as
`Reports/open-bugs-handoff.md` §W4, with the numbers and the untried
candidates.

### Known limitations

- **The lab's own colony still does not dig a nest you can photograph.** The
  wall census moves 40 → 182 and `roofed` void does not, because the colony
...
Both new rules ship with the same shape `lining_enabled` and `spoil_kept`
...
**Every founding line has a name, and the LOG page reads as sentences.**
`FIRST FED #4471` is now `VERNAL-9 FED HERSELF FOR THE FIRST TIME`;
`LINE 0 ENDED` — which is what *every* line-ending read before this, because
`LogEvent::other` is a `u16` and a lineage is a `u32` — is now `THE VERNAL
LINE ENDED, 14 GENERATIONS`. The name is a pure function of `(box seed,
lineage number)` through `rng::stream`, so it costs no storage, no
serialisation and no migration, reproduces on rebuild, and cannot move a
single cell: nothing here draws from a shared `Rng`. 251 stems, all A–Z so
the 5x7 font can draw every one (the glyph guard now walks the table), and
an individual is `NAME-GENERATION` — the ninth generation of the Vernal line
is `VERNAL-9`, and siblings share the tag on purpose; the pin resolves the
individual.

**Built for a thousand ants, not four.** The owner's constraint, 2026-09-09:
per-individual names, per-birth log lines and per-event pauses all drown at
the populations the game actually reaches. So identity lives at the
*lineage* (a colony click founds 52 of them), and the chronicle carries only
**line-bounded events** — bounded for the whole run per lineage, never per
birth:

| kind | fires when | bound |
|---|---|---|
| `LineEnded` | the last individual of a founding line dies | once per line |
| `GroupSplit` | `regroup_by_scent` mints a new colony label for a drifted cluster | once per minted group |
| `LineMilestone` | a line's deepest generation first reaches 5, 10, 20, 35, 50, 75, 100, 150, 200, or its living count first reaches 10, 100, 1,000 | ≤ 12 per line |
| `LineRecord` | an individual is the first of its line to sit a further 0.5 beyond the founder on one trait's −1..1 axis | ≤ 4 per trait per line |

The LOG page's new `SHOWING` row cycles `LINES / ALL / PINNED`. `LINES` —
the chronicle — is the shipped default; `ALL` is the page's old view, every
hatch and death, which is a wall of `BORN` the moment a colony gets large;
`PINNED` is the pinned individual's own timeline. Measured on the harness
bed (`colonies: 1, founders: 8`, 20,000 ticks): 60 lineages claimed, **52
line events against 149 births and deaths**, and 51 of those 52 are lines
ending — on that bed the 52 founders are 52 lineages and they starve. The
milestone and record kinds fire once a line gets deep or populous, which is
the played bed, not the harness bed.

**Evolution is reported as an event twice over.** On the line: a record or
a milestone is a log line. On the individual: `born_with` on every
`OrganismState` encodes the strongest single change at its birth (the trait
slot and a signed bucket of the −1..1 axis for animals; a discrete fate,
allele or parameter change for plants — never continuous jitter, which fires
every seed and would be 90% of the log), and the CELL page's story shows it
as `BORN WITH GUT +12%`, only when something did change. `brain::mutate`'s
return value — how many synapses moved — was computed and discarded before
this; it is read now.

**Guards, each watched going red.** `a_line_ended_line_names_the_line_that_ended`
(red on the tree before this); `line_events_are_bounded_per_lineage` (asserts
births happened first, or it is vacuous); `a_bud_that_mutates_records_what_moved`
(with `mutation_rate = 0` the field must stay 0 — without that arm a
formatter that always writes passes); a width guard that fails the moment a
...
pass a test that runs a handful of ticks, and land the reaction a thousand
frames late. `the_reaction_is_checked_inside_the_tick_loop` is red for that
placement. The detection is a difference of `RunLog::total()` across one
tick — monotonic, because `push` bumps `dropped` whenever it trims — so no
copy of the log is held.

**Built for a thousand ants.** Two things stand between an armed kind
arriving every few frames and a dial that can never leave 1x: the mask is
over line-bounded kinds only, and a four-second wall-clock cooldown swallows
everything inside it (the event still reaches the log). Measured on the
harness bed: the first reaction fired at frame 39 — a founder's line ends
almost at once on that seed — and over the next 20,000 ticks **one reacted
and 51 were swallowed**.

`Reaction` is mirrored into `Ui` the way `creature_colour` is, because the
BOX page reads a `Ui` that does not hold `TimeControl`; `Lab::act`'s
`CycleReaction` arm is the one writer.

## Life marks status — a pause-time mark, and what the owner rejected

**A marker over every living animal, off by default.** `Y` and the ANTS
page's `LIFE MARKS` row cycle `OFF / HALO / TICK`: a one-cell ring one world
cell outside the animal's body, or a single cell above its head, in the
colour the animal already wears under `ANIMALS WEAR`, mapped through
`world_rect_to_screen` so both scale with zoom. Neither touches a body
pixel — and that is enforced against *every* body on screen, not just the
mark's own: the first halo painted onto an adjacent ant's body at the nest,
and `a_life_mark_never_touches_a_body_pixel` caught it on its first run. An
active organism overlay (`L`) suppresses the marks, the same precedence
`render.rs` gives that dial over `group_colour`.

**The first form shipped was a solid 3x3 square over the animal, and the
owner rejected it the same evening** (card `20260909T193830388Z-0a8f39`:
*"I don't like the new"*). It made the animals findable by hiding the
animals — their two-cell bodies and the pink heavier-load cue under them.
And the finding it was built against was smaller than measured: *"Movement
is honestly the biggest thing that makes creatures visible. So if you are
just viewing static frames that explain why they are so hard to see."* The
0.016%-of-the-picture figure came from a contact sheet; a two-cell animal
that moves is findable in play and invisible in a still. So the marks are
a pause-time aid, they ship off, and **a visibility claim in this project is
judged on a moving sequence, never a sheet** — the review skill already
said so. The real answer to "the animals cannot be seen" is creature
appearance — bodies that are more than a two-cell chain — which is the
owner's stated number-one issue and gets its own lane.

**Cost.** The pass reads the world's live-organism list on drawn frames
only, never per simulated tick, and the lab has no dirty-rect skip to defeat
(`Ui::is_dirty` is unconditionally true). Measured paired inside one run
(`labshot bench=1`): 0.0013 ms per drawn frame at 59 animals, 0.0027 at
136; the thousand-animal figure is a labelled extrapolation (~0.02 ms), not
a measurement. `labshot marks=off|halo|tick` draws it onto the sheet.
...
shipped and read by no species, so an ant at 1% and one at 100% behaved
identically and the specimen page said *CANNOT FEEL HUNGER* in as many
words. `ant.ron` (and `ancestor.ron`, and the four appearance forks, which
were stale copies missing ten instincts and a hidden unit until this
change brought them level) now carries `(Energy, Move, -1.75)`, with
`(Bias, Move, 2.0)` kept as the *starving* ant's rate and
`(FoodAdjacent, Move)` re-derived from `-1.5` to `-1.16` — the shared-budget
trap the trophallaxis spec found: the stop-on-food weight was calibrated
against the constant bias, and left alone it froze a full ant on a leaf it
should have carried home. A full ant now moves on one tick in five instead
of two in three, still wanders, and forages harder as it empties. Digging
is no longer an unconditional drive: `(Bias, Dig)` is a residual `0.15`
behind a new hidden gate on crowding at the nest, so a colony digs as it
grows rather than tunnelling the world out of boredom — the owner's own
play report, *"they tend to just dig out the world when they have nothing
else to do"*, had an authored cause.

**Rest is the absence of a reason, not the presence of a full stomach.**
The owner's constraint, and the reason the bias came down rather than off:
the trail wires and the alarm wire are untouched, a hungry colony's demand
reaches the legs once trophallaxis lands (`KinNeed`), and an arm that
flattened the cliff by collapsing bed coverage was to fail.

**Measured, four arms, three seeds** (`labforage`, `labstats`, default bed).
A is the unwired ant; B is what ships; B′ normalises `Energy` to the
breeding bar instead of the founding grant; B″ is B at `-1.0`:

| seed | arm | alive at 4,500 | columns walked by 49,500 | digs by 9,000 | births by 49,500 | walking share of burn |
|---|---|---|---|---|---|---|
| 1 | A / B / B′ | 6 / **46** / 10 | 448 / 383 / 388 | 477 / 243 / 179 | 19 / **131** / 27 | 52% / 41% / 52% |
| 2 | A / B / B′ | 8 / **46** / 10 | 435 / 279 / 411 | 497 / 288 / 222 | 25 / 6 / 15 | 53% / 41% / 52% |
| 3 | A / B / B′ | 11 / **47** / 13 | 342 / 254 / 326 | 476 / 243 / 200 | 30 / 18 / 7 | 52% / 39% / 52% |

**The cliff is walking cost, and only rest closes it.** Every arm that kept
the founders walking — B′, B″ — put the cliff straight back where the
unwired ant had it. B ships with its price on the page: aggregate births
155 against 74, a lower per-seed median (18 against 25), 14–50% less bed
walked, and one seed at six births in 49,500 frames — a colony that
survives and mostly rests.

**And on the played bed, the wire stays.** The frame-0 cliff B closes is
partly the harness's own artifact (a colony founded on grown plants holds
31 under arm A already — see the tools section), so the number that decides
is A against B where the owner actually plays: `labforage ants_at=6000
frames=30000`, three seeds, the shipped ant against the same tree with
`(Energy, Move, -1.75)` removed:

| seed | arm | alive at 30,000 | births | intake J | burn J | columns walked |
|---|---|---|---|---|---|---|
| 1 | with the wire / without | 27 / 27 | 23 / 13 | 50,707 / 49,569 | 22,792 / 34,827 | 243 / 301 |
| 2 | with / without | 43 / 46 | 30 / 37 | 72,651 / 86,490 | 26,276 / 42,857 | 230 / 360 |
| 3 | with / without | 45 / 34 | 31 / 22 | 70,731 / 73,043 | 28,280 / 45,729 | 286 / 367 |

Neutral on survival, ahead on the birth median (30 against 22), a third
cheaper to run for the same intake, and a quarter narrower in range. Three
seeds is not a sweep; it is enough to say the wire does not hurt where the
game is played, and the composed Move row (`KinNeed`, below) is what widens
the range again.

**Two tests that assumed a full ant walks were repaired, not skipped.** One
asserted that the shipped ant never swings; it held only because two full
strangers walked apart before either bit, and a resting stranger eats the
non-kin ant beside it, alarm rises, and the alarm-to-attack wire shipped on
2026-09-08 fires as designed — the claim that survives is *never
unprovoked*, asserted beside a nestmate. The tunnel test gives its ant half
a grant so it is hungry enough to walk the geometry it tests.

**Also in the same change.** The ANTS, COSTS and GENOME pages describe the
*armed* animal — `COLONY_SPECIES = "ant"` was hardwired at three sites while
the chip's choice sat unread, so stocking a beetle and opening ANTS edited
the ant's numbers and saved to the ant's file. The shelter readout counts
real exposure — its counter sat inside a gate on a price no species sets,
so `labstats` printed *exposed on 0 of 33,848 ticks* on an open bed, a null
wearing a measurement. And `labstats` prints the flight counters and a
round-trip line, so the lab can see a hop or a delivery happen.

## Hopper status — the jump has a species, and it hops itself to death

**The engine's ballistic jump has an animal.** `BrainOutput::Impulse` →
`creature::launch` → `step_flight` shipped on 2026-08-29 with five guards
and the owner's verdict (*"with the new jump is great"*), and no species
file authored it: zero launches in every real scene, 275 the moment one
wire was added. `assets/species/hopper.ron` authors `(Bias, Impulse, 2.0)`
— byte-identical to `filmstrip`'s own positive control — with a faster tick
and its own body plan and colour, and sits on the COLONY chip. **It could
not hatch until a second file existed**: `place_creature` resolves an
animal's material by its species' name, and with no
`assets/materials/hopper.ron` the species loaded, showed on the chip, and
placed nothing — the wiring audit's pattern, one file short. With the
material (ant physics, warm ochre so a body in flight reads against dark
air):

| `labstats colonyspecies=hopper frames=3000` | |
|---|---|
| placed | 52 |
| launches | 2,551 (1,798 refused mid-air) |
| airborne frames | 29,421 |
| dead by frame 3,000 | 42, seven of them in the air |

**A bias of 2.0 on the jump was a hop on two ticks in three, and it killed
the animal in a session.** Measured against two alternatives
(`labstats colonyspecies=hopper frames=9000`, three seeds), the bias ships
at **0.5**: launches 2,791 → 2,187, refused-in-the-air 1,683 → 971,
starved-aloft 11.3 → 7.0, survivors unchanged. A hunger-gated jump was the
other arm and did not beat it. Still a hop on one tick in three, still
visible, and it no longer starves in the air on a third of its attempts.

## Rest status — a rest that ends, so a resting animal is not a stopped one

**The owner, 2026-09-13, overruling his own ruling of four days earlier:**
*"How long do they go without asking to move. If they never ask to move that
is still stuck, it is just because the rest mechanism needs fixing."* Rest is
still the absence of a reason to act (2026-09-09, Hunger status above); what
changed is that **having stood in one spot long enough is now one of the
reasons**.

**What was wrong.** `p_move` is `squash(sum).clamp(0.0, 1.0)`, and
`brain::squash` returns a genuinely negative number for a negative sum — so
every degree of "would rather not" lands on the **same exact zero**, and
`draw.unit_f32() < 0.0` is never true. An animal there cannot move at all, and
nothing it does changes that; only the world moving one of its other inputs
does. Measured over 120,000-frame beds (`CreatureStats::p_move_hist`, added
for this): **48.7–64.0%** of long-ant decision ticks and **54.4–77.5%** of
*shipped two-cell ant* ticks sat at that zero, with single stands reaching
33,300 frames. The shipped ant rests as long as the articulated one — this was
never a long-ant defect, it is every creature in both games, and it went
unreported because two motionless pixels read as scenery.

**What ships.** `BrainInput::Stillness` — the one input read off the animal
rather than off the world. `OrganismState::still_ticks` counts decision ticks
since the body last moved (reset by a step, a launch or a flight frame), and
the input is the **square** of `still_ticks / creature::STILL_SATURATION` (192
ticks, 1,152 frames for an ant), wired `(Stillness, Move, 1.5)` in all eleven
shipped creature species. A rest therefore ends in a step, the odometer
resets, and the animal settles again.

**The ramp is squared because `Move`'s negative half is doing two jobs.**
`creature_tick`'s own comment: *"a laden ant walking away from the nest scent
computes a low `Move`, fails the roll, and re-orients. That is the whole of
the homing mechanism."* So a low `Move` means either *re-orienting* (tens of
ticks) or *resting* (thousands of frames), and a linear ramp cannot tell them
apart — built that way first, it took `deliveries` from **4,908 to 54** on one
seed while the colony survived and simply stopped provisioning
(`Reports/dead-ends.md`).

**Measured, nine seeds, control being the same binary at
`wire=Stillness:Move:0.0`** — not `main`, because a new input column changes
`live_slots()` and therefore every `random_genome` draw:

| | control | with the rest fix |
|---|---|---|
| longest stand, long bodies (stops of 900 frames) | 13–47 | **1–4** |
| longest stand, all bodies | 23–66 | **3–6** |
| ...shipped two-cell ant, `played_bed` | 45 / 30 / 41 | **6 / 7 / 6** |
| **median rest** (`rest_bout_hist`, the animal's own ticks) | 2–4 | **2–4, unchanged** |
| rests longer than 1,024 ticks | 18–422 | **0–14** |
| decisions that produce no step | 84–96% | **83–94%** |

**The short pause is untouched and only the tail is cut**, which is the point:
an animal still declines to step on the overwhelming majority of its
decisions, so this is a distribution with a middle rather than a colony that
never settles.

**What it costs.** Deliveries per ant-tick fall on 7 of 9 seeds (median
2.68 → 1.63) and steps per delivery roughly double — the homing term being
partly overridden — though the two seeds where the control's foraging had
collapsed entirely recover. Starvations fall on 7 of 9. Colony size is not
moved (up on 4 seeds, down on 5). **No measurable frame cost**: `antcost` at
~650 standing ants, four runs alternating, reads 12.14 and 12.31 µs/ant on
`main` against 11.22 and 12.68 on the branch — the branch's own two runs
bracket `main`'s — and ~8.0 ms/tick whole-frame on both. `live_slots`
**846 → 870**, so every species' `mutation_rate` is re-derived to
`3.18 / 870 = 0.0036552` in the same change.

Known limitation, and it is the open question: the delivery cost is real and
has not been isolated from colony drift. The clean repair is a signal that
separates *navigating* from *resting* as data rather than by timescale;
nothing in the brain says which an animal is doing.
`Reports/evolution-lab-rest-with-an-end-2026-09-13.md` and
`Reports/open-bugs-handoff.md` §Z13.

## Creature tools status — a hand in the box: scent, alarm, fling, lamp

**The owner's idea, the same day it was proposed**: *"it might be fun if
the user could manually lay down pheromone trails or have other tools to
interact with the creatures."* Four verbs, each on a key and on the help
page, because the bar is measured full:

- **`I` — SCENT.** Drag to lay the food route (channel B, the plane an
  *empty* ant follows outward); a second press switches to home scent
  (channel A, the plane a *laden* ant follows back) without disarming the
  tool. The deposit ramps up from where the drag began, and that shape is a
  finding: a flat deposit carries no direction at all, because the ant's
  along-heading sense reads `(ahead − here) / (ahead + here + 1)`, which is
  zero everywhere inside a plateau. Only a slope says *that way*.
- **`J` — ALARM.** Drops alarm scent at the cursor at the strength a bite
  writes, and the alarm plane — 512 deposits per 4,000 frames on the
  shipped bed, invisible until now — has its overlay (`O` cycles it;
  `filmstrip channel=pheromone_alarm`).
- **`Q` — FLING.** Click an animal and it launches on the engine's jump,
  away from the side you clicked, refused in mid-air. The cheapest way to
  see the hop before the hopper.
- **`U` — LAMP.** Click a fixture to remove it, bare ceiling to place one,
  drag to move it. `place_lamp`, `remove_lamp`, `move_lamp` and `lamp_near`
  were fully built with zero callers — `lamp_near`, the click hit-test that
  exists for a UI, had none even in tests. Light layout is a lever now.

**And the finding under the scent tool, which matters more than the tool.**
Its positive control laid a real, climbing trail from the nest to a plant
and counted ants near the plant with and without it, 3,000 ticks, twenty
animals: **1,903 ant-ticks, byte for byte, both arms, twice.** The tidy tie
is the tell. The arithmetic on `ant.ron`'s hidden layer says why: the laden
gate is `-45 + 75·Carrying`, which parks the unit at 30 on the squash curve
where the slope is one in a thousand, so the `±6` trail term moves the
chance of a step by about **±0.003**. Round twenty-four's *"the circuit is
live end to end"* is true, and the circuit is inert — an additive gate large
enough to switch a unit off flattens the signal it lets through. Filed as a
bug with its bar (`Reports/open-bugs-handoff.md`); until it is raced, the
scent tool lays a gradient the shipped ant cannot act on, which the ethos
calls unfinished, and the starvation report's cold-start hypothesis sits
upstream of a reader that cannot read.

**The played bed, at last.** `labshot` and `labforage` take `ants_at=N`:
the colony is founded at frame N on a bed grown bare until then, which is
how the owner plays and not how any harness had ever run. Same seed, same
bed:

| frame | colony founded at frame 0 | founded at frame 6,000 |
|---|---|---|
| 6,000 | 5 | 39 |
| 12,000 | 4 | 31 |
| 30,000 | 13 | 31 |

Dropped on seedlings the colony collapses at once; founded on grown plants
it seats fewer and holds. Every §Z6 figure quoted before 2026-09-09 was the
left column. `plant=` on the same harnesses (and on `chronicle`) names the
species, because every one of those figures was also on eight herbs.

## Trophallaxis status — the colony's stomach, shipped on, and a first measurement that says wait

**An ant can feed a hungry nestmate.** Owner's ruling, 2026-09-09: a brain
output the genome can evolve, shipped on, never a rule — a rule-based
always-on transfer hardcodes the colony where the output exposes it.
`BrainOutput::Share`, when it fires beside living kin, moves energy down
the gradient to the neediest neighbour; `BrainInput::KinNeed` is the
largest deficit among adjacent kin, computed in the pass the mouth already
makes; one constant, `SHARE_FRACTION = 0.25`, is at once the cap (the donor
stays the richer), the floor (a share can never kill the giver) and the
grading; the price is one jaw closure through the existing bill. Energy,
not crop, because a 480 J leaf arriving in one lump is another binary. The
default wiring, `(Energy, Share, 2.5) (KinNeed, Share, 1.9) (Bias, Share,
-2.5)`, is derived so a full ant beside a starving sister shares on two
ticks in three and a starving ant never gives away its last joules; the
predicted behaviour is *whoever eats redistributes*. Counters `shares` and
`shared_j` print in `labstats` and `labforage`; `PIXEL_PHYSICS_TROPHALLAXIS=off`
is the ablation switch, proven in both directions. Cost: 706 → 763 live
genome slots, `mutation_rate` re-derived in three files, every ant's RNG
stream shifts. The plainspeak page says `FEEDS HUNGRY NESTMATES`.

**Founders no longer start on identical wallets.** `founder_reserve_spread:
0.5` on the ant draws each founder's grant from `start_energy ± 50%` through
a pure hash, symmetric so the cohort's total is unchanged — the cheap half
of the queen question (the synchronised cliff exists *because* every founder
carried the same number). Measured alone, sharing off, 6,000 frames, three
seeds: mean survivors **8.0 → 15.7**, intake +29%. It ships at 0.5.

**The first measurement, on the harness bed, was a null with a warning
sign; the second, on the played bed, is the answer.** Same seeds, harness
bed, 6,000 frames: sharing on ended with fewer than half the survivors of
sharing off on two seeds of three, inside an eight-fold spread, and undid
the founder stagger it shares down. But that bed is 52 ants dropped on
seedlings. On the played bed — colony founded at frame 6,000 on grown
plants, 30,000 frames, six seeds — the picture inverts and grades:

| arm | survivors | births | deaths | intake J | columns walked |
|---|---|---|---|---|---|
| sharing off | 38.7 | 29.7 | 27.2 | 65,300 | 263 |
| sharing on | 43.7 | 19.3 | 11.8 | 58,200 | 257 |
| sharing on + `(KinNeed, Move, 1.25)` | **46.0** | 22.3 | 12.5 | **64,900** | **282** |

Sharing alone buys survival — deaths halve — at the cost of births, intake
and range: a colony that pools its stock forages less. The one line the
spec designed beside it, a full ant beside hungry kin going *out*, keeps the
low deaths and recovers the intake and the range, with the most survivors on
five seeds of six. That is the composed Move row (`Bias 2.0`, `Energy
-1.75`, `KinNeed +1.25`, `FoodAdjacent -1.16`), and it ships on the seven
ant files. The first arm of this measurement was built from an unedited
file — its edit's assertion failed on a spacing mismatch and the arm came
back byte-identical to "on" on all six seeds, which is this repo's standing
tell — and was rerun with a check that the edited species file is embedded
in the binary. Sharing is not exclusive with moving or eating (checked in
`act` and by two variants that measured inside the shipped arm's spread).

**Held for the marker pass:** the on-screen share flash; `last_share_frame`
on every animal is written for it.

## Stacking status — many ants of one colony in one cell, off by default

**Exclusivity belongs to the grid, not to creatures**, and that is the whole
shape of this. `Cell` carries one `organism_id`, so `classify_step` reads a cell
held by an ant exactly the way it reads rock; nothing in `creature.rs` forbids
co-occupancy, the array does. So the grid is left untouched — one owner per
cell, unchanged meaning for all 366 readers of that field — and everyone else in
the cell is a **rider**, in a sparse `PosMap<Vec<Rider>>` no existing reader
consults. `World::stack_cap` is the toggle **and the rule**: at 1, *may I enter a
cell holding one creature* **is** *is this cell occupied*, so the old behaviour
is a value of the parameter and there is no forked logic to keep in sync.
`PIXEL_PHYSICS_STACK_DEPTH`, the BOX page's `animals_per_cell`, default 1, 20 to
arm. Owner's rulings: no cost, a cap, same colony only, a strike hits the cell
owner, fire hits everyone in the cell, an involuntary exit promotes the oldest
rider, a corpse goes to the first free neighbour else is suppressed **and
counted**. Design of record, with the four candidates and why three were
refused: [`Reports/creature-stacking-design-2026-09-17.md`](Reports/creature-stacking-design-2026-09-17.md).

**The failure this is built against is freezing, not dying.** A rider owns no
grid cell and the scheduler site sits on the head, so `creature_tick`'s opening
guard reads a mismatch and returns **no site**: not dead, not scheduled, an
orphan standing in the world for ever. A frozen ant is never charged, so it
never starves, and a frozen colony reads as a *thriving* one — measured once
already on the pass-through branch at `ticks` per 1,000 ant-frames of 42 and 24
against the 167 a 6-frame interval implies, with a whole 12-seed sweep that was
the bug rather than a result. `reconcile_chain` resolving a body against
`OrganismState::cells` rather than against the grid is what makes a rider alive
by the engine's own test, and is why there is no death cascade to design for.

**Read `stacks_entered` and `max_stack_seen`, never `stacked_cell_count`.** The
latter two are standing censuses of the final frame and a stack is momentary by
design — riders dismount as an ordinary move — so the snapshot understated
frequency **33x** on the thousand-ant bed (15 standing against 498 events) and
went red on a run that stacked *more*. `shares_in_stack` is the same kind of
number for trophallaxis reaching a rider: 43–55 over six seeds at cap 20, 0 at
cap 1.

**No render work, cancelled on a measurement rather than a change of mind.** A
cell with three ants in it draws as one ant. At play zoom one cell **is** one
screen pixel, so there is no sub-cell space for a pip, and
`creature-appearance-design.md` measured that extent is the only appearance
lever and has to roughly quadruple — a stack has no extra extent by definition.
So the payoff is judged as *flow* rather than as visible piles: ants moving
where they used to stall.

**Known limitations.** A falling stack drains one rider per tick, so twenty ants
in one cell come apart over twenty ticks rather than falling together. A
re-emerging `Segmented` lateral is written with a plain `World::set` and does not
ask `can_stack_into`, so above cap 1 a `longant` re-widening into a nestmate's
cell would overwrite it rather than ride it (`longant` is the only `Segmented`
species shipped). Suppressed meat is counted in `corpses_suppressed` /
`corpse_worth_suppressed` but not booked to `EnergyLedger::meat_lost`, which
leaves `max_standing_meat` a looser upper bound rather than a wrong one.

## Trail status — the ant reads its way home, and the food trail is the larder's problem

**The homing trail works; the food trail is deliberately left.** §Z7 found
the trail circuit wired and inert, and the fault is a regression with a
date: the gate was once `Bias -30, Carrying +30`, which sums to zero when
carrying and sits on the responsive part of the squash curve; the crop-fill
rework rewrote it as `75·(fill − 0.6)`, moved the open state to +30 where
the slope is a thousandth, and called it a virtue. The shipped ant followed
a trail only in a narrow band of crop fill around 0.6 and was inert when
empty — the state of every ant following the food route. The null was
reproduced on an independent harness (`examples/trailfollow.rs`: 595
near-target ant-ticks with a laid trail, 595 without, exactly) and broken
by re-gating (open state +0.5: 13,796 against 595, three seeds of three).

**And the bed punished the ant that had it.** Both pairs re-gated, the
arena (`creature_arena`, mirrored, six seeds, 24,000 frames) put the
re-gated line at **25% of animals, zero seeds above half**, against a
zeroed-brain control at 15% and a position control at 41%; the played bed
lost survivors on three seeds of three, and the diagnostic was
nest-clustering — the doorstep band eaten out, the far band untouched.
Clumping the larder (`plant=tree`, `plant=conifer`) narrowed the gap to
nothing and never reversed it. The arena then split it: **the homing pair
alone reads 63.6%, five seeds of six above half; the food pair alone
47.6%.** Directed return plus random search is a central-place forager;
directed search toward patches the colony has already eaten is not,
because channel B is laid by ants coming home from a patch already found.

So units 0 and 1 (a laden ant follows home scent) land at `-45 / +45.5 /
±6`; units 2 and 3 keep the saturated gate with a note beside them, and §Z7
stays open for that half, restated as a finding about the larder rather
than the gate — recruitment pays where food is patchy and worth
advertising, and a decay knob for the food trail (`bdecay=`,
`Pheromones::set_channel_rho`) is in for whoever picks it up. The `SCENT`
tool now defaults to home scent, so what a player draws is a road home
that laden ants actually follow; its second press lays the food route,
which the help text says no ant can read yet.

## Trail lifetime status — the way home keeps, the food trail still fades

**The two trail planes now forget at different speeds, and that is the
change**: `pheromone::TRAIL_A_RHO = 0` against `DECAY_RHO = 0.03`, which
channel B keeps. `ALARM_RHO` already established that a plane's lifetime
belongs to what the plane is *for*; this splits the trail planes on the same
axis, because the jobs are not the same job. Channel B is news about a patch
and its decay is the lever against a trail that outlives what it advertises
(§Z7). Channel A is the way home and a nest does not move.

**The defect it fixes is arithmetic, not tuning.** A laden ant nets about
**0.014 cells per tick** homeward at best, so a 90-cell walk is tens of
thousands of frames — 2,000 decay passes at `PHEROMONE_INTERVAL`. At any
non-zero rate the homing plane is gone between one ant's visit and the next.
`examples/pherolife sweep=rho` states it in the units that matter, on its own
2,200-frame round trip: at the shipped 0.03 a trail laid once is **no longer
worth steering by after 0.49 of a round trip** and gone at 0.67; at zero those
are **1.06 and 2.04**.

Measured, 36 seeds at gap 90 on `trailfollow`, paired within seed on the share
of ants that reach the larder and then reach the nest band again:

| channel A rho | route a homing ant can read | foodward | plane lit | round trips closed | sign b/w/t |
|---|---|---|---|---|---|
| 0.03 (shipped) | 13.6% | 12.9% | 26.1% | **0.88%** | — |
| 0.01 | 20.5% | 15.0% | 35.4% | 1.02% | 12/8/16 |
| 0.005 | 27.9% | 17.6% | 44.8% | 1.51% | 16/8/12 |
| **0** | **53.2%** | 15.6% | 69.2% | **14.84%** | **23/6/7** |

Monotone in both. The foodward column barely moves while the homeward one
quadruples, so the plane does not merely get louder — it becomes a ramp that
points home, 3.4:1 against the shipped arm's 1.05:1 coin flip.

**It does not mean the plane never forgets.** `ascii scene=pheromone` is the
control and is titled for the claim: at rho 0 a blob still reaches `max 0` by
frame 4,000 and the plane still sleeps, because a 3x3 mean of a thin trail
rounds to nothing. **Diffusion is the eraser** — 16.7% per pass against decay's
2.9% — and what the constant was adding was a second, faster one on top.

**Cost**, `ascii scene=ants` over 12,000 frames: pheromone tiles per pass
**16.7 → 17.4**, mean frame 0.779 → 0.773 ms. A 4% rise in awake tiles and no
measurable frame cost; the counter moving *up* is what rules out a cost that
vanished because the work did.

**How it works is not the obvious reading.** The share of laden ticks on which
an ant reads the plane as pointing its way *falls*, 17.0% → 6.6%, because a
plane lit on 69% of cells instead of 26% converts "no readable gradient" ticks
into readable ones, most of which are downhill. What rises is what such a tick
is **worth**: on a scatter, "uphill" points at the nearest random burst and is
worth +0.0026 cells homeward; on a persistent plane it points at the nest and
is worth **+0.0602**. Pooled drift +0.00068 → +0.00541 cells/tick — 21.6 cells
...
**Nothing in this engine had ever died of age.** `DeathCause` had Starved,
StarvedInFlight, Killed, Culled and two plant causes, and
`OrganismState::born_frame` was an age nothing read for a creature. So every
death in every session was hunger or a fight, and a colony could only shrink
by famine — which is one shape, and the owner watched it: a colony booms on
the bed's larder to hundreds, strips the bed, and goes all at once.

**The ant now has a lifespan, and it is the plants' hazard rather than a
second model.** `CreatureDef::life_half_life` (frames; `0` is immortal and is
every species' default, so a species file that does not name it is
bit-identical) is rolled once per creature tick through
`plant::old_age_chance_over` — a Weibull with shape 2, so **T is the median**,
**96%** of a cohort is still walking at `T/4` and **1.3%** reaches `2.5T`. A
cohort therefore dies over a spread rather than all at once, which is the
ethos's first law arriving on the animal side of the box, and the corpse is
laid exactly as a starved animal's is: a death is also carrion a nestmate can
eat. `DeathCause::OldAge` is appended to `DEATH_CAUSE_LIST` rather than filed
beside `Starved`, so no existing cause's index moves. Ant and long ant are
authored at **40,000 frames**; hopper, flitter and beetle stay at 0.

**The interval had to become an argument, and that is the part worth
copying.** `old_age_chance` baked `ORGANISM_TICK_INTERVAL` (45) into the
per-tick chance; an ant rolls every 6. Called unchanged it would have put an
ant's median at `T/2.7` — and `pace` is a heritable trait, so how often an
individual takes its turn would silently have decided how long its lineage
lives. `old_age_chance_over(age, T, interval)` takes the *individual's* own
interval and divides it back out, and
`the_hazard_is_a_property_of_the_half_life_not_the_interval` asserts the three
survivorship numbers hold at intervals 6, 12 and 45.

**What it does to the boom, measured.** `latecensus scenario=played_bed
frames=500000 sample=20000`, `RAYON_NUM_THREADS=1`, peak ants and the frame
it fell on, then ants/plants/bank at 500,000:

| seed | lifespan | peak ants | at frame | 500k ants/plants/bank | OLDAGE / starved |
|---|---|---|---|---|---|
...
Measured on `main` at **`c7ee0f40`**, after it shipped nest odour and
`scent_drift` at 0.15, which moved this bed under the first version of this
table (the before-and-after is below).

**These rows are one trunk behind the code they ship with, and deliberately
so.** This branch merges `39b31c7e`, which lands seed cargo, the drained door
and the long-ant pile — three more changes to the same bed, and the round's
queue had no room for another 500,000-frame sweep before landing. The owner
ruling this ships under is that the mechanism and the dial are the deliverable
and the constant is re-derivable from the ANTS page. Whoever re-derives it:
the three-seed sweep is six runs of `latecensus scenario=played_bed
frames=500000 sample=20000` at `RAYON_NUM_THREADS=1`, about two hours on four
cores, and the `killed` column below will not stay near zero.
...
**137 → 63**, plants standing **34 → 101**, seed bank 85 → 166, animal
generations **5 → 15**. Age deaths thin the colony before it strips the bed,
...
here and in `plant.rs` as `0.4%` and is **1.3%** (`0.4%` is `2^-8`, the
survival at `2.83T`); no bar depended on it, so the model is unchanged.

## Held world status — the third game, and time you carry in a circle

**`cargo run --release --bin druid` is a third game on this engine**, beside
the outdoor sandbox (`src/main.rs`) and the evolution lab (`src/bin/lab.rs`).
Owner's ruling, 2026-09-13: *"I don't want to build this into the existing
gnome game. This is a fully new and separate game."* The design of record is
[`Reports/held-world-game-concept-2026-09-13.md`](Reports/held-world-game-concept-2026-09-13.md);
this section is what is built.

**The premise is an inversion.** The land is *held* — nothing grows, breeds,
ages, rots or weathers, and the sky does not turn. Physics is untouched: rock
still falls, water still flows, a struck wall still comes down. What is
stopped is *life*. The player carries the only time there is, in a small
circle that follows him, and pays for standing ones he leaves on the ground.

**It buys the thing a whole-world speed dial cannot.** Running an entire world
fast is what the sandbox cannot afford; running it all slowly means you can
never grow a tree. Holding everything and quickening a circle is the same
trade taken the other way round, and it costs what is *inside* the circle
rather than what is on the map.

**Everything it stands on defaults off**, so the sandbox and the lab are
byte-identical without it. `World::held` is `false`, `World::quickenings` is
empty, and `World::time_runs_at` returns `true` on the first comparison —
`render.rs`'s `the_other_games_cannot_see_the_held_world_look` and
`plant.rs`'s `an_unheld_world_never_consults_its_quickenings` are the guards
over that.
...
*past* this point, so gating it too would leak organism slots for as long as
the world stayed held.

The per-cell gate re-dates rather than drops. A dropped site is gone for good;
a re-dated one costs one heap pop every two seconds and is still there when a
circle arrives over it. **Placing a circle does not wait for that**:
`World::wake_region` drains both scheduler heaps and pulls every site inside
the new circle forward to *now*, so a placement lurches into life instead of
trickling in over the recheck interval. It only ever moves a site **earlier**,
which is what keeps it from being a way to postpone work.

**A held world is measurably held**, positive control on `scene=forest`,
living plant tissue at frame 2,400:
...
**A world held at frame 0 contains nothing alive**, and this was very nearly
shipped. `life_scatter` places a *single* `seed`-material powder cell per
plant, so holding immediately means no seed ever germinates: the land is bare
ground plus a scatter of seed pixels, for ever. The fix is also the fiction —
the land was alive and *then* stopped — so `Druid::new` runs `GROW_FRAMES`
(8,000) of full ticks before setting `held`. Measured, `scene=worldgen` unheld
at 512x320, living plant tissue:

| frame | 0 | 2k | 4k | 6k | 8k | 10k | 12k | 14k |
|---|---|---|---|---|---|---|---|---|
| cells | 20 | 3,252 | 9,084 | 14,474 | 19,244 | **20,247** | 18,469 | 16,535 |

The stand peaks near 10,000 and then **self-thins**, so past that you are
freezing a wood already in decline. 8,000 sits on the rising side: a young
mature wood rather than a tired one. It is a knob
(`PIXEL_PHYSICS_DRUID_GROW`) and should be read as *the age of the world at
the moment it stopped*, not as a technical budget.

It is also the whole of startup cost — **3,772 organisms in 53.8 s** at the
shipped 2560x960 — and the reason the size and grow length both take
environment overrides.

**The bones come free.** Standing grown plants that are simply stopped *are*
the "somewhere that died" readability the concept asked for, with no deadwood
or stump pass to write.

### The world it builds

**2560x960 — about five screens wide and three deep**, against the sandbox's
8192x2560. This game has no mining, no caving and no destruction to spend
depth on, and depth is where generation cost lives. It builds from the
`druid` preset in `assets/worldgen.ron` (more soil, less rock, no vaults or
pockets, a higher water table); the size is a constant plus
`PIXEL_PHYSICS_DRUID_SIZE`, and the preset takes `PIXEL_PHYSICS_DRUID_PRESET`
so two worlds can be judged side by side rather than one against a remembered
impression.

**No `Spec` variant.** Expressing the subset as a worldgen `Spec` would buy a
match arm in `generate_ablated` and `generate_reported_with`, a moved
interference matrix, and a harness `scripts/worldgencheck.sh` does not gate —
it gates *presets*. Preset data is the gated form, so the preset is the form
this uses.

**Spawning needs a dry column.** `Player::at_scaled` (never `at`, which builds
a half-size gnome at any `cell_scale` but 1) needs a y on the ground, and the
sandbox never auto-spawns — it summons the gnome where the player clicked — so
there was no pattern to copy. The first generated world put a pond exactly at
...
its radius** — honest to the engine, since that is literally what costs, and
it means the same circle gets dearer as a colony grows in it. **Income is
animals only** (owner's ruling): a wood with no colony pays nothing.

| term | value | |
|---|---|---|
| `POWER_START` | 600 | |
| `DRAIN_PER_CIRCLE` | 1.0 /s | per standing circle, before anything in it |
| `DRAIN_PER_PLANT` | 0.02 /s | per plant inside a **standing** circle |
| `INCOME_PER_ANIMAL` | 0.6 /s | per animal wherever time runs, carried circle included |
| `ECONOMY_INTERVAL` | 30 ticks | one walk over all organisms, twice a second |

The carried circle (`CARRIED_RADIUS` 28) is free, so a colony under your feet
pays without costing and walking somewhere is worth doing. Running out does
not end anything: the newest standing circle closes, so the map contracts
rather than stalling.

**These are round numbers chosen to make the meter move at a rate a person can
watch, and nothing has been measured because nobody has played it yet.** `U`
takes the economy out of the way entirely so the mechanics can be judged
without it — which is what it is for, and why the readout says `POWER
UNLIMITED` rather than showing a full bar, since a full meter and a disabled
one look identical.
...
does, so a placed quickening had no appearance whatsoever. Gold is the circle
you carry, faint grey is what `SPACE` would place at the current radius, and
blue is a standing one. Deliberately plain: this is interface, **not the rim**,
which is arrested matter and its own piece of work.

**The panel fill is written, not blended, and that is the interesting part.**
The sandbox and the lab both draw translucent panels and both force a full
redraw every frame an overlay is up, which is what makes a blend safe there.
This game's whole premise is a world standing still, which is exactly where
the dirty-rect skip earns its keep — so the interface forces a redraw only
when it *changes*, and a blend into a region the renderer skipped compounds.
Measured on the first headless shot: four frames of `blend(.., 0.72)` over
unchanged sky left the panel at **(9, 12, 20)**, flat opaque, against the
**(30, 44, 63)** a single pass gives. Worse than being too dark, it
*oscillates* — a chunk updating underneath repaints that strip back to
one-pass translucency. Everything drawn here is now idempotent, and
`drawing_the_same_interface_twice_changes_nothing` is the guard.

### How held ground is drawn

A runtime selector rather than a decision, per the house rule. `L` cycles it.

| | |
|---|---|
| **`Unchanged`** (default) | held ground draws exactly as running ground does; the only tells are that nothing moves and the sky does not turn. The owner's pick, and the cheapest — one comparison per cell |
| **`OneHue`** | held ground holds a single cold blue-grey, **luminance preserved exactly and only the hue replaced**, so every silhouette, shadow and depth cue survives and the picture reads as *lit differently* rather than as broken |

Owner's verdict on `OneHue`, 2026-09-13: *"It is a decent first step. We will
want to improve it later."*

The quickening set is folded into the renderer's `LookKey`, and it has to be:
...
this is a limitation of the engine rather than of the game. No world-edge
behaviour beyond the sandbox's. No determinism statement. The rim, the sky
treatment for a held world, and the three other look treatments (rime,
photograph, ash) are on the list and not started. The economy is unbalanced
by construction, per above.

## Colony books status — what each colony is living on, in joules

**The world's energy ledger already balanced; what nothing could answer was
whose energy it was.** `World::energy_ledger` is a closed double-entry set of
accounts with its conservation identities asserted by tests, and every number
in it is world-wide — so "is *this* colony feeding itself" had no reading at
all, in an engine where starvation is 70–90% of every ant death in the lab bed.

**Shipped:** the same accounts kept per `OrganismState::colony`
(`World::colony_books`), a diet band booked in joules by source material, a
`raided` / `raided_by_others` pair for flesh taken across a colony line,
`examples/colonybooks` to read all of it from a terminal, and the lab's FOOD
page to read it in play. Reached from the ANTS page's own **WHAT THEY ARE
EATING** heading and from MENU; it has no key, because the bar has been
measured full twice and the key table was another lane's file that round.

**Two design facts are load-bearing and easy to get wrong later.**

**Splitting a closed ledger per colony breaks closure unless transfers between
colonies become their own account**, and the transfer here is trophallaxis:
kin is `creature::is_living_kin`, which is *species* identity unless
`World::colony_rivalry` is on, so a shipped bed lets two ant colonies feed each
other mouth to mouth. Measured on the guard's own two-colony bed over 12,000
frames: colony 1 gave 1,122.75 J and took 1,207.65 J, so **84.9 J crossed the
line**, against a world-ledger rounding drift of 0.049 J. Booked as the matched
`SharedOut`/`SharedIn` pair, both colonies close; without the pair the per-colony
identity is short by exactly that 84.9 and the temptation is a free term — the
failure `EnergyLedger`'s own doc records costing 300 conjured joules.

**The split is live-only, and meat stays one world-wide pool on purpose.** A
corpse cell carries no colony: `Cell::aux` holds its worth and there is nowhere
to put a label. So colonies pay into meat (`StoredInMeat`) and draw out of it
(`HarvestedCorpse`), `meat_lost` stays world-wide, and no per-colony meat
identity is claimed. `raided` is face value at the bite and deliberately
appears in no identity at all.

**Known limitations.** No key opens the FOOD page. The page draws as many
colony blocks as the screen holds and names the rest rather than scrolling.
...
colonies never feed themselves: ANT 1 took in 10,400 J of founding grant and
foraged **399 J**, and both colonies together foraged 485 J against 18,800 J
granted — **2.6%**. They ate 84% and 95% corpse. That is the mechanism under
"almost every death here is starvation", stated in joules: not that the colony
cannot find its dead, but that plant income is approximately zero. Walking is
27% of everything spent and the brains 8.3%.

**And that reading is now the first thing on the page, because the page it was
buried in did not survive its reader.** The owner's verdict on the first FOOD
page was *"Nope. I don't understand what these visuals are trying to tell"* —
not a complaint about a chart type. Every row on it was true and **none of them
was an answer**: it said what a colony had eaten, spent and was holding, and
never whether that added up to a colony that can keep itself alive. Each
colony's own line now opens with **`FINDS n%`** — the share of everything it
has spent that it went out and found in the world for itself, with the founding
grant excluded because it runs out once and scavenged corpse excluded because
it is food something else already paid for. On the shipped two-colony bed that
reads 13% and 9%. **Both charts also state their own peak**, top right: they
...
**Two channels on `F7` in the lab** (`src/food_road.rs`, drawn by `render.rs`):
a **road**, per cell and decaying, split into where animals walked *carrying*
(a saturated red-orange to near-white) and where they walked empty-handed (cool
blue); and a **harvest map**, per 8-cell tile and per colony, in that colony's
own hue, weighted by the face value taken out of the world there. Observed from
the tick loop and never from `draw` — the lab's rack draws other chambers
through the same renderer, and observing at draw time read another world's
animals into this map against `u16` handles every world reissues. Off by
default and free then.

**The two food numbers disagree on purpose**: the colony books price a mouthful
by the eater's gut, this map prices what left the world at that tile.

**The owner rejected the first version, and both complaints were one defect
each.**

***"Why is the amber hatch drop like a huge box?"*** — the wash was painting
**sky**. Every live harvest tile sits on the surface band, where an 8-cell tile
is about one row of ground and seven rows of air, and the wash covered its
whole tile: the shape on screen was the tile grid, not the ground. It is now
clipped twice, and **either clip alone still draws a box** —
`harvest_on_ground` refuses open air, and `near_open_air` refuses ground more
than `HARVEST_SKIN` = 3 cells under the exposed face, without which the block
is simply anchored to the bottom of the tile instead of the top. `HARVEST_MAX_
COVER` went 0.8 → 1.0 in the same change: the fifth held back existed so the
tile did not cover the plants underneath it, and the skin does not cover them.
**Shrinking the tile was ruled out by rendering it** — `tile=4` still draws
boxes in the sky, only smaller, and `tile=1` loses the channel into the road
entirely. The box is the air, not the size.

***"Is it too much to track actual paths and make trail?"*** — it never was,
and the road was already tracking them per cell, every tick. It was
**forgetting** them: `road_half_life` was 600 frames, so what reached the
screen was the last few hundred steps anybody took. At **3,600** — a minute,
already the harvest map's own half-life — the same run holds 1,024 road cells
against 279 and draws one unbroken line. **The longer memory is free**, which
is the half of the answer that needed measuring: on a settled bed, where the
channel is dearest because it decays every tick and so defeats the dirty-rect
skip, the short memory cost **+0.82 ms** a frame over having the view off and
an unlimited one **+0.69 and +0.71 ms**, twice, at 87% more road; over a whole
running frame, **+0.56 ms against +0.47**. The control that says the instrument
can see drawing work at all is the second channel on the same bed, which moves
the same delta to **+1.17 ms**. What this view costs is being switched on, not
what it remembers.

**Known limitations.** The harvest map is per tile, so it ranks regions and
does not measure them — `examples/foodroad` prints the raw values beside the
picture. Both channels decay every tick, so they defeat the dirty-rect render
skip by construction and the settled bed is the real price. A binary that only
draws gets nothing: `FoodRoad::observe` is the tick loop's, and
`observed_frames` against the frames run is what says whether a road is a road
or a smear. The outdoor sandbox has no key for either channel.

## Field wake status — the field solves for the ants, and only one half of the fix is free

**What this is for.** An ant's step is a cell write, a cell write marks its chunk
dirty, and `field::step` seeds its solve set from exactly that — so every field
tile under a walking colony is re-solved on all six channels and re-scanned for
occupancy every tick, for as long as the ants keep walking. The owner put it
first: *"I think the field is still a huge part of our cost. Creatures and the
field are all interlinked."* He is right about the mechanism. The two switches
here are the attempt to stop it, and the useful part of what shipped is the
measurement of how little that buys.

**Two settings on `FIELD_CREATURE_WAKE`, both off by default.**

- **`blocked`** — a write between two cells that derive nothing stops marking
  `Chunk::stale_blocks`, so `rebuild_blocked` inherits that block instead of
  re-reading its 256 CA cells. **Bit-identical**, world hash and field hash, and
  free: against `origin/main` over six paired alternating reps the whole tick is
  −1.4%, which is noise in a ±2% spread and is reported as noise rather than as
  a win.
- **`0`** — additionally, a chunk with no stale block does not wake its field
  tile at all. **Not bit-identical**, and the reason is not a channel reading a
  creature cell: it is the three momentum passes, which run over the solve set.
  `FIELD_MOMENTUM=0` in both arms makes the two hashes agree exactly.

**The predicate is keyed on emission, not on `MaterialKind::Creature`, and that
is the load-bearing detail.** `rebuild_blocked` derives five per-block arrays,
and `Creature` is excluded by name from the two that decide `blocked` and
`transmission`, and by content from `moisture_source`. But **`glow` and `beam`
are `max`ed over every cell whatever its kind** — those two *are* read from an
ant today, and answer zero only because `assets/materials/ant*.ron` sets
neither. A glowing creature would make the skip silently wrong. Keyed on the
emission, such a material drops out of the skip instead, and
`Material::field_inert` is computed rather than cached because `lab::params::write`
and the tunables panel both assign `glow` on a live world.

**What it buys, measured: almost nothing, and the reason generalises.** The
brief's falsifier asked whether the field's awake set falls back toward the
no-ant figure at two hundred ants. **It does not move by a single tile** — 65.4
solved a tick with the skip on, 65.4 with it off. The lab bed is 128 chunks and
the field already solves ~65 of them for the plants and the sky, so every chunk
an ant occupies is inside the one-tile halo of a tile that was solving anyway.
**The halo defeats the solve-set gate, and will on any bed dense enough to
play.** What the ants do add is block rescans, 45.9 → 63.3 a tick, of which the
write gate recovers 2.6 — the other 14.8 are marks they earn by digging,
dropping spoil and disturbing soil water. On the clock 52 ants cost the field
0.081 → 0.117 ms and the two switches give back 0.002 and 0.004, both inside
their own arm's spread. The counter says the mechanism fired; the clock says it
bought nothing.
...
in the addendum under it. `#374` declined stopwatches in the live loop; gated,
the cost when off is one `OnceLock` read and eight `Option` tests **per tick**,
and `Instant::now` is never called. On the shipped lab bed at 52 ants it already
says the field is **20–21% of the tick** and `ca_sweep` is **56–60%**.

**Known limitations.** Neither switch is on by default and neither should be
flipped on this evidence: the number that would justify it is the stopwatch run
on the owner's own colony of thousands, which no harness here can reach
(`antcost`'s stocking loop tops out near two hundred). The 14.8 residual block
rescans are unattributed — an arm with the digging verb off would separate the
ants' own `Solid` writes from the soil-water pass re-marking, and that was out of
the lane's scope. The solve-set gate's momentum divergence is measured per
channel but not judged: the velocity channels move by 70% of their own largest
value, because the air in a sealed box was substantially the ants stirring it by
walking, and whether that is a bug or the wind is the owner's call — the same
ruling `field.rs`'s reverted per-tile momentum subset already carries. And the
research review's own field row does not reproduce at its stated size on this
box: 1.44x for 52 ants, not 2.09x. Full account, with every table and command:
`Reports/ant-field-wake-2026-09-19.md`; the GPU field priced and declined in
`Reports/gpu-field-design-2026-09-19.md`; one bug filed on the way,
`Reports/open-bugs-handoff.md` §Z31.
