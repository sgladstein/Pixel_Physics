...
| `F10` / `;` | Cycle how a **solid cell's** light reads: `Depth` (dims from the skyline down to `DEPTH_LIGHT_FLOOR`) vs the flat default that depth does not change — the default was reversed here by a playtest, against the 2026-08 world review's own finding; see `render::TerrainLight`. The `;` alias exists for the same reason `Tab` joined `PageUp`/`PageDown` on the tunables menu: 60% and many laptop keyboards have no function row, and a key only reachable on an F-row is unreachable on them |
...
| `F12` | Cycle where the dark **behind a void** comes from: `Depth` (a pure function of depth below the frozen skyline, so a dug pit is uniformly black however wide) vs sky light propagated on a 4-cell block grid, the default. Named on screen while it is not the default, so a screenshot can say which it is; see `render::SkyLight` |
