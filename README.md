...
Generated by `scripts/readmetoc.py` from the `##` headings — 75 of
...
|---|---|
| [Running](#running) | 175 |
| [Finding things](#finding-things) | 191 |
| [Controls](#controls) | 227 |
| [Materials](#materials) | 298 |
| [Architecture](#architecture) | 372 |
| [The coarse field grid](#the-coarse-field-grid) | 574 |
| [M12/M13 status](#m12m13-status) | 836 |
| [Canopy throughfall status — rain drips through a wood](#canopy-throughfall-status--rain-drips-through-a-wood) | 863 |
| [Liquid physics: compressible volume, not discrete occupied cells](#liquid-physics-compressible-volume-not-discrete-occupied-cells) | 992 |
| [M14 status](#m14-status) | 1018 |
| [M7 status](#m7-status) | 1166 |
| [M15 status](#m15-status) | 1195 |
| [M6 deferral](#m6-deferral) | 1326 |
| [M5 status](#m5-status) | 1336 |
| [Plant lines merged: the genome, and the ecology](#plant-lines-merged-the-genome-and-the-ecology) | 1431 |
| [The generation loop: plants die, seeds expire, slots come back](#the-generation-loop-plants-die-seeds-expire-slots-come-back) | 1501 |
| [The economy re-derived: standing tissue costs something](#the-economy-re-derived-standing-tissue-costs-something) | 1582 |
| [Stems draw a line: the growth walk renders its heading](#stems-draw-a-line-the-growth-walk-renders-its-heading) | 1790 |
| [Plants that stop: organs, determinacy, and a price on both](#plants-that-stop-organs-determinacy-and-a-price-on-both) | 1872 |
| [M16 status](#m16-status) | 1940 |
| [M17 status](#m17-status) | 2126 |
| [M18 status](#m18-status) | 2353 |
| [UI improvements — overnight run, section 9](#ui-improvements--overnight-run-section-9) | 2760 |
| [Live tunables panel — overnight run, section 10](#live-tunables-panel--overnight-run-section-10) | 2805 |
| [Rendering performance — overnight run, section 11](#rendering-performance--overnight-run-section-11) | 2873 |
| [Zoom-out resolution status — more pixels instead of fewer cells](#zoom-out-resolution-status--more-pixels-instead-of-fewer-cells) | 2893 |
| [Zoom-in styles status — what a cell's 64 pixels can say](#zoom-in-styles-status--what-a-cells-64-pixels-can-say) | 3062 |
| [M8 status — started, not complete](#m8-status--started-not-complete) | 3121 |
| [M9 status — the gnome](#m9-status--the-gnome) | 3234 |
...
| M | Section | Line |
|---|---|---|
| 5 | [M5 status](#m5-status) | 1336 |
| 6 | [M6 deferral](#m6-deferral) | 1326 |
| 7 | [M7 status](#m7-status) | 1166 |
| 8 | [M8 status — started, not complete](#m8-status--started-not-complete) | 3121 |
| 9 | [M9 status — the gnome](#m9-status--the-gnome) | 3234 |
| 10 | [M10 status — the worldgen half](#m10-status--the-worldgen-half) | 3740 |
| 12 | [M12/M13 status](#m12m13-status) | 836 |
| 14 | [M14 status](#m14-status) | 1018 |
| 15 | [M15 status](#m15-status) | 1195 |
...
**Shipped on 2026-09-24, the owner's ruling.** An ant with a nest walks the
heading chooser (`creature::chooser_step`, `PIXEL_PHYSICS_CHOOSER=trailaway`,
now the default): every decision picks one heading from all the usable ones,
preferring to keep going; while carrying food, home pulls with a patience that
relaxes when it makes no progress; the trail is read where a step would go;
and an empty ant on a trail is pulled away from home along it. The step-roll,
three-cell-cone and tumble walk it replaces is `PIXEL_PHYSICS_CHOOSER=off`,
and it is still what every species without a nest walks (`chooser_for`).
`ant.ron`'s `Drop` also gained `Energy` (Bias −2.0, `Energy` +1.8), so a
hungry ant keeps and eats what it holds and a fed one delivers as before.

- **Colony bed, no trail laid** (founders' round trips a run, 24 seeds): 10.5,
  4.5 and 2 at 90, 140 and 200 cells, where the old walk made 0 at each.
- **Lab box** (12 seeds): 3.7x the food carried home, deepest breeding
  generation 10 → 26, colonies ending twice today's size.
- **Costs:** about 9% of the ant scene's frame (`ascii scene=foraging`, 0.658
  → 0.717 ms mean), and 2 of 12 lab boxes eaten bare and extinct (0 before).
- **Known limitation:** at 400 cells no founder reaches the food (its range on
  a starting tank), and on the painted nest strip the ants on the far side
  from where food arrives still starve.

Full account: `Reports/ant-scenes-2026-09-23.md` §12–§15; the walk itself is
