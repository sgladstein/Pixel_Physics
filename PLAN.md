...
| 2 | `touch_neighbours`'s fast-path guard is unreachable (`MAX_REACH..CHUNK_SIZE-MAX_REACH` is an empty range at today's constants) — every `World::set` pays the full neighbour-wake loop. **Deprioritised 2026-08-24 by measurement, not closed:** it optimises the CA sweep, which `scale_probe phases=1` puts at ~10% of the frame at the shipped 8192x2560 size. The issue's own estimate ("plausibly a large share of the 23 ms serial worst frame") was taken at 512x320 against the *serial* driver, before the parallel sweep and the dirty-rect work. Still real; it cannot repay being done before the field or `step_organisms`. See `Reports/frame-cost-audit-2026-08.md` | perf |  <!-- still open, re-confirmed 2026-08-23: `world.rs`'s guard carries its own comment saying it is a no-op at today's constants and is kept as documentation of that fact. Narrowing it needs `parallel.rs`'s cross-chunk write-safety proof re-derived from an equality to an inequality. -->
