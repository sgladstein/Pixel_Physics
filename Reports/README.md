...
- [measurement-under-contention.md](measurement-under-contention.md) —
  **built and verified.** Why a timing number on this machine needs a
  `TRUSTED` stamp: the box measured 8% quiet over 45 minutes, three
  detectors that were wrong before the fourth worked, and the one
  mechanism (builds taking the timing lock) deliberately left unbuilt.
