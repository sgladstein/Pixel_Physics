...
**Contents** — 10 sections, ~27,719 tokens. Read a section, not the file: the line number is there so you can `sed -n 'START,ENDp'` it, and the token column is there so that is a priced decision rather than a guess. Regenerated by `scripts/reporttoc.py`; a merge conflict inside this block is never hand-merged — take either side whole and re-run.

| Section | Line | ~tokens |
|---|---|---|
| [0. Decision log](#0-decision-log) | 45 | 3,043 |
| [1. What is being built, in one page](#1-what-is-being-built-in-one-page) | 67 | 478 |
| [2. The stages](#2-the-stages) | 102 | 17,171 |
| [3. What each stage needs from the ones before it](#3-what-each-stage-needs-from-the-ones-before-it) | 1311 | 171 |
| [4. Standing guards](#4-standing-guards) | 1329 | 2,785 |
| [5. Deferred, and why](#5-deferred-and-why) | 1512 | 500 |
| [6. The dead-end register](#6-the-dead-end-register) | 1545 | 818 |
| [7. Is open-ended reachable, and what would it additionally require](#7-is-open-ended-reachable-and-what-would-it-additionally-require) | 1569 | 653 |
| [8. Open questions for the owner](#8-open-questions-for-the-owner) | 1613 | 1,278 |
| [9. Provenance](#9-provenance) | 1698 | 196 |
...
| E11 | **Ants should be able to get across a gap, so the impulse verb is authorised.** Asked directly whether crossing is wanted *at all* — given that the current refusal is deliberate and cost two attempts — the owner chose *"Yes, they should cross"* over both *"No, refusing is fine"* and an offer to render both readings first. This closes **call 4** of `creature-motion-design.md`, which the 2026-08-29 handoff named as the gate on all further locomotion work. **The condition travels with the authorisation**: two previous attempts put falls at **59-80% of all moves**, so the verb ships with §7's guards and *especially* falls-per-move, or it does not ship. That baseline is itself stale — §7's 11,031 moves / 1,629 falls = 14.8% predates `d007c156` and `4c95233` — and is being re-taken before anything gates on it. **Sequencing, deliberately**: held until the creature-slowdown diagnosis lands, because the verb edits the same `creature.rs` and its falls would otherwise be inseparable from the bug's. | **Decided by the owner**, 2026-08-29. Answered in session rather than by review card, so there is no card id to cite — unlike E9 and E10 above. |
| E12 | **S6 reproduction is authorised and building.** The owner asked for it directly on 2026-08-29, which moves it off the creature handoff's discussed-but-not-authorised list. Build per §2.6 as specced: budding, `reproduce_threshold > start_energy` so that doing nothing becomes extinction, per-weight mutation width `MUT_ABS_FLOOR + MUT_REL·|w|` clamped ±40, and both pre-flights before any mechanism. **Five constraints from `creature-review-2026-08.md` §T4 travel with it**, each already paid for elsewhere in this codebase: never derive a child's genome or its draws from its slot id (dead-end 670 — generation wrap at 16 hands out bit-identical ids that cluster spatially); a birth scheduled from inside a tick must not take the live heap out from under the scheduler (dead-end 1094); the 4095-slot ceiling needs a **release-mode** guard, because `push_organism`'s range check is a `debug_assert` that the app and every long harness run never compile; close the S3b stamp seam; and tests are populations and paired comparisons only (dead-end 552 — creature outcomes already spread 0.103–0.541 across random genomes, so a single-individual assert is a coin flip wearing an assertion). **Counters first**, `births` and `births_denied_no_space` reading 0, and re-take the frame cost at breeding population: creatures were measured free at 55 ants and a breeding population is not 55. | **Decided by the owner**, 2026-08-29, in session. Needed E5, which was already settled. |
| E13 | **Predation starts — as E7's probe, not as a milestone.** The owner asked for it directly on 2026-08-29. This **executes E7 rather than overturning it**: the one-file probe E7 deferred it to is the work, and the milestone is still not authorised. The blocker is **perception, not hunting** — beetles measured **bit-identical at `beetles=0` and `beetles=9` over 6,000 frames**, so the predator currently moves no counter at all. The pre-flight runs first and is reported on its own: total channel-B mass, and the fraction of prey heads within a sensor offset of a nonzero B cell. **If the trail barely exists where beetles are, the predator is blocked on movement rather than perception** — which redirects the whole line and is worth knowing before any fear-scent channel is designed. The third pheromone plane stays behind its settled-world cost gate: **0.5 ms**, against the two existing planes at **0.0014 ms** settled. Slot 12 stays unnamed and `Strike` is **not** authorised by this. **The standing trap**: a beetle following a gradient is the fifth instance of CLAUDE.md's coarse-field degeneracy, which has been hit four times on three lines and never once caught by a test. | **Decided by the owner**, 2026-08-29, in session. Amends the sequencing of E7; does not replace it. |
| E14 | **The horizon change is authorised: cut the birth grant so ants can starve.** The owner's words: *"Yes — let them starve"*, chosen over both leaving it alone and shipping it as a runtime knob. **An earlier session offered this twice and never got an answer**, so it stood as unauthorised and the creature handoff records it that way; put with its arithmetic, it is now decided. **The arithmetic**: `ant.ron` is `start_energy: 900`, `idle_cost: 0.10`, `tick_interval: 6` — an idle life of ~9,000 ticks = **54,000 frames** — against scenes that run **12,000**, or **22% of an idle lifetime**. Food supplies **2.9%** of the colony's energy. So foraging perfectly and never eating at all come out the same inside any horizon a harness can afford, and `deaths` reads **0** everywhere. `creature_space` already overrides `START_ENERGY` to **90.0**, one tenth of shipped, precisely so its 18,000-frame runs reach the idle death its own output line advertises — a working precedent rather than a guess. **This is a shared-budget reallocation and must be scoped as one**: it changes what `idle_cost`, `move_cost`, `eat_energy`, the synapse tax and `reproduce_threshold` each mean, and CLAUDE.md requires those constants be named and re-derived as part of the work, never inherited. **Two consequences past selection**: `deaths` becomes a live counter and its own positive control (non-zero with the cut, back to zero with the grant restored), and **corpses start appearing** — the first real carrion in the world, which §2.5 measured as the reason the diet curve has one hump. | **Decided by the owner**, 2026-08-29, in session. Reverses the standing not-authorised state. |
| E15 | **Vision comes before predators, and predation waits for it.** The owner's words, 2026-08-30: *"We want vision and then predators."* **E13's probe executed and answered, and this is the answer's consequence.** The pre-flight found the beetle blocked on neither hunting nor the trail's existence but on **sampling** it: channel B carries mass 294 over **33 nonzero cells** in an 81,920-cell world, **77%** of *ants* sit within a sensor offset of a nonzero cell (they make it) against **32%** of beetles, the mean beetle-to-nearest-cell distance is **46 cells** against a **6-cell** sensor span, and the two sensor reads therefore hold **different values 1.3% of the time** (`\|along\|` 0.0067). There is no gradient to climb anywhere a beetle stands. **The kill is not what is broken**: the pre-flight's own positive control paints a saturated trail, prey-within-sensor goes 0.467 -> 1.000, and a hungry beetle beside an ant feeds -- ant cells 24 -> 22 -> 21. The search is what fails. **The structural statement is that no sensor reports another organism at a distance at all**: `FoodAdjacent` and `AtNest` are contact-range, and the two pheromone planes are the only distal sense the suite has -- the ones measured failing here. So predation is not blocked on a fear-scent channel, a `Strike` verb or slot 12; it is blocked on a sense that reaches. **The standing trap travels with this**: a two-point difference on a coarse field is CLAUDE.md's degeneracy, hit four times on three lines and never once caught by a test, so "the same smell, but stronger" walks straight back into it while a line-of-sight read at CA resolution does not. **Predation stays unauthorised as a milestone** (E7, E13) until vision exists and a beetle can be shown finding prey it did not bump into. | **Decided by the owner**, 2026-08-30, in session -- like E11-E14 there is no card id to cite. Sequences E13's answered probe; does not overturn E7. |
...
> **Superseded 2026-08-29 — every figure in this section was measured
> before this week's worldgen work** (`39e6f36`, *"double the sky, deepen the
> soil blanket ~4x"*), which reshaped the terrain the whole creature line
> stands on. Re-measured on `main` at `ba6fc98`, same-session and paired:
> **frame cost 2.906 / 2.943 ms** (was 3.488 / 3.491, a different container —
> do not subtract them) and **determinism still identical**. The
> `ascii` foraging scene's own figures moved much further: **98 round trips
> → 23, deepest 18 → 28**. Full account, including why `eats 6 / deaths 0`
> does not mean the world feeds the colony for free, in
> `Reports/creature-rebaseline-2026-08-29.md`.
>
> **The other three rows were re-measured too, and they held:** foraging pays
> **+0.427 / +0.459** (was +0.474 / +0.466), ants fed **0.68 / 0.75** (was
> 0.75 / 0.75), reference genomes **0.696 / 0.299** (was 0.690 / 0.297). The
> `zero` control reading 0.299 across *both* this week's merges — and the
> economy sweep's `immobile` column reading 0.298 at all four settings —
> says the wetland economy scene these guards run on was barely touched,
> while the `ascii` foraging scene over the same period lost three quarters
> of its trips. **Two creature scenes, opposite answers**, which is why this
> had to be measured rather than reasoned about. Caveat kept visible: the
> economy sweep was taken on `ba6fc98` and the reference pair after the
> plant-organs merge (`f96c08d`); they are not on the same tree.
...
* **Predation.** §13o measured beetles=0 and beetles=9 **bit-identical** over
  ...
  **Run 2026-08-29 (PR #122), and the answer was a third thing: neither.**
  The trail exists (mass 294 over 33 nonzero cells) and the kill works -- a
  hungry beetle beside an ant feeds, ant cells 24 -> 22 -> 21, under the
  saturated-trail control. What fails is *sampling*: a beetle sits a mean
  **46 cells** from the nearest nonzero cell with a **6-cell** sensor span,
  so its two reads differ **1.3%** of the time and there is no gradient
  anywhere it stands. **E15 is the consequence** -- vision first, because no
  sensor in the suite reports another organism at a distance.
