...
**Contents** — 10 sections, ~27,333 tokens. Read a section, not the file: the line number is there so you can `sed -n 'START,ENDp'` it, and the token column is there so that is a priced decision rather than a guess. Regenerated by `scripts/reporttoc.py`; a merge conflict inside this block is never hand-merged — take either side whole and re-run.

| Section | Line | ~tokens |
|---|---|---|
| [0. Decision log](#0-decision-log) | 45 | 2,803 |
| [1. What is being built, in one page](#1-what-is-being-built-in-one-page) | 66 | 478 |
| [2. The stages](#2-the-stages) | 101 | 17,171 |
| [3. What each stage needs from the ones before it](#3-what-each-stage-needs-from-the-ones-before-it) | 1310 | 171 |
| [4. Standing guards](#4-standing-guards) | 1328 | 2,785 |
| [5. Deferred, and why](#5-deferred-and-why) | 1511 | 354 |
| [6. The dead-end register](#6-the-dead-end-register) | 1536 | 818 |
| [7. Is open-ended reachable, and what would it additionally require](#7-is-open-ended-reachable-and-what-would-it-additionally-require) | 1560 | 653 |
| [8. Open questions for the owner](#8-open-questions-for-the-owner) | 1604 | 1,278 |
| [9. Provenance](#9-provenance) | 1689 | 196 |
...
| E10 | **The shipped ant keeps `Chain(2)` for now; heritable body is wanted later, and the cheap route is chain *length*.** The owner's words: *"I am fine with the shipped ant for now. Eventually I would like creature body to be an evolve feature but this would be complex. Is there any cheaper way to do that for larger creatures without having to develop complicated body mechanics and cost system."* **There is, and it is measured** (`creature-appearance-design.md`): a chain follows its head, so it needs no rigid-body fitting or rotation — chain bodies block **2-6%** of their moves against **25-43%** for rigid bodies of the same size — and ~~per-cell metabolic cost already prices a longer body, so no cost system is owed~~ — **THIS HALF OF THE PREMISE IS FALSE, corrected 2026-08-29.** `creature.rs` charges `spent = def.idle_cost + synapse_tax` and `spent += def.move_cost` on a move: both **flat per organism**, and nothing anywhere reads `chain.len()`. Found by `session_019RCxVVRPrYbeRtqCmofeUK` and verified independently at the two call sites. **A longer chain is currently free**, which is exactly the strictly-better trait the owner's own cost-and-benefit principle exists to prevent, and the same shape as S5's single-humped curve — a gene with no compensating disadvantage converges. So heritable chain length **does** owe a cost, and owes it before it is heritable rather than after. The cheap form is to scale `idle_cost` by body cells at the one site above; that makes the claim true rather than abandoning the route. **The other half of E10 stands and is measured**: a chain needs no rigid-body mechanics (2-6% blocked against 25-43%), which was always the larger part of the answer. `Chain(6)` puts **2.9x** the shipped ant's ink on screen for *less* blocked movement. So heritable body = one integer, zero new mechanics. **The catch and its one-line fix**: a long chain reads as a worm, because every cell of a creature is one material resolved by species name (§7 of that report). `shade-by-cell-type` at the `plant_creature_seed` seam gives a chain a visible head — and is the only route by which any part of appearance becomes per-individual. | **Decided by the owner**, card `20260829T045336581Z-34c3d3`. |
