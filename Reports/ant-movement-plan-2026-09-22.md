...
existed. **It ships on**, on the owner's ruling, because the loop improves;
`PIXEL_PHYSICS_DROP_REACH=adjacent` turns it off.

**§Z33's fix must keep every drop possible.** The owner rejected making a
part-eaten cell undroppable: an ant that nibbled its only cell on the way
home could never unload, and a colony on single-cell trips would walk in
circles. The fix proposed instead is that the ground keeps the remaining
worth, so a part-eaten fruit goes down, and is picked up, at what is left.
