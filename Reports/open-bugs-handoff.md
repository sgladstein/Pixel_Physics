...
what is left, `unit - digesting`. **`Carried::into_cell` writes that worth into
the cell only for a `worth_in_aux` material, and `corpse` is the only one.**
Every plant food goes down as a plain cell of its material, so the next animal
to pick it up is paid its material's full worth. A fruit half-chewed (480 of
960) and put down comes back at 960.

**Reproduction:** `a_part_eaten_fruit_put_down_and_picked_up_holds_only_what_was_left`
(`creature.rs`, `#[ignore]`d as the reproduction). It reads **480 when put down
and 960 when picked up**.

**How big it is.** `trailfollow`'s `FOOD BUDGET` line balances every larder cell
taken from the pile against what was chewed and what still stands. On the gap-90
forage bed (arm A of `ant-decision-census-2026-09-22.md` §2, 24 seeds):

| | Taken from the pile | Chewed | Created (chewed beyond what was taken) |
|---|---|---|---|
| shipped | median 31 cells | median 57 | median 31, **53% of everything eaten** (15–69%), in 24 of 24 runs |
| `PIXEL_PHYSICS_DROP_REACH=bodies` | median 30 | median 44 | median 15, 40% |

The creation runs through the nest's put-down-and-pick-up churn: `deliveries`
counts about 1,400 cells put down at the nest in a run whose colony took about 31
from the pile. **The budget closes exactly** once the created part is counted
(`CreatureStats::drop_worth_restored`, 15–37 cells' worth per run) and food paid
into births is named. Nothing decays; census report §12.

**What it has already cost.** The drop stopgap (census report §11) was read as
"starves the colony" because starvation rose with it on. The budget says the
stopgap does not reduce the food brought home (taken: −1 cell, median); it cuts
the churn that was creating food (created: fewer in 21 of 24 seeds). So §11's
starvation is this bug, and every survival or loop figure on this bed, before
and after, is measured on a colony living half on food that does not exist.

**Why nothing caught it.** The energy ledger books eating plant matter as a
source (`HarvestedPlant`) and does not track worth on the ground, so its
identity holds. `a_sealed_colony_never_grows_its_own_biomass`, the guard that
watches for creation, seals ants in a box with their own corpses, and a corpse
carries its worth in the cell, so it cannot see plant food at all.

**The fix is the owner's to choose** (it changes every colony's food supply):
