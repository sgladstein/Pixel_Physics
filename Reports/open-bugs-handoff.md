...
**65 open, 126 bugs** (plus 20 landing-note items,
...
|---|---|---|---|
| 0-z | **OPEN** | 180 | Leaves are the only channel a plant has, and four separate "bugs" are all that one fact |
| 0-a | closed | 237 | Dark bands under overhangs, objects and open-cast digs (render) |
| C1 | **OPEN** | 315 | A forest-floor bank is a wall the gnome has no way over |
| D1 | closed | 352 | The brush and fire license nothing, so a burnt trunk leaves its crown in the air |
| D2 | **OPEN** | 509 | A room's collapse arrives at frame ~350 where it used to arrive at ~150 |
| D3 | **OPEN** | 532 | Near-surface blasts do not throw chunks into the air |
| D4 | **OPEN** | 565 | At a bounded reach a collapse can stop part way and leave a slab in open air |
| 0 | **OPEN** | 602 | Roofed water: ponds fills both sides of an overhang (worldgen) |
| 0b | closed | 620 | The deep massif reads as television static, and it is a per-cell palette dither (worldgen) |
| 0c | closed | 742 | Cave light is quantised to 8-cell squares (render) |
| 0d | **OPEN** | 844 | The organism support search asks the wrong question |
| 0e | closed | 886 | A decay site does not follow its cell |
| NEW | closed | 949 | Plants grow nothing on generated terrain |
| U | closed | 1109 | Water stress makes a tree BIGGER |
| V | closed | 1154 | A tree with no seedlings under it never stops growing |
| Z | decided | 1207 | The stand still reads as one mass |
| Z3 | **OPEN** | 1341 | A settled piece is re-promoted for ever, at about one body every five frames |
| Z2 | closed | 1400 | A free particle drops Cell::aux, so a blast under-prices a corpse |
| Y | closed | 1518 | The gnome cannot get through the wood |
| X | decided | 1669 | A desert with no desert plants |
| -- | historic | 1718 | X (original). A desert with no desert plants |
| W | decided | 1795 | The water-cycle branch and this one are two halves of one mechanic |
| A | **OPEN** | 1869 | The slot-1 root spread has collapsed |
| B | closed | 2169 | anchor_support runs over creature organisms, unguarded |
| C | closed | 2256 | grass and creeper root branching is running a retired model |
| D | closed | 2305 | Two smaller things the merge exposed |
| E | closed | 2337 | A test scene can outlive the economy it was written for |
| F | **OPEN** | 2355 | Cross-line seams neither branch's tests exercise |
| P1 | **OPEN** | 2426 | The water book, the root-tip counter, and what they said about §A and §U |
| P3 | **OPEN** | 2713 | The generation loop |
| V3 | **OPEN** | 2909 | Die-back's shed tissue feeds a pile that grows up through the canopy |
| V2 | closed | 3026 | A tree cannot die of drought |
| P2 | **OPEN** | 3167 | The economy re-derivation |
| G | **OPEN** | 3382 | Grassfire arrives with a standing negative verdict |
| -- | historic | 3445 | G (original). Grassfire arrives with a standing negative verdict |
| 0f | closed | 3467 | A melting Powder manufactures water |
| 0g | closed | 3522 | scene=lavapour's pond simmers forever |
| 0h | **OPEN** | 3584 | Lens-stress at 2048x640 puts gravel and water in motion, with no cave anywhere (worldgen) |
| 0i | **OPEN** | 3628 | Terrace risers are inert: erosion deletes them at any nonzero world_age (worldgen) |
| 1 | closed | 3668 | Whiskers on a spreading front |
| 1l | **OPEN** | 3785 | Boiling never puts a bubble *in* the water |
| 1m | **OPEN** | 3849 | Damp-soil evaporation barely runs, and the humidity shadow that would switch it off is al... |
| 1b | **OPEN** | 3920 | diffuse_heat does not conserve heat, and a hot cell is an amplifier |
| 1c | **OPEN** | 3962 | A rigid body loses about a tenth of its cells when it lands |
| 1d | **OPEN** | 3991 | A large lava lake never finishes solidifying |
| 1h | closed | 4007 | Falling rock grinds itself to powder in deep water |
| 1k | **OPEN** | 4113 | A splash droplet loses about 1% of a cell somewhere |
| 1j | **OPEN** | 4145 | MAX_LOAD_CELLS_PER_FRAME does not bound the load model's frame cost |
| 1i | closed | 4177 | The rigid-body rotation probe is vacuous, and a body can turn through a wall |
| -- | historic | 4189 | (was) 1h. Falling rock grinds itself to powder in deep water |
| 1e-ter | closed | 4256 | A boulder that never leaves the sky |
| 1e-bis | closed | 4296 | Slabs of rock hanging over a solidifying lava lake |
| 1e | **OPEN** | 4329 | One cell in a lava pour is still left hanging, and the route is unknown |
| 1f | **OPEN** | 4356 | A pond with rock in it never stops shuffling fill |
| 1g | **OPEN** | 4383 | scene=lavapour leaves one 3-cell raft that a poke does not drop |
| 2 | **OPEN** | 4399 | Sand-into-water displacement |
| 3 | closed | 4426 | Scheduler under-enforces max_active_tips |
| -- | historic | 4444 | (was) Scheduler under-enforces max_active_tips (a tree bug) |
| 4 | **OPEN** | 4484 | Levelling is O(width²) |
| 4b | closed | 4504 | A cell alone in the air drops its column's skyline |
| 5 | **OPEN** | 4537 | Automatic promotion |
| 6 | **OPEN** | 4566 | The heightfield does not deliver the speed it was built for |
| H | closed | 4617 | ascii's ants moisture-gradient scene asserts a gradient the scene no longer has |
| H2 | closed | 4735 | The ascii colony has gone sessile |
| H3 | closed | 4797 | Both worldgen at-rest tests are red on main, and both are water |
| I | closed | 4920 | The disturbance-extent guard inverts once rubble stops anchoring |
| J | **OPEN** | 4985 | A blocked substep still vents the smoke it was only *probing* |
| Q | **OPEN** | 5015 | Settled debris stands in one-cell vertical needles that never topple |
| P | **OPEN** | 5200 | scene=worldcrack is not deterministic, so seedsweep.sh cannot compare two models on a cha... |
| K | closed | 5347 | try_step's rotation-fit probe compares every cell against itself |
| N | **OPEN** | 5429 | Decayed litter makes soil that does not match the soil around it, and roots will not ente... |
| O | **OPEN** | 5497 | Litter rots into soil that never leaves, so the floor rises all run |
| M | closed | 5554 | Two gating worldgen tests are red, and both are the same thing: generated water never com... |
| R | closed | 5764 | filmstrip scene=colony panics at its own default seed, and degrades badly at others |
| -- | historic | 5824 | R (original). filmstrip scene=colony panics at its own default seed, and degrades badly a... |
| L | closed | 5903 | The colony has gone sessile: 98 round trips became 2 |
| R2 | **OPEN** | 6035 | An ant put down on open water stands on the surface for ever, and found_colony puts them ... |
| -- | historic | 6091 | R2 (original). An ant put down on open water stands on the surface for ever, and found_co... |
| S | closed | 6153 | Every destructive verb but the brush leaves the structural scheduler pinned at its cap fo... |
| S2 | **OPEN** | 7150 | The brush's anchor rule destroys structures the other two rules leave standing |
| R3 | **OPEN** | 7314 | A creature chain above two cells overwrites its own head |
| R4 | **OPEN** | 7427 | BrainOutput::Turn is nearly inert for a surface walker on level ground |
| V4 | **OPEN** | 7496 | breaking_free_writes_a_pressure_impulse never reaches break_free |
| T2 | **OPEN** | 7554 | The colony forages and never brings anything home: 1,651 pickups, 4 deliveries |
| S4 | **OPEN** | 7768 | Rock still crushes itself on an idle world |
| S5 | closed | 7840 | A fully-cracked chunk stays welded because the load model never finishes asking |
| T | **OPEN** | 7961 | A starving plant strands a cell: growth races dieback |
| S3 | closed | 8005 | A world nobody has touched pulls its own ground apart |
| -- | closed | 8054 | The plant model bounds height and does not bound width FIXED |
| 1 | note | 8145 | MAX_ROOT_FRACTION feeds the staleness counter, permanently retiring roots |
| 2 | note | 8159 | Grow into soil destroys the soil's stored water |
| 3 | note | 8171 | Capillary exchange can push a neighbour above its own capacity |
| U | note | 8184 | A crown hangs on by its leaves, so a snapped limb never falls |
| W1a | note | 8303 | creeper.ron's root tips still run the superseded in-tick branch path |
| W1b | note | 8324 | A material-counting guard cannot see a species |
| W1c | note | 8337 | generated_terrain_is_already_at_rest went red on main |
| T1a | note | 8471 | load::grain_is_footing reads *attachment* where it means *supported* |
| T1b | note | 8549 | The structural opt-out did not hold against bearing |
| T1d | note | 8560 | acceptance.sh's lavadrop sits close enough to its frame budget to flake, and is over it o... |
| T1e | note | 8594 | "The pieces hit the ground and turn to dust" was not settle, and the measurement says so |
| T1f | note | 8648 | The felled pile is 74% powder because the tree is 56% leaves. The piece ladder cannot fix... |
| T1g | note | 8702 | A "refixed" claim went out over a settled state that had barely moved |
| T1c | note | 8731 | §1c's settle loss is now a counter |
| -- | note | 8748 | What landed |
| -- | note | 8771 | Do not re-derive these |
| -- | note | 8799 | Measurements that contradict something written |
| -- | note | 8819 | Open |
| -- | note | 8854 | Unmerged at close, and one of it is a fix main needs anyway |
| 1n | note | 8872 | grass sets zero seeds on main |
| B2 | **OPEN** | 9065 | A living plant in the lab pulls its own anchorage out from under itself and is felled whole |
| Z4 | closed | 9133 | World::germinations can exceed the number of seeds that ever existed |
| Z5 | closed | 9218 | Every homing odometer in the tree is dead: its charge wire is below W_EPS, so eval_brain ... |
| W2 | **OPEN** | 9423 | Two soil constants annul each other, and the root depletion zone lost 6.3x of its depth |
| W2a | **OPEN** | 9499 | The refutation, measured |
| E2 | **OPEN** | 9580 | Narrowing the CA sweep's region changes the world, and the RNG is not the reason |
| W3 | closed | 9652 | PlantScene's bed stands on a 512-span slab anchored only at the world edges, and it survi... |
| W4 | **OPEN** | 9772 | A rooted bank now sheds *more* of its own soil than a bare one, because it still has the ... |
| W5 | **OPEN** | 9832 | The lab's bed grows a water table on its stone floor, and it does not stop |
| W6 | closed | 9883 | A plant EVOLVES root tips into shoot tips, and the shoot it then grows is made of root wood |
| W7 | closed | 10275 | A severed plant is still one economy: the roots' water feeds a crown they have no path to |
| Z6 | **OPEN** | 10381 | Every shipped bed starves its ant colony inside one play session |
| Z7 | **OPEN** | 10559 | The trail-following gate saturates the signal it gates: the ant reads its own trail at ±0... |
| Z8 | closed | 10711 | A fruit severed by ordinary structural failure lands as an ownerless windfall, and it can... |
| Z9 | closed | 10832 | A hopping animal that comes down on water never lands: it hangs there, is charged the air... |
| Z10 | closed | 10945 | The flitter's float never switches off on a bed that has flowers in it |
| Z11 | closed | 11042 | At the widest zoom-out the view drew one cell in sixteen and dropped the rest, so thin th... |
| Z12 | **OPEN** | 11137 | Most of what piles up in a long-run long-ant colony is one-cell ants, and they are bred t... |
| Z13 | closed | 11217 | Every animal in both games can reach a rest it cannot leave (engine) |
| Z14 | **OPEN** | 11612 | The played bed's 500,000-frame trajectory is chaotic, and scent_drift: 0.15 re-rolled it |
| Z15 | **OPEN** | 11704 | A plant holds a creature up and also blocks it, so a bed of foliage is a cage: between a ... |
| Z16 | closed | 11782 | DeathCause::Killed is not a killing counter, and the played bed's colony is being overgro... |
| Z18 | **OPEN** | 11923 | Dug spoil stands in open sky, and the owner sees it before he sees anything else |
| Z19 | closed | 12133 | A pellet of spoil crosses up to 116 rows with nothing carrying it |
| Z17 | **OPEN** | 12250 | World::ground_datum is built and wrong inside a sealed lab box, and it reads as "the whol... |
| Z20 | closed | 12302 | labgif wire= was a silent no-op for every card it has ever produced (lab) |
| Z21 | closed | 12343 | The held world's grown and dead starts fill every organism slot, so C founds nothing |
| Z22 | **OPEN** | 12530 | A colony inside a quickening eats about a sixth of the garden, and nothing on screen says so |
| Z23 | closed | 12625 | nearest_foe counts a plant as a foe, so a fed colony quietly vandalises its own larder |
| Z24 | **OPEN** | 12835 | A loop of plant_ant is a crowd of strangers, and nine harnesses still do it |
| Z25 | closed | 12916 | Nothing can hear an alarm: the plane's audible radius is about two cells |
| Z26 | **OPEN** | 13081 | review.py inbox shows every lane's verdicts as yours in a cloud session, and --mark-seen ... |
...
| Z31 | **OPEN** | 13394 | field::step carries derived arrays forward over a settled chunk that still holds an un-ta... |
...
**The open-cast half of that came back on a real playtest, 2026-08-29, and
is fixed again** — *"the background remains black ... looks bad when I break
up the ground near the surface"*, against a broken-open hilltop drawing as a
black slab beside the sky it was level with. Propagation was the right
mechanism and only half the question: it measures **distance from a lit
cell**, and distance cannot tell a hole from a hollow, which the design
round's own table says outright — a 64-wide pit's floor at 0.023 against a
1-wide shaft at 0.010. A second term beside it traces each block back toward
the sky along eight equal-share directions, attenuating through *material
only*, and the two are combined with `max`. A 40-wide pit's floor goes
**0.045 → 0.504** with a shaft at the same depth unmoved at 0.
`Reports/sky-light-design.md`'s *What play found wrong with it* carries the
...
>
> **Two more corrections landed together, 2026-08-29, and they were the
> largest cost in the frame.** The splat itself hashed a `ChunkCoord` twice
> for every cell of every disc -- ~615 of them per glowing cell -- which the
> perf lane found by bisect and callgrind and fixed by walking the disc
> chunk-major; `Reports/frame-cost-the-render-half-2026-08-29.md` is the
> measurement record, and its *"every image-level check of this is blind"*
> section is required reading before verifying any change here by rendering.
>
> **And the trigger took a second correction: `force_full` was in it too.** That flag means *"an overlay is on
> screen with no tracked footprint, so repaint every pixel"* -- and in the app
> it is true whenever the cursor is over the window, because the brush outline
> follows it. So the splat was rebuilt on essentially every frame of play,
> exactly the failure the paragraph above records being fixed once already,
> arriving through a different term of the same condition. Measured on the
> shipped 8192x2560 world: a full redraw was **39.6 ms and 37.2 ms of it was
> this rebuild**, against 1.9 ms for the pixel loop. Removing `force_full`
> and making the rebuild itself 7x cheaper takes the redraw to **2.4 ms**. The one thing `force_full` covered by accident -- a new
> world from `App::reset` with the same emitter tile coordinates -- is now
> `Renderer::forget_world`, called from `reset`, which is a correctness gain:
> `force_full` only covered it when the cursor happened to be over the window.
>
> **The guard above did not catch it, and could not have**: it drew its eight
> frames at `force_full: false`, a state the game is almost never in. It now
> runs the same loop at `true`, and
> `a_glow_halo_is_symmetric_across_a_chunk_seam` (the perf lane's) covers the
> clip that no render reliably does. The sky was a correlate of the cost, not
> the cause -- the report has the control that separates them.
>
...
### Z3. A settled piece is re-promoted for ever, at about one body every five frames — **OPEN, found 2026-08-29**

**In both arms of a control, so it is not the fall's doing — but the fall
makes it start sooner.** Found while running the negative control the fall's
own handoff asked for (*"a piece that is well seated must not be re-promoted,
or a settled pile will never stop moving"*), which is the only reason anyone
looked: nothing in the existing readout says it is happening.

**Reproduction.** `filmstrip scene=fell fell=7150 species=tree frame0=10800
start=9000 every=3000 count=4`, and read `actually moved: N bodies` across the
four tiles. It should stop growing once the fall has settled. It does not:

| frame | `FALL=off` bodies promoted | `FALL=on` |
|---|---|---|
| 7,700 | 45 | 157 |
| 9,000 | 46 | 417 |
| 12,000 | 46 | 1,017 |
...
Both arms end up promoting a body roughly **every five frames, indefinitely**.
The control arm is the pre-2026-08-29 engine exactly (`FALL=off`, verified
byte-identical to the previous binary on this scene), so the loop predates the
fall; what the fall changes is *when* it starts — before frame 7,700 rather
than somewhere past 12,000.

**It is one small piece, not the pile unravelling.** The body is **8 cells**,
which is `rigid::MIN_BODY_CELLS` exactly; `log` and `deadwood` totals stay
flat across the whole run while the promotion count climbs; and the census
reports `bodies 1 (8 cells)` at every sampled frame. So the same fragment is
settling, being failed by the structural model, being promoted, going nowhere,
and settling again.

**Nothing anyone looks at shows it.** `awake` stays at 4-10 of 40 chunks and
`sites` *falls* (1,452 → 710) over the window, so the scheduler reads as
converging; the worst frame does not move; and `filmstrip`'s "everything came
to rest between frames X and Y" line reports `- and 0`, which reads as a
missing measurement rather than as a world that never rests. The only counter
that moves is the cumulative promotion count, and nothing was watching it
because promotion is supposed to be an event.

**Why it matters beyond tidiness.** It corrupts any cumulative per-event
census on this scene. The fall's own per-piece orientation census
(`FailureCounts::settled_lying`) counts a settle each time round the loop, so
a contaminated arm reports hundreds of settles for one fragment and its
orientation share is that fragment's pose, not the fall's. That is
`CLAUDE.md`'s "ask what your number counts when nothing is wrong" arriving
through a loop rather than through a definition — see
`Reports/tree-fall-2026-08-29.md` §4 for the measurement it nearly ruined.

**Not yet diagnosed**, and the first question is the usual one: *which* piece,
and what does the load model say about it? The loop must be `rigid::settle`
writing cells that `structural`/`load` then fail and `fracture_failing_region`
re-promotes, with the body landing back where it started — but nothing above
establishes that, and `settle`'s landing `aux` (`SETTLE_AUX`, §S) is the
obvious first suspect to rule in or out. `scene=fell species=tree frame0=0` is
clean in both arms at every frame sampled, so there is a working negative case
...
> **2026-09-05: that counter now exists, the firing rate has been moved
> about as far as it can go, and the amplifier theory does not survive
> it.** `note_root_tip_exit` counts every exit from `break_root_tips` by
> reason, and it says the gate was refusing **95–99.6%** of initiations
> across four beds and two genotype draws while `ROOT_TIP_POOR`, the
> carbon exit, was **zero in every run**. `root_gate_is_local` (shipped on
> the same day) opens that gate: root cells **4.74x** over 12 paired
> seeds, 12/12, and the root zone recovers 0.016 → 0.839. If the firing
> rate were what switches slot 1 on and off, this is the change that
> should have switched it on.
>
> It did not. `root_and_shoot_branching_read_different_slots` re-run at
> the new defaults: **mean ratio 0.957 over 8 seeds, 0/8 clearing the 1.10
> bar**, against 0.994 and 2/8 on 2026-08-23 — 0.8 SE apart, so no change
> rather than a reversal. **Whatever holds slot 1 flat, it is not
> `break_root_tips` being gated shut**, and the next hypothesis should
> look elsewhere. The reproduction's own doc comment in `plant.rs` carries
> the four-row arc; this note is here because §A is where the amplifier
...
**Update 2026-08-29: candidate 1 landed on `main`, in a different place from
where this entry proposed it, and the canopy half of this bug is gone. The
entry stays OPEN for the other half.** The owner reported the same defect from
the other end -- *"when leaves fall off trees they often get stuck and built up
in the tree branches"* -- so it was never only die-back's.

**What landed.** `Material::falls_through_organisms` (litter only) makes
`update::update_powder` carry a falling litter cell down through
organism-owned cells to the first air below, after the ordinary straight-down
and diagonal moves have failed. That is the "fall through what is already
lying there" this entry ranked first, moved from `shed_to_litter` to the CA
sweep -- which sidesteps the care this entry asked for, since the shared
early-return that stops a low leaf being deleted is not touched at all, and it
catches litter that reached mid-canopy by *any* route rather than only at the
moment of abscission. A leaf still stops at the first cell that is not
organism-owned, so a drift piled against a root collar is unchanged.

**Measured with this entry's own instrument**, `crown_census frames=28800
trees=8`, one seed, one build each, on `main` rather than on P2:

| | before | after |
|---|---|---|
| y 120–160, litter | 283 | **25** |
| y 120–160, soil | 84 | **6** |
| y 80–120, litter + soil | 11 | **0** |
| highest row holding any soil | y 115 | **y 145** |
| total soil above the ground line | 508 | 498 |
| band 0 (y 160–200), litter | 661 | 1009 |

The total above the ground line is unchanged; the material moved from the
canopy to the floor, which is where the profile above says it was always
trying to go. `litter_probe frames=12000 trees=8` agrees: standing litter more
than 32 rows above the ground **23% → 0%**, resting on plant tissue 57.6% →
44.1%, and the four cells still airborne at frame 12,000 gone.

**What is still open, and it is this entry's second half.** Candidate 2 --
*cap the standing column above the original ground line* -- is untouched, and
the numbers above show why it still matters: band 0 litter went **661 → 1009**
because the same material now arrives at the floor instead of hanging in the
crown. Litter rots, so the floor should reach equilibrium rather than
integrating (`litter.ron`'s own rates), but nothing weathers the 5% that
becomes soil back out of the world, so the floor still has a source and no
sink -- `Reports/soil-accumulation-and-the-carbon-cycle.md` §4. And this was
measured on `main`; **P2's die-back was not re-measured**, so whether its
6,595–6,886 extra shed cells per stand still build a pile is unknown and the
die-back-off arm is still the control for it.
...
**The outcome exists now, 2026-08-29** — `rigid::topple`, and
`Reports/tree-fall-2026-08-29.md` is the write-up. A body that has come to
rest with its mass-weighted centre outside the middle third of its footing
turns that way instead of settling, on the far side of the flight where it is
still a *body* and going over is something it can do. The kern test above was
the right predicate all along; what it was missing was somewhere to put the
answer, and `rigid` is that place because the load model has no verdict
between *holds* and `breaks_into`. Nothing in `load.rs` changed.

**And the reason nothing had ever laid a piece down turned out to be one
counter.** Before this, `scene=fell fell=7150` reported **`quarter turns: 0
asked, 0 refused`** — the rotation mechanism has existed since M8, its fit
probe was repaired here as §K, and on a falling tree it had never once
executed. `spin` accrues at 0.012 per unit of speed and turns at 1.0; the
same census reports the fastest piece at **3.30 cells/frame** over a drop of
about thirty cells, so a whole fall banks 0.36 of a turn against a threshold
of one. So the tipping test could not have been judged before the rotation
worked, exactly as the paragraph above predicted.
...
is where a bug about what something looks like has to end.
...
### R. ~~`filmstrip scene=colony` panics at its own default seed, and degrades badly at others~~ — **FIXED AND GUARDED 2026-08-29. It was never the lake; the ground was under the trees.**

**The cause, censused rather than guessed.** The scene grows trees for 2,400
frames *before* founding anything, and then looked for dry ground by scanning
each column from `y = 0` for the first non-empty cell — which, in a wood, is a
leaf. Over the 308 candidate columns, topmost-cell kind:

| seed | Plant | Liquid | ground |
|---|---|---|---|
| **1 — the default** | **217** | 91 | **0** |
| 2 | 132 | 161 | 15 |
| 3 | 228 | 21 | 59 |

Seed 1 has no column whose topmost cell is ground, so `.expect("some dry
ground")` fired. §R2 below guessed this was the scene choosing a lake and
flagged that reading as unestablished; it is established now and it is a
different cause. It also explains why the numbers moved under the
2026-08-29 worldgen work (doubled sky, ~4x deeper soil): more soil, more
trees, more canopied columns.

**A second fault, larger, that the first was hiding.** `found_colony` searched
downward from **one cursor row** across a 204-cell span, so every column whose
ground stands *above* the cursor began its search inside the hill and found the
hill's interior. Of 52 sites that cost **18 (seed 1), 33 (seed 2), 20 (seed
7)** — the largest single term, and invisible because placement fails silently
and the count was never printed against what was asked.

**The fix.** `creature::colony_surface` passes through `Plant` with
`Empty`/`Gas`, and rises to the air above the column before descending — which
keeps the `Y` key's cave behaviour, guarded. `creature::colony_ant_site` is the
single site rule; `filmstrip`'s private `dry_surface` copy is deleted and the
scene calls it, so the two predicates can no longer disagree (that duplicate is
what §R2 is). The panic is now a message naming the seed and censusing the
band.

**Placed, before and after** — the scene now prints all three numbers, because
52 → viable and viable → placed have different causes:

| seed | before | after |
|---|---|---|
| **1** | **panic** | 13 of 23 viable |
| 0 | 29 | 27 of 40 |
| 2 | 19 | 14 of 26 |
| 3 | 18 | 21 of 33 |
| 7 | 26 | 23 of 31 |
...
**Read the drops honestly**: seeds 0, 2 and 9 place *fewer* ants than before
because the ants that used to be counted were standing on water (§R2). The
count is smaller and it is now true.

**Still open, and deliberately not chased here:** `placed` sits below `viable`
on every seed. Nest material is `Solid`, so the nest paint is not the cause;
the remaining term is `plant_ant` refusing sites where a two-cell `Chain(2)`
body will not fit. That is a real constraint rather than a defect — a colony
founded in a wood is genuinely sparser than one on a beach — but nobody has
measured which it is, and the three-number print is what makes it visible.

**Original entry follows.**

### R (original). `filmstrip scene=colony` panics at its own default seed, and degrades badly at others — found 2026-08-23 (WP-9 arm 1)
...
### R2. An ant put down on open water stands on the surface for ever, and `found_colony` puts them there — **PLACEMENT FIXED AND GUARDED 2026-08-29; the locomotion half is still open and is a design question**

**Reproduced on today's `main` before fixing** — the picture is four ants
standing on an open lake at frame 600 with nothing beneath them, `scene=colony
seed=2` at zoom 8.

**Placement: fixed.** Cause exactly as this entry diagnosed — the nest-painting
loop refused water from the day it was written and said why; the ant loop six
lines below never got the guard. `creature::colony_ant_site` now requires
`Solid | Powder` ground, and it is the *single* definition of the rule: this
entry's "not established, and stated as a guess" note about the scene and
`found_colony` using different predicates was correct that they differed, and
the duplicate is now deleted rather than reconciled. See §R for the census that
settled which predicate was wrong and why.

**Guard:** `creature::tests::found_colony_never_puts_an_ant_on_water`, on a bed
holding a plateau, open water and a canopy. It asserts the property this entry
is about — **every organism has at least one of its own cells resting on
ground** — rather than "no creature cell is ever over water", which fails for a
...
### R2 (original). An ant put down on open water stands on the surface for ever, and `found_colony` puts them there — found 2026-08-23 while rendering a WP-9 review card
...
**Corrected 2026-08-27, twice over — read this before acting on the
paragraphs above.** They say *"a one-line aux carry"* and credit
`particle::landed_cell` alone. There are **two** false anchors and both are
needed: `rigid::settle` lands rigid bodies through the same `Cell::new` zero.
Alone they recover **23% and 1%**; together, **99.5%** (36,348 wrong cells →
186). The 37,629 → 186 figure above pairs `main`'s baseline with a branch
that has *both* flags, so it folds in more than the one line it names.

`landed_cell` **ships on by default**; `settle` does **not** — it turns
`a_disturbance_extent_licenses_the_wound_but_not_the_chain` red and inverts
it (367 cells away with the wound licensed against 418 with a point licence,
against recorded figures of 840 and 649). **So §S is still open, at roughly a
quarter of the available win.** Full account, and the reason the first
measurement honestly read 1.6% and then stopped being true, in
`Reports/structural-support-model.md` §6.4a and §6.5b.
...
#### Root cause found, and one sign corrected — 2026-08-26
...
- **Two of the three verbs are now closed, and §S is down to the hammer —
  ...
  *bedrock-adjacent*, and everything around it inherits that stronger claim.
  ...
  `a_disturbance_extent_licenses_the_wound_but_not_the_chain` red.

  **The behaviour gate now guards this entry directly, and the earlier claim
  that it could not was wrong.** It was recorded here as "acceptance cannot
  see this rule", inferred from `grounded` reading 0 through `rockdrop` and
  `ligament` — which is exactly the weak-zero reading that counter's own
  caveat warns against. Tested differentially instead (every case at the
  shipped value against `GROUND_ROOT=flat`, diffed): **19 of 22 cases are
  blind, three see it** (`rockdrop`, `lavadrop`, `strike`) and all three
  passed anyway. `scene=strike` turns out to be §S in miniature — sites
  pending at frames 2/62/122/182 read **958, 968, 824, 289** shipped against
  **958, 2747, 5034, 7145** flat, one draining and one climbing
  monotonically at 512x320 in 182 frames. `max_sites=1500` now gates it and
  goes red for `GROUND_ROOT=flat` (7,145), `SETTLE_AUX=zero` (8,134) and all
  three off (9,079). It does **not** cover the particle seam, which lands
  three cells in a whole 8192x2560 run and none here.
...
- **And §S has a root cause — two of them, and both are needed.**
  ...
  (`AUX_TRAP=<frame>`, reporting any write that makes a cell body material
  reading `aux <= 2` with no bedrock adjacent, with a backtrace) caught
  **twelve consecutive false anchors on the two frames after the charge, all
  twelve `ParticleSystem::step`**, every one `empty aux 0 -> stone aux 0`
  beside `stone:2405`.

  `landed_cell` carries a particle's `aux` only for materials flagged
  `worth_in_aux` — the **food-value** flag — so every thrown rock lands
  claiming `aux == 0`, *adjacent to bedrock*. Its doc enumerates the `Liquid`
  fill and `Powder` moisture conventions and the organism packing, and is
  right about all three; on an **inert `Solid`** there is a fourth, the anchor
  distance, and there `Cell::new`'s 0 is not "no claim" but the strongest one
  available.

  **`PARTICLE_AUX_MAX=1` lands body material at `u16::MAX` instead** — the
  honest value and the cheap one, since an improvement is a single relaxation
  round whatever its size. Same scene, oracle at +1,300 frames: wrong cells
  **37,629 -> 186** (and the climb bucket, `|delta|` 1k-60k, **37,593 -> 0**),
  `produced` **7,491 -> 13**, `pending` **36,818 -> 5,957** against a ~5,400
  idle, scheduler **14.68 -> 0.03 ms**, frames over budget **86.9% -> 72.8%**.
  That is the state this entry's own `RECONVERGE_AT` oracle reached with a
...
### V4. `breaking_free_writes_a_pressure_impulse` never reaches `break_free` — **OPEN**

**A blind guard, not a broken one, and it has been blind since it was
written.** Found 2026-08-30 by the `FIELD_SCALE` 8 -> 16 lane, which is the
only reason it surfaced: at 16 the test went red, and the investigation into
*why* found that the assertion had never been connected to the function in
its name.

`src/sim/structural.rs`'s `breaking_free_writes_a_pressure_impulse` builds a
stone cantilever one cell past `max_unsupported_span`, schedules a structural
check at the tip, runs 200 frames, and asserts the field carries a pressure
impulse. Architecture §5c's claim — that a structural collapse stopped being
the one destructive event in the engine with no field footprint — is what it
is named for.

**Three measurements, each on its own sufficient:**

| control | result |
|---|---|
| `panic!("...")` as the entire body of `break_free` | test stays **green**, at `FIELD_SCALE` 8 and 16 alike |
| `break_free`'s `add_pressure_impulse` call deleted | pressure read is **byte-identical**, peak 32.07803 |
| `git blame` on the scene | the beam is free-floating, so it fails as one region |

The cause is in the test's own comment, which already half-noticed it: *"A
failure takes the whole overhang now, so the tip is usually lifted rather
than converted."* The overhang leaves the grid as a **chunk body**, so the
impulse the test observes is `rigid.rs`'s launch impulse (`rigid.rs:1012`,
`:4129`), never `structural::break_free`'s. `break_free` is not called once
in the whole run.

**Why `FIELD_SCALE` 16 exposed it.** The impulse is written at the cells that
actually break, and `COLLAPSE_IMPULSE_RADIUS` is 3, so `paint_field`'s
`radius / FIELD_SCALE` is 0 at either scale and each write lands in exactly
one field block. At 8 this 23-cell beam spans three field blocks and the tip's
(16..=23) happened to catch one; at 16 it spans two, every site lands in the
first (0..=15), and the tip's block reads exactly **zero**. The test was
passing on a binning accident on top of a misattribution.

**What was done, and what was deliberately not.** The read was generalised
from the tip's own block to the peak along the beam, which is scale-invariant
and is the property actually observable here — a structural collapse has a
field footprint. `CLAUDE.md` says a guard that will not go red is *blind*, and
that the remedy is to **replace it rather than widen it**, so the widening is
explicitly not offered as the fix: the entry is filed instead, and the test
now carries the panic-injection evidence in its own comment so the next reader
cannot cite its green as evidence about `break_free`.

**The repair this wants** is a scene in which a region genuinely crumbles
rather than being lifted — a failing region under `rigid::MIN_FRACTURE_CELLS`
(6) takes the `grit += usize::from(break_free(...))` path at
`structural.rs:1024`/`:3642`. That is structural-lane work and needs someone
who owns the rigid-vs-crumble branch; the `FIELD_SCALE` lane deliberately did
not redesign it in passing.

**Not a regression, and worth being explicit about that**: `break_free` does
write its impulse, and nothing here says the mechanism is broken. What is
broken is the only test that claims to check it.

### T2. The colony forages and never brings anything home: 1,651 pickups, 4 deliveries — **OPEN, found 2026-08-30 while landing Gate 0**

**What it is.** In the evolution lab's sealed herb bed, over 24,000 frames, a
52-ant colony picks food up **1,651** times, puts it down **1,586** times, and
delivers it to the nest **4** times. Censusing the stores at the end of the
run finds **0 food cells** standing within reach of a nest cell. The colony's
larder does not exist.

This is not cosmetic any more. Since 2026-08-30 an ant short of a child's
price keeps eating **from the nest's stores** (`Reports/creature-gate0-births-
2026-08-30.md`), so the stores are the ladder a birth climbs. Put food on the
ground beside the nest by hand and the same colony reaches **generation 13**;
leave it to forage and it reaches **generation 0**. Gate 0 is blocked on this
and on nothing else in the creature economy.

**Reproduction.** `cargo run --release --example windfall_probe -- frames=24000`
— the `the larder:` line.

**Isolated to a term, with a controlled probe.** `creature::act` sets the
out-of-nest drop probability to `drop_urge * moisture_gradient(x, y)`.
Scaling that product by a constant, same bed, same seed, nothing else
changed:

| | shipped | route drop x 0.1 | route drop x 0 |
|---|---|---|---|
| pickups | 1,651 | 319 | 115 |
| deliveries | **4** | 17 | **13** |
| nest larder at the end | **0** | 6 | **3** |
| births | 0 | 0 | **2** |
| deepest generation | 0 | 0 | **1** |

So the drop rule is the term. The probe was **not** landed — see below.

**The leading hypothesis, and why it is not being fixed from the creature
lane.** `moisture_gradient` is `|grad moisture|`, a **magnitude**. It is large
at *any* moisture boundary, and a soil bed under air is one continuous
moisture boundary that every forager walks along — so `drop_urge x gradient`
is near `drop_urge` for the whole route, and a laden ant unloads almost at
once. The stigmergy model this implements says material accumulates at
convex, *drying* sites; a magnitude cannot tell drying from wetting, and the
surface reads as maximal either way. That is a design question about the
construction mechanic (the same term drives digging, inverted), not a knob,
and §H's own entry above records that the guard over this scene is
**vacuous** — deleting `moisture_gradient` from the drop probability
entirely left it passing *harder*.

**What is ruled out by measurement.** Not starvation: the colony is eating
throughout (131-168 eats). Not the pickup verb: 1,651 pickups. Not the nest
being unreachable: 4 deliveries did happen, and `nest_visits` is in the
thousands in comparable scenes. Not a lab-only scene error either — the same
mechanism runs outdoors, where deliveries do land (1,733-2,156 per
`creature-birth-grant-2026-08-30.md` control C), so **whatever it is, it is
sensitive to the moisture field the bed sets up** and the sealed bed at field
capacity is the case that breaks it.

**Cheapest next step.** A counter pairing `drops` with the gradient value at
...
## Awaiting a decision
### S4. Rock still crushes itself on an idle world — smaller than it has ever been, and not zero — **OPEN, found 2026-08-28 by the owner's eye on the §S3 card**

Posted the §S3 before/after and the owner asked why the sea-stack in the
*fixed* arm is still cracked. It is a fair catch and the answer is that this
is a **different, older** bug that §S3's fix shrinks but does not remove.

**What the marks are.** `structural::sever_joint` — the crush path, a
*confined* overload failure that scores a joint in place instead of moving
anything. So they are structural failures, not weathering and not a worldgen
texture: the same stack is **clean at frames 2, 62 and 122** and acquires them
during the run.

**It predates §6.5d, which is what says it is not §S3's residue.** Same seed,
same frames, `scene=worldcrack strike=0 preset=rolling seed=7`, frame 3,302:

| build | overload failures | rock | the stack |
|---|---|---|---|
| before §6.5d (`GROUND_ROOT=flat`) | 167 | −262 | heavily bitten into |
| §6.5d as shipped (the §S3 regression) | 1,963 | −1,756 | gone, a rubble cone |
| with §S3's fix | **57** | **−76** | stands, finely cracked |
...
**Reproduction.** Grow one `tree` on the standard bed for 8,000 frames
(`plant_tree_on_ground(&mut w, 100, 20)` then `run_with_fields`), then flood
fill from any organism cell over `MaterialKind::Plant` cells with that
`organism_id`. About one individual in twenty-four ends with a cell the fill
cannot reach.

**What it looks like.** In the case traced, cell `(153,30)` — an underground
root cell — ends as a `MatureBody` with **all eight neighbours empty**.
Sampled every 50 frames, the sequence is:

| frame | `(153,30)` | organism neighbours |
|---|---|---|
| 7,950 | not an organism cell | 3 |
| 8,000 | `MatureBody` | **0** |

So `(154,30)`, `(153,31)` and `(154,31)` were root tissue at 7,950 and gone by
8,000, in the same window that a root tip grew forward into `(153,30)`. The tip
advanced while starvation dieback removed the tissue behind it, and the new
cell was orphaned by its own parent's death.

**It is not the growth walk's doing, and that is measured rather than
assumed.** Over 24 individuals: **1 stranded with `stem_stiffness` authored,
and 1 stranded with `STEM_STIFFNESS=0.0` forcing it off.** Identical rate. The
growth-walk change only reshuffles *which* individual lands on it.

**What it already cost.** `shedding_every_leaf_does_not_disconnect_the_stem`
asserted that the survivors of a total leaf-shed form one piece, which is a
strictly stronger claim than the leaf invariant it is named for — so any
change that reshuffled the stand could move this 1-in-24 onto the individual
the test happens to pick and fail a test about leaves for a reason involving no
leaf. It did exactly that. The test now compares connectivity *across* the
shed, which varies with leaves and with nothing else; it was confirmed to
still go red when a load-bearing stem cell is relabelled a `Leaf`.

**Not yet investigated:** whether the orphan matters. It has no carbon path,
so it should starve and be shed on its own, and one cell in ~1,800 is not
visible. The candidate fix is the one `shed_stranded_leaves` already
implements for foliage — a bounded component walk after removal — extended to
non-leaf tissue. That sits in the dieback machinery rather than in growth, so
it was left alone rather than widened into an unrelated change.
...
some are disintegrating a little. nothing falls over or breaks."* He is
right, and the reason is not in the breaking rule.

`plant::anchor_support` walks **every** organism cell at eight neighbours
with no cell-type filter, so foliage conducts support exactly like wood. A
crown therefore stays attached to the ground *through its own leaves*, and
severing the wood that actually carries it changes nothing.

Measured on `scene=grove seed=1` with `SNAP_PROBE=1`, three self-breaks over
9,000 frames:

| | severed by the support check | left as pieces |
|---|---|---|
| shipped | **42 cells** | 79% |
| leaf conducts nothing at all | 4,227 cells | **1%** |
| leaf holds leaf, never wood | **810 cells** | **91%** |

The snapped cells read **section 1 carrying 820** — a single-cell-wide member
bearing most of a crown. It breaks, and fourteen cells come away.

**`leaf.ron`'s own doc already says a leaf must not be a load path**, and the
span rule honours it (`max_cantilever_reach: u16::MAX`). This field never
did.

**The middle row is a dead end and is recorded so it is not retried.**
Blocking a leaf from conducting to anything is much worse than the bug:
foliage grows in clusters, so only the one leaf touching a twig is ever
reached and every leaf behind it reads as detached — the stand sheds its
whole canopy as grit. The rule has a *direction* in it: leaf-to-leaf carries
...
structural bridge then all of them are being answered by a plant that is
held up by its leaves.
...
./target/release/examples/plant_probe species=grass trees=16 frames=45000 worldseed=1
  -> population: 16 organisms -- 8 established, 8 seeds or seedlings; 0 seeds set in total
     generations  [gen 0: 16]
...
Appended rather than filed beside §0d and §B, which it belongs with: the
file is co-owned and reordering it conflicts with every open branch (see the
index block above). The index is what says whether a bug is live.

### B2. A living plant in the lab pulls its own anchorage out from under itself and is felled whole — **OPEN, found 2026-09-01 from an owner playtest; masked, not fixed**

Owner, from play: *"I turned COLLAPSE UNDER LOAD off, but trees are still
falling over."* They were right about the symptom and the switch was doing
what it said; what it said was the wrong scope, and chasing that turned up
this underneath it.

**What was measured**, `examples/labshot founders=8 colonies=1
frames=0,20000`, one binary, the two arms switched at run time:

| `plant_load_failure` | severed by the support check | of those, alive | standing plant cells |
|---|---|---|---|
| on (shipped) | 683 | **654** | 1,935 |
| off | 31 | **0** | **2,923** |

`snapped under load` reads a flat **0** on this bed in both arms — the two
*load* rules the switch originally covered never fire here at all. Every one
of those 654 cells came down through `structural::organism_structural_tick`'s
`detached` branch.

**It is whole plants, not crowns.** A trace on the felling site reported
every piece as `N of N cells, anchors 0` — the entire organism, up to 75
cells, with no anchor left anywhere in it. A second trace on the anchor
seeding split those plants into two populations, and neither is a plant that
has been damaged:

- plants with **zero** root cells, and
- plants whose one root cell touches only `litter`, `deadwood`, `log` or
  `wood` — never soil.

**The loop.** `plant::anchor_support` schedules a structural check on any
cell whose distance *rose* (§0d's closing paragraph records that this became
true and that the latency argument protecting it expired). A plant that never
had anchorage therefore marks its own cells `u16::MAX`, schedules its own
check, and `organism_structural_tick` reads `detached` and fells it. Nothing
disturbed it. It is the plant's own upkeep pass calling the demolition.

**Why the lab and not the outdoor game**: the lab bed accumulates litter and
deadwood, and seeds germinate on top of it, so a plant can root into a
material that anchors nothing. Outdoors a seedling almost always finds soil.

**Two repairs were tried and both are recorded as measuring nothing** — do
not re-run them without changing the condition:

- letting a root anchor to an *anchored neighbour's* rootwood. No measured
  change.
- treating "never anchored" as different from "newly unanchored". Fired
  4,919 times, all of them seeds with no `breaks_into`, i.e. cells that were
  not being felled anyway.

**What shipped is a mask and says so.** `World::plant_load_failure` now also
covers the `detached` branch, but only for a **living** organism — a
senescent one still comes apart, so culling, rot and the axe are unchanged,
and the default is on so the outdoor game sees none of it. That is a control
in the owner's hand, not a repair: with the switch on, this bug is exactly as
live as it was.

**The real fix is one of two questions, and neither is answered here.**
Either the anchor rule is too narrow (should a plant rooted in packed litter
be anchored? `anchors_organisms` is a per-material bit and litter does not
have it), or a plant that has *never* been anchored should not be routed to
felling at all — it has not lost anything, it never had it, and `detached`'s
own doc is written about tissue that *came off*. The second is closer to the
...
```
cargo run --release --example reseed_probe -- frames=13500 every=13500 founders=1 col=60 seed=3
```

…with `falls_through_organisms: true` added to `assets/materials/seed.ron`
(the §3 candidate in [`plant-reseeding-2026-09-03.md`](plant-reseeding-2026-09-03.md);
rebuild, per the `include_str!` gotcha). That arm reports **79 seed cells
borne and 164 germinations**. The other three world seeds on the same arm
read 154 → 75, 138 → 62 and 98 → 39, i.e. normal, so it is not every run.

**The birth count is not the thing that is wrong, and that was checked
before the bug was filed.** Three independent counts of seed creation agree
on 79: `World::seeds_borne` (added for this, incremented in
`plant::bear_seed_at` where both seed paths meet), the sum of the parents'
own `OrganismState::seeds_set`, and a per-frame watch for organism ids
appearing as fresh single-`Seed` organisms. The excess is real
`plant::germinate` calls.

**Suspected route, unconfirmed — do not treat this paragraph as the
finding.** `becomes: Seed` is resolved by *detaching* only under a
`FateWhen::Ripe` rule, which routes through `plant::drop_organ` →
`bear_seed_at` (`plant.rs:4003`). Every other `when` falls through the
general path at `plant.rs:3212`, `self_type_after_grow = fate.becomes`,
which **relabels the cell in place on the living parent**. A `FateGenome`
`Insert` mutation writing `becomes: Seed` on a `Grew` or `Node` rule would
therefore turn a live cell into a `Seed`, which then meets
`Behavior::Germinate` and converts straight back into a `GrowingTip` — a
germination on a cell nobody paid for, repeatable, and a fresh
`plant::seed_genotype` draw over a living individual's genome each time.
`FateGenome::mutate`'s own doc records that three of its four operators,
`Insert` among them, are **unmeasured**.

**Why it matters beyond the counter.** `germinations` is the exact number
every reseeding, establishment and viability measurement in this engine
divides by, including all of `plant-reseeding-2026-09-03.md`. While this is
live, every germination rate quoted anywhere is an overcount of unknown
...
every ant, for its whole life.

**How it was found**, which is the part worth keeping: not by looking for it.
`lab::plainspeak` reads a genome back as English, and its guard
`a_scent_is_named_for_what_the_animal_lays_on_it` went red on the merge
asserting `ALWAYS LAYS HOME SCENT`. That assertion *should* have gone red —
`(Bias, EmitA, 2.0)` was deliberately removed, because a constant floor on A
drowns the falloff that is the homing gradient (`ant.ron` says so at the
weight). But the sentence that should have replaced it did not appear either,
and chasing *that* absence is what reached the mechanism. **A page written to
say what an animal does is a reader for channels nothing else reads**, which
is the standing check `dead-ends.md` calls this project's three-time failure.

#### The mechanism

`ant.ron` authors the odometer as three weights on hidden unit 4:

```
hidden:     (AtNest,  4, 0.0005)
outputs:    (4, EmitA, 1609.1)
recurrence: (4, 0.9999)
```

`brain::eval_brain` adds the recurrence term unconditionally but gates every
**input → hidden** contribution on `w.abs() >= W_EPS`, and `W_EPS` is `0.01`.
The charge wire is **twenty times below it**, so it is skipped. Unit 4 has no
bias and no other input, starts at 0, and `0.9999 * 0 = 0` for ever. `EmitA`
is then `squash(-0.35) = -0.259`, and `creature.rs` clamps the deposit to
`[0, 1]`, so nothing is ever laid.

#### Measured, not read

Driving the shipped ant's own genome through `eval_brain` directly — 400
ticks with `AtNest = 1.0` (standing *on* the nest), then 3,000 away:

| | h4 | EmitA | clamped |
|---|---|---|---|
| on the nest, t=0 … 399 | 0.000000 | −0.259259 | **0.000000** |
| away, t=0 … 2,999 | 0.000000 | −0.259259 | **0.000000** |

Flat at every sample. The positive control is the same wiring with the charge
at `W_EPS * 2`, which charges and decays as designed — so the probe can
report a live odometer and does.

#### It is both odometers, and only those two

A census of every authored weight in every shipped species against `W_EPS`
returns exactly two entries, and they are the two odometers:
...
Nothing else in the tree authors a sub-`W_EPS` weight. So this is not noise
in the species files; it is one modelling step taken twice.

#### Why it was not caught

The design note records the curve being *"verified before it was built,
because §4's safety claim rests on it"*, simulated over the real recurrence
at the real weights and giving 0.785 → 0.046 across 3,000 ticks. That
simulation is right about the arithmetic and was run **outside `eval_brain`**,
so it never saw the `W_EPS` gate. `CLAUDE.md`'s *ask what your number counts*
with the instrument named: a **simulation** lies by modelling the mechanism
rather than the path to it.

Nothing in the suite covers it either. The brain's own guards assert
round-trip fidelity of the weights (`wiring_from_genome` deliberately emits
sub-`W_EPS` weights, and a guard asserts it does), not that an authored
...
### W2. Two soil constants annul each other, and the root depletion zone lost 6.3x of its depth — **REFUTED by measurement the same day. The depletion zone is at the floor already; raising the constant is a monotone tax. Read W2a.**

Found 2026-09-05 by adversarial review of
`Reports/plant-soil-nutrient-plan-2026-09-05.md`. Nothing here is a new
mechanism; it is two existing constants that stopped composing, one day
apart, and nothing re-derived either.

**What `SOIL_UPTAKE_PER_TICK` is for.** Its own doc (`plant.rs:2625-2634`):

> *Sized so a root draws a soil cell **from field capacity down toward the
> wilting point over a handful of its own ticks** rather than instantly: the
> interesting behaviour is a root system **competing with itself and its
> neighbours for a finite local store**, and an instant drain would make
> every root independent of every other. **Untuned beyond that reasoning,
> and a first-class target for the economy pass.***

**What annulled it.** The maximum standing depletion a root can hold around
itself is the capillary rest gap, because below that gap capillary does not
run. That gap fell from 380 to 60:

| | commit | date |
|---|---|---|
| `SOIL_UPTAKE_PER_TICK = 60` | `aef0a53d` | 2026-08-30 |
| `SOIL_CAPILLARY_REST_UNSATURATED = 60` (was an unconditional 380) | `3c46ddad` | **2026-08-31** |

| | rest gap | cell parks at | ticks of drinking to reach it | plant-available there | per-cell uptake lost |
|---|---|---|---|---|---|
| before 2026-08-31 | 380 | 240 | **6.3** | 0.136 | **86%** |
| today | 60 | 560 | 1.0 | 0.864 | **14%** |

`380 / SOIL_UPTAKE_PER_TICK = 6.3` — the doc's own *"a handful of its own
ticks"* is exactly the pre-change arithmetic.

**Why it is a regression and not a design change.** `3c46ddad` is correct
and necessary on its own terms — the wide threshold was making fresh ground
permanently sterile (0 of 16 germinations against 16 of 16), and that fix
must stand. What was missed is that the same number was load-bearing for a
*second* mechanism in another file. This is `CLAUDE.md`'s *"fixing a bug
often exposes a constant that was compensating for it"* in reverse: a fix
exposed a constant that was **depending** on it.

**The measured consequence.** The depletion a root can hold is now 60 of a
440-wide plant-usable band — **13.6%** — so a crowded root and an isolated
root drink essentially the same water, and root density buys nothing. The
equilibrium gap a single drinking root can sustain against capillary is
`g* = 1.333 / (3 x 0.06 x 0.62) = 12` units, far below even the 60 gap, so
**capillary is not the limiting term — the rest threshold is.**
Independently confirmed by `SOIL_CAPILLARY_REST_UNSATURATED`'s own doc,
which records a standing profile of 168 / 188 / 253 / 313 / 373 —
gaps of 20 / 65 / 60 / 60 — *"standing unchanged from frame 12,000 to
48,000."*

**Why this matters beyond soil water.** It is the reason
`plant-roots-and-transport-2026-09-05.md` measures roots as saturating after
~20 contact cells, and that report attributed the saturation to water being
the world's only soil resource. It is at least partly this instead — a dated
constant regression with a one-line candidate fix.

**The cheap test, and it costs no calibrated income constant.** Raise
`SOIL_UPTAKE_PER_TICK` 6-10x. `absorb_water`'s own comment
(`plant.rs:520-528`) is explicit that this is free of the economy:
*"**Income is unchanged**, and that is the point: the plant still gains at
most `rate` per tick per wet neighbour... What changes is how fast the
*pond* goes down."* `drawn` does not reference the constant; only `taken`
does. Run `plant_severance` for income-against-root-mass and
`selection_arena` for the teeth test, at an order statistic over >=12 seeds,
and watch `a_tree_denied_water_dies_and_a_watered_one_does_not` —
`dead-ends.md:1122` records it as the test that moves whenever soil water is
touched.

**Do not read this as "the nutrient plan is unnecessary."** Restoring the
depletion zone plausibly delivers root *density* and *exploration* paying
(that plan's S1 and S2). It does not deliver two viable root morphologies in
different beds (S3), which needs a second resource axis with a different
spatial distribution, nor does it decouple root income from rainfall.

### W2a. The refutation, measured — the rest gap sets the depletion zone's *width*, not its depth

W2 above was filed on arithmetic and refuted by measurement within hours.
Both halves of its reasoning were wrong, and so was the same reasoning in
the adversarial review that produced it and in
`plant-soil-nutrient-plan-2026-09-05.md` §0a.

**The measurement.** `examples/plant_severance` now reports a depletion
contrast — median plant-available water in soil touching root tissue,
against soil at the same depths >= 8 cells from any root. At the **shipped**
constant, 12 seeds, frame 24,000:

| | median plant-available |
|---|---|
| soil touching roots (`near`) | **0.016** |
| soil >= 8 cells away (`far`) | **1.000** |

The soil a root touches is **already at the wilting floor**. There is no
missing depletion zone. W2's central claim — that the depletion zone lost
6.3x of its depth — is false.

**Why the arithmetic misled, and this is the part worth carrying.** The
capillary rest gap is a bound on the difference between **two adjacent soil
cells**. It therefore caps how *steep* the depletion boundary is, not how
*deep* the depletion goes: a root system can draw a whole neighbourhood to
the wilting point in a staircase of 60-unit steps, and does. The measured
profile says so almost exactly — 0.016 to 1.000 is moisture 187 to 620, a
433-unit rise over the 7 cells between BFS distance 1 and 8, which is
**61.9 units per cell against a rest gap of 60.**

So lowering the gap 380 -> 60 on 2026-08-31 made the depletion zone
**wider** (a 7-cell ramp instead of a 1-2 cell cliff), not shallower — and a
wider zone means *more* overlap between neighbouring root systems, i.e.
**more** competition for a local store, not less. The change helped the
thing W2 accused it of breaking.

**And the lever does not work.** `PIXEL_PHYSICS_SOIL_UPTAKE` at 1x / 2x / 4x
/ 8x, 12 seeds each, paired by seed, read at frame 24,000:

| arm | root cells | contact | income | water_status | `near` |
|---|---|---|---|---|---|
...
| arm | root | income | seeds root up | seeds income up |
|---|---|---|---|---|
| 2x | 1.36x | 0.78x | 8/12 | 4/12 |
| 4x | 1.61x | 0.46x | 10/12 | 1/12 |
| 8x | **2.08x** | **0.11x** | **12/12** | 1/12 |

It reliably drives root investment — 12 of 12 seeds at 8x — and income falls
faster than roots rise at **every** setting. Roots double; income falls to
11% of control. **There is no usable band.** The reason is the first table:
with `near` already at 0.016 there is nothing left to deplete, so a bigger
draw takes the same nearly-zero water faster and only pushes water status
down.

**The one thing W2 said that survives**: `SOIL_UPTAKE_PER_TICK`'s doc does
state this design goal and does call itself untuned, and the two constants
did land a day apart. That is a real observation about the record. It is not
a regression, and the constant is not the lever.

**What this reframes.** Roots do not fail to pay because the soil is not
depleted. They fail because the soil they can reach is **already exhausted**,
so the only thing worth anything is reaching **fresh** soil — which makes
root *reach* the binding constraint, not root income. That is the biology
review's F2 arriving by a second route, and it is what the plan now leads
with.

**A null the design cannot deliver, stated so nobody re-reads it as one.**
Pooling the 48 runs and splitting on `break_root_tips`' 0.95 gate gives
median root 305 (gate shut) against 364 (gate open) — but the two groups
also differ 8x in `near` (0.016 against 0.002), because the arm drives both.
**Arm is a confounder and this split cannot separate the gate from the
depletion.** Whether the root-tip gate is the binding constraint is still
open and needs an arm that moves the gate alone.
...
Four documents asserted that per-row dirty spans are *strictly tighter and
still a superset of every cell the sweep can act on*, so that the only reason
they change behaviour is the per-chunk RNG stream shifting under a narrower
visit order (`chunk.rs`'s `row_spans_enabled`, `dead-ends.md`,
`evolution-lab-frame-cost-2026-09-01.md` §5, and
`sweep-positional-rng-2026-09-05.md` §1). **None of them measured it.**

**The measurement.** Key the sweep's per-cell draw on position and frame
instead — `PIXEL_PHYSICS_RNG=positional`, which exists for this — so that
visit order cannot reach the draw at all. If the premise held, the two sweep
arms would then produce a bit-identical world. They do not:

| frames | box vs rows, `PIXEL_PHYSICS_RNG=positional` |
|---|---|
| 2,000 | identical, `0xddb39ab874e7afb4` |
| 4,000 | identical |
| 4,329 | identical, `0x179bfe9180fbb3a2` |
| **4,330** | **first diverging frame** — `0x7db58b0506d8ccaf` vs `0x132e705e5eb068c9` |
| 40,000 | `0xc686c0eaaeb4fc96` vs `0x34fef6b42b4cb6fe` |

Every hash above was re-verified on `main` after the 2026-09-05 merge, so they
are values to check against rather than a snapshot of one branch.

Bisected to the frame. Every arm's hash reproduces exactly across three reps,
so this is a real behaviour difference and not noise, and the engine's
determinism is intact. The control is in the same table: under the shipped
per-chunk stream the same two arms differ from the first frames, so the
comparison can report a difference and does.

**Reproduce:**

```
cargo build --release --examples
export PIXEL_PHYSICS_RNG=positional
for f in 4329 4330; do for sw in "" rows; do
  PIXEL_PHYSICS_SWEEP=$sw RAYON_NUM_THREADS=4 ./target/release/examples/lab_cost \
    width=1024 height=288 soil=96 founders=6 colonies=0 species=tree seed=1 \
    frames=$f every=$f phases=1 render_every=0 plant_load=0 | grep "world hash"
done; done
```

4,329 gives one hash twice; 4,330 gives two.

**What is ruled out.** The RNG: the whole point of the positional arm is that
the draw a cell gets is a pure function of `(seed, x, y, frame)`, so a
narrower region cannot shift it. Noise: three reps, exact reproduction. A
drift that accumulates: 4,329 frames identical then a split is **one event**,
not an error growing.

**What is not ruled out, and the leading guess.** Chunk wakefulness.
`field::step` is gated on `active_chunk_count()`, so a narrower sweep letting
one chunk settle a frame earlier changes the field's solve set, and the field
feeds light, heat and moisture back into everything. **This is a hypothesis
and nothing has measured it.** Whoever picks this up: frame 4,330 on `seed=1`
is the handle, and the first question is whether the two arms' awake-chunk
sets differ *before* their cell state does.

**Why it matters beyond the spans.** If the coupling is wakefulness rather
than anything about spans specifically, then it binds *any* future change to
what gets swept or to when chunks sleep — which is a live constraint on the
engine, not a footnote on one rejected optimisation. That is the reason this
is filed here rather than left in `dead-ends.md`, which owns "was this tried?"
and answers "yes, don't retry it".
...
defect with a general lesson under it, not a plant bug.**

**The geometry.** `examples/common/mod.rs`'s `PlantScene::build` lays a
stone floor from `ground_y + depth` to `ground_y + max_soil_depth +
STONE_DEPTH`. At the shipped defaults -- `width` 512, `height` 320,
`ground_y` 200, `SOIL_DEPTH` 34, `STONE_DEPTH` 6 -- that is a slab **512
wide and 6 thick, spanning the whole world, with y 240..319 empty beneath
it**. `load.rs` reads `Cell::OUT_OF_BOUNDS` as bedrock, so its only anchors
are the two world edges. Stone's `max_unsupported_span` is 16, so
`capacity_within`'s base is `16^2 / 2` = **128**, against a 3,072-cell
section spanning 512 cells. The slab cannot pass the load model. It is the
`[3072]` entry in every long grove run's `hanging census`, and the
arithmetic is exact: 512 x 6.

**Why nobody noticed: it is never asked.** Control, and it is the whole
finding -- a bare bed, `filmstrip scene=grove plants=0`, over 24,000
frames:

| frame | hanging census | overloaded | unsupported | awake |
|---|---|---|---|---|
| 6,000 | `[3072]` | 0 (0 cells) | 0 (0 cells) | 0/40 |
| 12,000 | `[3072]` | 0 | 0 | 0/40 |
| 18,000 | `[3072]` | 0 | 0 | 8/40 |
| 24,000 | `[3072]` | 0 | 0 | 1/40 |

Zero failures of either kind for the whole run. Nothing disturbs the slab,
so `structural::tick` never evaluates it, and a structure the load model
would reject on sight stands indefinitely.

**`plants=0` could not be expressed until this was filed**, which is why
the control had never been run: `filmstrip`'s `plants` was a `usize` with
zero as its not-given sentinel, so the argument silently meant "8". It is
`Option<usize>` now. `CLAUDE.md`: a control that cannot be expressed cannot
be run.

**What wakes it is a tree falling on it.** `settled log pieces` is zero in
every arm until the first large fall, and the collapse follows: with the
root switches off the first fall lands at frame ~14,450 (10 pieces holding
928 cells, the largest a **619-cell log, 45x74, standing upright**),
`overloaded` first appears at 17,000 (19 failures, 1,002 cells), the floor
cluster drops 3,072 -> 2,189, and by 18,000 it is 553 failures over 6,032
cells with the slab and the soil above it lying at the bottom of the world.

**A confound that is not resolved, stated rather than buried.** With the
root switches ON the same seed runs to 36,000 frames carrying 33 settled
log pieces holding 1,568 cells and reports `overloaded 0` at 18k, 24k, 30k
and 36k, floor intact -- more fallen mass over twice the time than the arm
that collapsed. So the roots-on bed is genuinely more robust and not merely
later. **Why is not separated**: its largest single piece is 526 cells and
44 rows tall against 619 and 74, so "a smaller impact" and "a root-bound
bed spreads the load" (`rootwood.reinforces_powder`) are both live, and one
...
**The method note, because the error cost more than the bug.** The first
write-up of this reasoned from a contact sheet straight to a mechanism
("root-bound ground holds a slope bare soil loses") and shipped that claim
to `README.md` and `wiki/plants.md` before root-causing it. Both were
corrected. What settled it was the bare-bed control -- one run, no
argument -- which is the same remedy §A records after two wrong answers
reached by reading diffs and reasoning about mechanism.

---

### W4. A rooted bank now sheds *more* of its own soil than a bare one, because it still has the soil — **OPEN**

Filed 2026-09-06 by the branch that made buried tissue rot into the ground it
displaced (`plant::shed_to_litter`, `World::bed_cells_on_loan`). It is the
measured cost of that change, recorded rather than absorbed, and it is the
reason `a_rooted_bank_sheds_less_soil_than_a_bare_one` now asserts on `crest`
instead of on `shed`.

**What changed.** A root reaches its cell by displacing one, and when the
plant died that cell used to become `litter` and pay `litter.ron`'s 5%
humification yield — so about nineteen bed cells in twenty were destroyed per
generation of root turnover. Measured with `examples/labmass`, forced turnover
over 40,000 frames: **842 cells of bed destroyed before, 8 after.** That is
the owner's *"over time it seems like the soil is disappearing"*, closed.

**What it costs.** Attributed with the rule's own ablation switch, one binary,
same seed, `a_rooted_bank_sheds_less_soil_than_a_bare_one`:

| `PIXEL_PHYSICS_BURIED_REMAINS=` | shed (bare/sod) | share (bare/sod) | crest |
|---|---|---|---|
| `litter` (the old rule) | 327 / 130 (−60%) | 0.252 / 0.161 | +31% |
| default (the new one) | 327 / 411 (+26%) | 0.252 / 0.383 | +30% |

`share` is `shed / (shed + soil still in the footprint)`, added to that test in
the same change: a raw count is confounded by how much soil the arm *has*, and
under the old rule the sod bank shed less partly because its grass had
permanently eaten 38% of it. The share says the trade is real and not a mass
artifact.

**What did not change is the mechanic itself.** A live root still holds the
soil it threads — `crest` (occupancy of the bank's top four rows) is +30% for
the sod arm under both rules. What moved is the bank's *afterlife*: a dead
root now leaves a loose grain where it used to leave nothing at all, so a bank
whose grass has turned over is looser than one that never had grass. Under
this scene's disturbance — a ledge with both retaining faces removed at once —
that spills.

**Three repairs measured, none of them recovers it, so do not retry these.**

- Drawing the returned cell's water off its neighbours *greedily*, to the
  neighbours' mean: **459 shed, worse than doing nothing**. It empties the
  first donor toward the wilting point and trades one dry cell for a 440-unit
  gradient two cells wide.
- Levelling the whole nine-cell neighbourhood (what shipped, because it is
  right on its own terms — it conserves and leaves no gradient): **411 against
  418 for writing the cell dry.** A wash on this scene.
- Writing the cell at `SOIL_FIELD_CAPACITY`: **198, which recovers the
  number** — and it mints water out of nothing, which is the one thing that
  cannot ship (`decay::tick` records the same trap at length).

**Where to look next.** The returned grain is loose and sits in the root zone,
which is the most spill-prone part of a bank. Candidates nobody has measured:
returning the cell only where it is enclosed on all *eight* sides rather than
four; giving the returned cell a short-lived cohesion, which is
`reinforces_powder` pointed at the ground rather than at the root; or deciding
that this is correct and the scene's disturbance is simply extreme, which is
the cheapest answer and is not yet evidence.

---

### W5. The lab's bed grows a water table on its stone floor, and it does not stop — **OPEN**

Found 2026-09-06 while profiling the bed by depth to decide whether the wet
un-pack rule was a hazard or a dead zone (it was the second; §W4's sibling
finding, and the reason `PIXEL_PHYSICS_WET_COLLAPSE` now defaults to off).

**The measurement.** `examples/labnest frames=12000 seeds=1 founders=8`, the
shipped `LabBox`, cells past `SOIL_WATERLOGGED` and mean moisture per depth
band:

| rows below the surface | waterlogged, frame 1,333 | frame 12,000 | mean, 0 → 12,000 |
|---|---|---|---|
| 0–12 (surface) | 22 | 346 | 620 → 620 |
| 12–84 (the middle) | 0 | ~0 | 620 → 623 |
| **84–96 (the floor)** | **94** | **1,559** | **620 → 719** |

Monotone across every sample — 94, 288, 500, 636, 769, … 1,559 — with no
plateau, while every band above it holds flat. The middle of the bed is
unchanged; this is entirely a bottom-band accumulation.

**The mechanism is not in doubt and is arguably correct physics.**
`update_soil_water`'s drainage rule moves water down whenever a cell is over
field capacity; `stone` has `water_capacity: 0`, so the bottom row has nowhere
to send it; and the box is sealed, so nothing leaves. A pot with no drainage
hole waterlogs at the bottom. What makes it a bug rather than a feature is
that it has **no equilibrium**: the wet front rises for as long as the run
lasts, and `weather::condense_under_a_lid` keeps returning evaporated water to
the *top* of the box, so the profile is a slow one-way pump — surface drying
while the floor floods.

**What it costs today, and what it would have cost.** Today: not much that is
visible. It no longer takes nests down (that rule is off), and the plants root
in the upper bed. Before the rule went off it was the whole of the wet-collapse
problem — a rising band of ground no burrow could stand in.

**What has not been measured.** Whether the front reaches the root zone on a
long run, and what that does to germination and to `soil_water_threshold`;
whether it is worse at `founders=128`, where the bed is wetter overall (~4,755
cells past the line against ~1,850 at 8 founders, though the depth split was
not taken at that scale); and whether `weather::condense_under_a_lid`'s share
makes it faster or is irrelevant.

**Candidates, none tried.** A drainage sink at the floor (the physical answer:
a pot gets a hole, and the water would have to go somewhere — the atmospheric
bank is the obvious destination, which `displace_soil_water` already credits).
Capping a cell's moisture below saturation where it sits on an impermeable
floor. Or deciding this is correct and giving the player a drain to place,
which fits the owner's stated direction — *"the world starts with nothing, but
...
tree/sky."* The card's verdict chose that arm anyway (the other one lost the
whole world), so this is the residual defect in the arm that won.

**The chain is four links and every one of them is in the tree today.**

1. `organism::soil_water_fraction` returns a **`Liquid` cell's fill on the
   same 0..1 scale it returns soil moisture on** — so free water reads
   **1.000** where soil at `SOIL_FIELD_CAPACITY` reads **0.620**.
2. `moisture_pull` differences that read at `MOISTURE_SENSOR_OFFSET = 4`, so
   standing water four cells above a root bids **0.380** against a
   `MIZ_THRESHOLD` of **0.050** — it clears the bar by **7.6x**, and no soil
   gradient can outbid it short of full saturation.
3. `Grow`'s `RootTip` arm takes the hydrotropic direction **instead of**
   gravitropism, not blended with it: `Some((dir, s)) if s >= MIZ_THRESHOLD
   => dir, _ => (0.0, 1.0)`.
4. `growable` returns `true` for `EMPTY` **unconditionally**, so there is
   nothing to stop the tip leaving the substrate once it is pointed up.

So any standing water above a root — rain on the surface, a drip, a puddle,
the lab's own water table (§W5) — steers root tips **up and out of the soil
into open air**, which is what the owner is looking at.

**And the scene the report came from is a rainy one.** `filmstrip`'s own
`grove`/`wood` doc records that *"`weather::at` is a pure function of `(seed,
frame)`, so a different window grows the stand under different rain"* — so the
sheet the owner was reading had rain falling on the surface above every root
in it, which is the source term this chain needs. The lab's water table (§W5)
is the same mechanism with the sign the other way: it sits on the floor, so it
pulls roots **down**, and nobody notices because that is where they were going.

**Reproduction:** `free_water_above_a_root_outbids_the_soil_and_points_it_up`
(`src/sim/plant.rs`). Uniform damp soil reads flat; one row of free water
...
sealed pocket beside the probe. Its buried sibling uses dry-vs-wet *soil*.
Neither constrains the vertical sign at all, so the upward case was never in
...
`soil_water_fraction` blind to `Liquid` would break it. The two candidate fix
shapes, neither measured:

- **Refuse the step, not the smell** — a `RootTip` may not take a growth step
  into `EMPTY` that leaves the substrate. Narrow, and leaves the gradient
  alone; needs a definition of "leaves the substrate" that a root growing
  through a crack does not trip.
- **Blend instead of override** — sum the gravitropic and hydrotropic vectors
  rather than letting MIZ1 replace gravity outright. Closer to the biology
  (MIZ1 is an antagonism, not a switch) and it re-derives `MIZ_THRESHOLD`,
  which is the cost: that constant was tuned against a rule where it acted as
  a switch.

**Adjacent, same channel, different bug:** §F1's note above records
...
and will that somehow still feed the plant that it's not technically
connected to anymore?"* Yes, yes, and yes.

**The whole-organism channels do not check connectivity, and there is
...
Every cell asks *who owns me* and gets the organism's tank. `credit_water`
writes to that same tank keyed by `organism_id`, `water_status` multiplies
every photosynthetic credit in `allocate_to_frontier`, and **no step in that
path asks whether the cell can reach a root.** `state.nutrient_status` is
the same shape and inherits the same blindness, so the soil nutrient shipped
2026-09-06 is bought by roots the earning tissue is not attached to.

**Nothing splits an organism on disconnection.** There is no
`split_organism` in the tree. The only thing that removes a severed crown is
structural, and it is switched off in the lab:

```rust
// structural.rs:1323
if detached && !world.plant_load_failure && !senescent { return Vec::new(); }
```

**So which game you are in decides whether the defect is visible.** Outdoors,
at the shipped default, a basal cut is correctly fatal — `plant_severance`
measured the crown leaving the organism inside one stop via
`rigid::fell_severed_tissue`, with the rootstock rotted 221 → 21 cells. **In
the lab box, where `COLLAPSE UNDER LOAD` is off by the owner's own play, the
crown stays and the economy feeds it indefinitely.** That is the original
report this line has been chasing since #246: *"I have seen plants continue
to grow after the trunk has been fully severed near the base."*

**Carbon is the one channel that behaves, and the asymmetry is the clue.**
`carbon_at` reads *that cell's* own carbon and `organism::transport` moves it
only between touching four-neighbours, so a severed crown cannot draw the
stump's reserves. It does not need to: its **income** is unaffected, because
...
- **Resolve the whole-organism reads per connected component.** Honest and
  ...
  the component rather than the id. It touches the economy, so it needs its
  own paired measurement, and the component walk has to use the **eight**
...
repair, and the owner turned it off because living trees were falling over.

---

### Z6. Every shipped bed starves its ant colony inside one play session — **OPEN**
...
**Reproduction** (`RAYON_NUM_THREADS=1`, release, main at `389192a9`):

```
cargo run --release --example labstats -- founders=8   colonies=1 frames=300000 seed=N   # the default box
cargo run --release --example labstats -- founders=256 colonies=3 frames=300000 seed=N   # the full box the owner plays
cargo run --release --example labstats -- founders=8   colonies=2 frames=1000000 seed=N  # the two-colony harness bed, a campaign
```

Ants alive at each stop, then births over the run and the deepest animal
generation reached, then the cause of every death from the group tally:

| bed | seed | 50k | 100k | 150k | 200k | 250k | 300k | born | deepest gen | deaths |
|---|---|---|---|---|---|---|---|---|---|---|
| default (8 herb, 1 colony of 52) | 1 | 23 | 20 | 5 | 0 | 0 | 0 | 163 | 12 | 214 starved, 1 starved aloft |
| default | 2 | 14 | 10 | 5 | 5 | 6 | **3** | 92 | 13 | 146 starved, 1 aloft |
| default | 3 | 44 | 12 | 1 | 0 | 0 | 0 | 126 | 6 | 178 starved |
| full (256 herb, 3 colonies of 52) | 1 | 17 | 11 | 12 | 25 | 27 | **5** | 363 | 19 | 534 starved |
| full | 2 | 27 | 0 | 0 | 0 | 0 | 0 | 128 | 3 | 284 starved |
| full | 3 | 24 | 1 | 1 | 0 | 0 | 0 | 159 | 4 | 315 starved |
| two-colony harness bed, to 1M | 1 | — | 0 | 0 | 0 | … | 0 | 4 | 0 | 98 starved |
| two-colony | 2 | — | 23 | — | 0 | … | 0 | 252 | 9 | 346 starved |
| two-colony | 3 | — | 1 | — | 0 | … | 0 | 38 | 3 | 132 starved |

Two of nine runs have any ant alive at the end of a session (3 and 5
animals), none has a colony, and every death in every run is starvation.
The plants are not the casualty: the default box holds 6–18 plants
throughout, and on the beds where the ants die early the stand *recovers*
after them (harness seed 2: 12 plants at 200k with ants, 51–59 from 400k
without; full seed 3: 17 → 33). On one harness seed the stand went to zero
as well and the box was empty from 200k to 1,000,000.

**What this rules out, by measurement.**

- **Not the birth rate.** At 24,000 frames the same beds show 3–35 births
  and the lane's own plan read that as the limiter. At session length a
  colony makes 92–363 children and reaches generation 6–19 before it
  dies. Births are plentiful; the colony still ends.
- **Not predation, not fighting.** `predators=0` on every run above;
  `attacks 0` in every fight account. The only killer is the larder.
- **Not a harness artefact of the two-colony bed.** The default box and the
  full box die the same way; the full box's three colonies die one after
  another, the last at ~280k on seed 1.

**What it does not rule out, and the instruments that would.** Whether the
colony *overgrazes* (eats the stand faster than it regrows while it lives,
which the post-extinction recovery suggests) or *cannot reach* what is
there (round seventeen's note: one colony leaves 98% of a 1024-wide stand
standing and starves at 2 ants because it cannot reach the plants) are
different bugs with different fixes, and this census cannot tell them apart:
`forage_probe` (how far a colony actually forages), `stamp_probe` (what a
child costs against what an ant can bank), and `labnest` (whether the nest
stands and the ants are stuck or dead) are the three that would. Run them
at 300,000 frames, not 24,000, or they will answer the one-minute question
again.
...
is the bar the fix has to clear: **a colony alive at 300,000 frames on p90
of seeds, on the default box and the full box, with the census run at
...
### Z13. Every animal in both games can reach a rest it cannot leave (engine) — **RE-AIMED AND FIXED, round 33, 2026-09-13**
...
**Round 33, 2026-09-13: re-opened as a rest-mechanism defect on the owner's
ruling, then fixed. Not closed as a look problem — the framing above is
wrong and this section is re-aimed rather than appended to.** Owner, verbatim,
overturning his own 2026-09-09 *"rest is the absence of a reason to act"*:
...
**Everything measured above stands; what it means changed.** *"Resting, not
blocked"* is still true about the counters. It is no longer an exoneration,
it is the diagnosis.

**The mechanism, measured rather than named.** §Z13 asked twice for
`outputs[BrainOutput::Move]` printed beside the probe and twice left it for
whoever owns `src/sim/creature.rs`. Built (`CreatureStats::p_move_hist`, one
array index per decision tick): `p_move` is `squash(sum).clamp(0.0, 1.0)`, and
`squash` returns a genuinely *negative* number for a negative sum, so every
degree of "would rather not" lands on the **same exact zero**, from which
`draw.unit_f32() < p_move` can never be true. Over 120,000-frame beds,
`RAYON_NUM_THREADS=1`:

| share of creature decision ticks at `p_move` **exactly 0.0** | |
|---|---|
| long ant, `played_bed_longant`, seeds 1/2/3/7 | **48.7 / 64.0 / 54.6 / 62.5 %** |
| shipped two-cell ant, `played_bed`, seeds 1/2/3 | **77.5 / 66.3 / 54.4 %** |

An animal there is not *unlikely* to move. It **cannot**, and nothing it does
itself changes that — only the world moving one of its other inputs does. That
is the whole of §Z13, and it is `CLAUDE.md`'s first law failing: walking or
frozen, with nothing in between.

**The fix: `brain::BrainInput::Stillness`, an input read off the animal rather
than off the world.** `OrganismState::still_ticks` counts decision ticks since
the body last moved (reset by a step or a launch); `Stillness` is the
**square** of `still_ticks / creature::STILL_SATURATION` (192 ticks, 1,152
frames for an ant), wired `(Stillness, Move, 1.5)` in all eleven shipped
creature species. So a rest **ends** — the animal shifts, the odometer resets,
it settles again. **This does not contradict the 2026-09-09 ruling, it
supplies it**: rest is still the absence of a reason to act, and having stood
in one spot long enough is now one of the reasons.

**Why the ramp is squared, which is the whole of why it does not break
something else.** `creature_tick`'s own comment says *"a laden ant walking
away from the nest scent computes a low `Move`, fails the roll, and
re-orients. That is the whole of the homing mechanism"* — so a low `Move` is
**two different states**: re-orienting (tens of ticks; the declining *is* the
navigation) and resting (thousands of frames; the declining is this bug). A
**linear** ramp over 64 ticks cannot tell them apart and overrode the first to
fix the second: measured that day, `deliveries` **4,908 → 54** on seed 1 and
13,054 → 2,109 on seed 3, with the colony surviving (`alive` 202 → 679) and
simply not provisioning. Squaring puts the term under the noise of a tumbling
animal (+0.016 at 20 ticks, +0.10 at 50) and over the worst standing sum where
it is needed (+0.41 at 100, +0.92 at 150, +1.5 at 192). Recorded in
`dead-ends.md`.

**What it moves, nine seeds against a control that is the same binary.** The
control arm is `labforage wire=Stillness:Move:0.0` — **not `origin/main`**,
because a new input column changes `live_slots()` and therefore every
`random_genome` draw, so main and the branch are different animals and not
comparable per seed (§Z14's problem in another costume). 120,000 frames,
`sample=900`, `RAYON_NUM_THREADS=1`.
...
11,700–59,400 frames → **900–5,400**. The shipped two-cell ant, same treatment
(`played_bed`, all bodies): **45 / 30 / 41 stops → 6 / 7 / 6**.

**And the distribution keeps its middle, which is the claim that matters**
(`CreatureStats::rest_bout_hist`, power-of-two buckets of an animal's *own*
ticks — the instrument `idle_with_room` cannot be, because it samples every
900 frames and scores a walk-away-and-return as standing still):
...
| longest animal still standing when the run stopped | 873–10,039 ticks (5,200–60,200 frames) | 29–2,226 |

**The short pause is untouched and only the tail is cut.** That is the shape
the owner's first law asks for, and it is why this is not "the ants now walk
all the time": an animal still declines to step on **83–94%** of its decisions
(step rate 3.9–16.0% → 5.6–16.8%).

**Per *animal*, which is what settles the owner's own reading and what a rate
cannot.** Shown the pooled 74-76%, he said: *"95% of the creatures on the
screen are not resting and then a few decide to rest for a large portion of
the entire gameplay... It reads some creatures got frozen."* Both that and
"every animal takes brief breaks" produce 75%, so the census was rebuilt per
animal (`labforage`'s `SUMMARY per_animal`). **Neither story is right and his
is far the closer**: the idle fraction spreads across all ten deciles (median
animal 0.30-0.39, p90 0.79-0.85) -- so not a uniform brief break -- while only
18-25% of animals sit under 10% idle -- so not "95% never rest". **What is
exactly as he describes**: of 275 and 557 animals scored on seeds 1 and 3,
**157 and 338 have a run of 4,500+ frames, and 51 and 122 of those are never
once seen elsewhere again** -- 18.5% and 21.9% of every animal on the bed -- having
stood a median of **13,500 and 8,100 frames** and up to **43,200**. The
longest-run figures close the obvious confound: these are animals that were
alive and stationary, not animals that went quiet on the way to dying. **With
the fix, on the same seeds: 0 and 1 animals with such a run, 0 never
returning, and 0 animals above 90% idle.**

And what tells a frozen animal from a busy one at the moment it is frozen --
weighted by readings, so a longer state counts for more: mean energy **434/455
J against 425/400**, and **carrying something 8.1%/14.4% against
34.6%/41.5%**. A frozen animal is slightly richer and much less likely to be
laden, which is the shipped `Move` row read back out of the world
(`(Energy, Move, -1.75)`, `(FoodAdjacent, Move, -1.16)`, and a laden ant
having somewhere to be). The census reached the mechanism independently of the
arithmetic.

**What it costs, stated rather than buried.**

- **Foraging.** Deliveries per 1,000 ant decision ticks fall on **7 of 9**
  seeds, median **2.68 → 1.63**, and steps per delivery roughly double
  (median 32 → 76). *But the two seeds where the control's foraging had
  collapsed outright recover*: 0.02 → 0.71 and 0.20 → 3.15. The arm is worse
  on a healthy colony and better on a failing one. A gentler weight does not
  buy it back — `wire=Stillness:Move:1.0` over the same nine seeds reads a
  *lower* median (1.48) while letting the long tail return (`maxL` 4–12), so
  1.5 is not a compromise that could be softened.
- **Colony size is not moved**: `alive` is up on 4 seeds and down on 5.
  Treat the medians (152 → 99) as the chaos §Z14 documents, not as a finding.
...
- **Frame time: no measurable cost.** `antcost ants=0,900 width=1024
  ...
  main **12.14** and **12.31** µs/ant at 658/650 standing ants; arm **11.22**
  and **12.68** at 610/634. The arm's own two runs bracket main's, so the
  difference is not separable from the box; whole-frame is **~8.0 ms/tick at
  ~650 ants on both**.
- **The mutable genome surface**, which is the constant this change is
  obliged to re-derive: `live_slots` **846 → 870**, so every species'
  `mutation_rate` is re-derived to `3.18 / 870 = 0.0036552` in the same
  change, and every breeding scene's numbers move from birth 1.

**The owner picked the fix.** Card `20260913T172957391Z-dcab18`, read with
`review.py get` rather than off `inbox`: ***"A looks way better"***. Blind, so
the prose and the stored label are in different namespaces:
`blind_was: [1, 0]` means displayed pane 0 carried stored item 1, and stored
item 1 is *"B — rest with an end"*. His "A" is the fix. He left no click, so
the decode is the whole verdict.

**The card as posted** — blind, two
81-frame sequences of `played_bed_longant` seed 3 at his own original crop and
zoom, 4,800 ticks, with the rest-bout counts for the window shown in `meta`.
**Shipped as default** per this round's standing instruction, whatever the
verdict; if he wants it milder the lever is one weight in each species file
and `wire=` sweeps it without a rebuild.

Full account: `Reports/evolution-lab-rest-with-an-end-2026-09-13.md`.
...
### Z19. **A pellet of spoil crosses up to 116 rows with nothing carrying it** — **FIXED 2026-09-13 (lab/engine)**

**Why this letter.** `python3 scripts/bugindex.py --branches` over 69 refs
carrying the register, 2026-09-13: next free in the Z series is **§Z19**. Not
`--check`, which reads one working tree and passes on a letter already live on
an unlanded branch — this section's own ancestor, PR #221's, was filed as
**§Z4**, a letter already used and closed on `main`, and that is most of why
nobody read it for ten days.

**What it is.** `creature::act`'s spoil drop tries the animal's eight
neighbours, and when none will hold a pellet it scans **straight up the
column** for the first cell with two of three filled beneath it and clear air
above, as far as `SPOIL_LIFT = 160` rows — **with no check that a path
exists.** The ant never climbs. This is `CLAUDE.md`'s second law: *there must
be a verb, and it must deliver something*. The verb here delivers a pellet to
a place the animal could not have reached.

**It is not rare.** Measured on `examples/spoil_destination`, twelve seeded
beds with a tree and a 52-ant colony at its foot, `RAYON_NUM_THREADS=1`:
**8–25% of every pellet placed** goes through the up-the-column branch, the
mean lift is **7.8 rows**, and the **median seed's longest lift is 78.5 rows**
against a 320-row world.

**The picture it was found by is gone; the mechanism is not, and that is the
finding.** §Z18 and PR #221 both measured the *standing* consequence — pellets
left **+52/+67/+99/+94** rows above the surface with a tree in the box against
**+4/+3/+2/+2** without. On today's trunk that no longer reproduces: §Z18's
`needs_footing` repair means a pellet posted into a canopy falls out of it
again, and the same census now reads **+2/+4/+3/+2** with a tree against
**+3/+5/+3/+4** without. The heap is gone. **The lift is untouched by that
repair** and still fires at the rates above. A repair that removes a picture
while leaving the mechanism is exactly the case `CLAUDE.md`'s *look again for
what you did not measure* is about, and only a counter could say so — which is
why `CreatureStats::spoil_lift_rows` was added beside `spoil_lifted` /
`spoil_lift_max`: a max alone cannot separate one freak of 107 among ones from
a colony routinely posting pellets fifty rows up.

**The instrument.** `examples/spoil_destination`, ported from **PR #221**
(`claude/creature-plant-pathfinding-rjzkqe`) onto today's APIs — that branch is
**731 commits behind `main`** and was not revived in place. Three arms: a tree
with a colony at its foot, **the same colony with no tree** (the positive
control that makes any height claim admissible — ants mound over their own
nests by design, so the question is only ever whether the tree changes it), and
the tree alone (which **asserts** that worked soil is ant-only rather than
eyeballing it). Left behind are #221's `occlusions` column and `tree+gallery`
arm, which read counters belonging to that branch's *other* half — creatures
walking through living root, measured, left switched off, never landed.

**The fix, and the half #221 did not have.** `creature::lift_reach` stops the
scan where the animal could not have gone, under two clauses:

1. **Could it have cut through?** The species' own `dig_force` against the
   material's `penetration_resistance` — the same test the dig branch makes, so
   *"could I have come this way"* and *"could I have dug this"* cannot drift
   apart, and never a material whitelist. This is #221's rule.
2. **Could it have climbed?** A row of **open air** passes only where something
   solid stands beside it to hold. #221 had no such clause, so a clear column
   was unbounded by it.

**Clause 1 alone fixes the median and leaves the tail, and the tail is the
defect.** Twelve seeds, three modes, **one binary**
(`PIXEL_PHYSICS_SPOIL_LIFT=unbounded|dig|climb`), `RAYON_NUM_THREADS=1`:

| mode | median `lift_max` | p90 | max | mean lift | `lift_max` ≤ 10 | digs | spoil above ground |
|---|---|---|---|---|---|---|---|
| `unbounded` (trunk) | 78.5 | 106 | **116** | 7.81 | 3 of 12 | 3,431 | 452 |
| `dig` (PR #221) | 9.0 | 21 | **102** | 3.77 | 7 of 12 | 3,221 | 418 |
| `climb` (**shipped**) | 6.0 | 7 | **22** | 3.21 | **11 of 12** | 3,222 | **445** |

`climb` dominates `dig` on every column, and the climb clause is **free on top
of the dig bound**: 3,222 digs against 3,221.

**What it costs the dig, which was the risk.** A 160-row jump replaced by
nothing is a colony that cannot dispose of spoil at all. It is not that:
**`dumped` tracks `digs` to within five on every seed in every arm**, so no
pellet is stranded in a mandible and no animal is stopped from digging by
having nowhere to put what it cut. The cost is **~7% fewer digs** (3,431 →
3,222 pooled; median per-seed ratio **0.931**, and 2 of 12 seeds dig *more*).

**And it keeps the towers**, which is the thing §Z18's own candidate fixes kept
costing: worked soil standing **above** the original surface is 452 → **445**,
i.e. unmoved, while `dig` alone takes it to 418. Worked soil touching living
tissue falls 591 → **504**, where `dig` alone makes it *worse* at 753.

**Two honest limits.**

- **The arms are whole runs on a chaotic bed, so they are different worlds
  after the first divergent lift** — per-seed pairs are not comparable and are
  not read as such here (seed 4 is byte-identical between two arms, seed 3
  moves the wrong way). Everything quoted is an **order statistic over twelve
  seeds**, which is what `CLAUDE.md` prescribes for exactly this reason.
- **The climb clause models holding a wall, not a path.** It asks whether
  *something* stands beside each row, not whether a connected route exists from
  the animal to the destination. A staircase of disconnected ledges would still
  pass it. That is deliberate — a real path search per pellet is not affordable
  in the sweep, and the measured tail after this clause is 22 rows, not 116.

**Guard.** `creature::tests::a_spoil_lift_stops_at_what_the_animal_could_not_
dig`, all three modes from one process — which is why `lift_reach` takes the
mode as a **parameter** rather than reading the `OnceLock`: a guard that could
only see the default would cover a third of the rule and read as full cover.
Its arms include the one that proves the climb clause does something (`dig`
runs the full 160 rows up open sky where `climb` stops at 1) and one that
**contradicts #221's own comment**: that branch says *"plant tissue (resistance
100 by default)"* stops the lift, and `leaf.ron` authors **0.1**, well under
the ant's `dig_force` of 1.0. Foliage does not stop an ant and does not stop
this; trunk, root and stone do, at the default 100.

**What is not claimed.** Nothing here is judged by eye, and no card was posted
(owner's ruling 2026-09-13: the review queue is for visual evaluations only).
§Z18's standing warning applies unchanged — **the beds anyone has measured do
not reproduce what the owner is actually looking at**, and this section's bed
is a 52-ant colony under one tree, not the 1000+ long-ant session his report
came from. What is claimed is narrower and does not depend on that: a pellet
was crossing up to 116 rows with nothing carrying it, and now the median seed's
longest lift is 6.
...
### Z20. `labgif wire=` was a silent no-op for every card it has ever produced (lab) — **FIXED, round 33, 2026-09-13**

**What it was.** `examples/labgif.rs` applied its `wire=Input:Output:weight`
genome override to `lab.world.species` *before* `lab.load_scenario(sc)`,
copying the ordering and the comment from `labforage`, where before-founding is
correct because `place_creature` copies the genome at placement.
`Lab::load_scenario` calls `reset()`, whose second line is
`self.world = self.spec.build()` — **a brand-new `World` carrying the compiled
species table**, which discards the override entirely. Every `labgif` card
using `wire=` was therefore the control wearing a label.

**Why the guard did not catch it, and this is the part worth keeping.** The
block carries `assert!(moved > 0, "wire= matched no slot the species did not
already carry; this arm is the control wearing a label")` — written for exactly
this failure, and blind to it: it checks the genome it is about to write, not
the one the run will use. It also *printed* `wire= set 1 of 1 weights on
longant`, so the log affirmed the knob had been set. `CLAUDE.md`'s *a knob
nobody can see the value of is a knob nobody can tell is disconnected*, with
the twist that here you could see it and it still was.

**How it surfaced.** Two 4,800-frame arms of `played_bed_longant` seed 3 for a
review card, one wired and one not, came back with **byte-identical PNGs at
every sampled frame and an identical `COLONY` line** (peak 88, born 105,
STARVED 80) — `CLAUDE.md`'s *identical output across a change that must have
moved something*, which is the only tell there was.

**Fixed** in the same PR as §Z13's repair: the override is read before the load
(it needs `sc.bed.colony_species`, which the load consumes) and **applied
after** it. The colony on these scenarios is founded by the timeline at frame
6,000, i.e. during the advance loop, so after-the-load is both correct and
sufficient. Verified by the same pair of arms now differing at every sampled
frame and at the `COLONY` line (126 ants against 88).

**Who should re-read their numbers.** Any card or arm produced with `labgif
wire=` before 2026-09-13 — `Reports/lanes/evolution-lab-flitter.md`'s
...
> **Footnote, 2026-09-15, by the lane that filed this — the reproduction had
> already stopped reproducing before the fix landed, by a different route.**
> Zeroing the druid preset's three life densities (#412) took `life_scatter`
> to nothing, and **the grow phase grows the scatter's seeds** — so with no
> seeds there was nothing to grow. Measured on `main` at `20628c10`, before
> the ceiling was widened, all three starts: **0 organisms after the grow
> phase, 0 births refused, 12 of 12 placed per stand with ground**, and
> `Start::Dead` printing *"marked 0 of 0 organisms senescent"*.
>
> Kept because the reasoning error will recur, not because the entry is in
> doubt — it is fixed, and the fix is the right one.
> `Reports/lanes/druid-program-coordinator.md` recorded the density change as
> *"relieves §Z21 where the owner meets it while leaving grown and dead
> untouched"*. Grown and dead were **not** untouched; they were relieved
> hardest, because their entire 4,093 organisms came from growing the
> scatter. **The grow phase looks like an independent source of plants and is
> not** — that is the shape worth carrying, and two of us had it backwards at
> once.
...
touched.** A walk gate over colony-dug nests read `54-92%` on 2026-09-13 and
**`0%` on every one of twelve seeds** the next morning, on an unchanged probe.
Nothing in the probe had moved. What had moved was `assets/species/ant.ron`.

**Mechanism, and it is a set-membership failure rather than a colony bug.**
`World::plant_ant` places through `Origin::Founder { colony: None }`, which
claims a **fresh label per call** — so a loop of it builds one-ant colonies
that only *look* like a colony. That was inert for as long as `scent_spread`
sat at 0 and every label smelled identical. `813fb890` (PR #423) authored a
live `scent_spread` on the shipped ant, and from that commit those labels are
mutual strangers — and **a stranger is food to the ordinary mouth before it is
ever a target for the fight verb**. They spend the run eating each other
instead of digging. `CLAUDE.md`'s *adding a member to a set enrols it in every
rule over that set, silently*, with the set being "every harness that loops
`plant_ant`" and the member being one asset default.

**Measured on `examples/burrow_probe arms=colony seeds=12 box=2x3`**, one
binary apart from the placement itself, frame 8,000, all twelve seeds:

| | strangers | one colony |
|---|---|---|
| `roofed` void | **5 - 9** cells | **22 - 60** |
| `lgroof` (roof over the largest run) | **0** on every seed | — |
| `digs` | 68 | **344 - 415** |
| walk, 2x3 body | **0% on 12 of 12** | 54 / p50 83 / 92 |

`lgroof 0` is the tell that names the failure in this file's own vocabulary: the
largest void run had no roof at all, which is its definition of **a quarried
face rather than a gallery**. The colony was not building a smaller nest, it was
not building one.

**Two controls, because a number that moves is not yet a finding.**
`examples/cave_probe box=2x3` reads **99% median** over 16 seeds x 8 presets, so
the morphological opening is sound and the zero is the scene; and `box=7x14`
(the real gnome against ant-dug galleries) reads 0% everywhere, which is the
sensitivity arm that must stay zero.

**`examples/ascii.rs` already paid for this once and the repair was not
ported.** `21bea815` fixed three of its scenes the same day (`digs 62 -> 354`,
roofed `0 -> 42` on the excavation scene) and its moisture scene had ruled on it
earlier still. The repair is four lines — the first ant founds the label, every
later one joins it:

```rust
let mut colony = None;
for &(x, y) in slots {
    if let Some(site) = creature::plant_creature_seed_in(&mut world, x, y, "ant", colony) {
        colony = colony.or_else(|| creature::colony_of_site(&world, &site));
        world.schedule_active_site(site);
    }
}
```

**Still carrying it** (`burrow_probe` is repaired; these are not):
`scale_probe.rs:597`, `forage_probe.rs:326`, `ant_ablation.rs:444`,
`creature_space.rs:590`, `stamp_probe.rs:164` and `:177`, `filmstrip.rs:1754`,
`field_sense_probe.rs:286-287`, `creature_probe.rs:399` and `:419`,
`spoil_curvature.rs:288`. **Any colony number taken from one of these since
2026-09-14 is a measurement of a brawl**, so check the date on a quoted
baseline before trusting it rather than assuming the file is fine.

**What this is NOT.** The game is unaffected: the sandbox's ant key goes
through `World::found_colony`, which claims one label, and `src/app.rs:5434` is
a single-ant test. The lab is unaffected *by this mechanism* — `labnest` builds
through `LabBox { colonies: 1 }` and has no loop.

**One loose end, deliberately not chased here because it is the lab's.**
`labnest founders=0 seeds=2` on `bed311d0` reads `roofed` **45-58** against the
230-260 this register's own instruments row publishes, with `packed` 488-492,
`digs` 502-563 and the documented die-off unchanged in shape (52 -> 8 by frame
5,000, against a recorded 52 -> 12). No commit has touched `examples/labnest.rs`
since that baseline was taken on 2026-09-06, so the instrument is the same one.
That is **not** explained by the stranger defect and wants a look from whoever
owns the bed; it is recorded as an observation with its command rather than
filed as a second bug, because the instruments row quotes two different lining
...
alone"* (its own doc). Measured, it reaches the animals already touching the
fight and nobody else.

**Reproduction**, no world needed — `cargo run --release --example pherolife
-- mode=alarm`. The alarm is read at the cell the animal **stands on**
(`creature::sense`, and the 8/38/28-against-296/258/266 measurement behind
that is sound), so the row that matters is `d=1`: the ant *beside* the fight.
`d=0` is the victim.
...
0.024, contributing **+0.047** to `Attack` against `ant.ron`'s authored
weight of 2.0. **Two cells away: exactly zero, ever.**

One display (`contest::DISPLAY_DEPOSIT`, 40 — the site that file's own doc
flags as *"this verb has not been measured"*): a neighbour one cell away
hears **zero, ever**. The verb writes into a plane nobody but the displaying
animal can read.

**Ruled out — it is not a too-small scene.** A single-bite reproduction and a
mechanism that is merely quiet look identical, so the sustained arm is in the
same command: a wound every 6 frames (`ant.ron`'s `tick_interval`) over a
2-cell body, 40 bites, saturating the victim's own cells at 157.
...
Even sustained: **two cells off reads 15/255 = 0.059 -> +0.118 into Attack,
and four cells off reads zero for the entire fight.**

**Ruled out — it is not the here-read.** `creature::sense` already names the
directionlessness of a here-read as a cost and calls it *"a limitation of one
slot, not of the plane"*, pointing at the trail planes for distance
recruitment. **It is the plane.** At `ALARM_RHO = 0.25` the signal is ground
down faster than `DIFFUSE` spreads it, so there is no distance for a
directional slot to read even if one were added. Adding a bearing slot
without moving the constants would measure as another null.

**Not fixed here, deliberately.** `ALARM_RHO`'s own doc calls itself *"a dial
on the parameters page rather than a tuned constant -- what the box wants has
not been measured"*, and the standing direction is to expose rather than
balance. Both halves are now dialable: `set_alarm_rho` already existed and
`Pheromones::set_channel_diffuse` landed with this measurement. **What a fix
wants is a seed sweep gating an order statistic**, not an A/B — the two
constants trade reach against the alarm staying an *event* rather than
...
[`lanes/evolution-lab-pheromones.md`](lanes/evolution-lab-pheromones.md).
Full account of both:
[`pheromone-lifetime-and-wiring-2026-09-14.md`](pheromone-lifetime-and-wiring-2026-09-14.md).

---

### Z26. `review.py inbox` shows every lane's verdicts as yours in a cloud session, and `--mark-seen` marks them all (engine/tooling) — **OPEN**

**What it is.** `_is_mine` (`scripts/review.py`) matches a card on branch
**or** worktree **or** agent name. Every cloud session clones to the same
path, so the worktree clause is true for every card in the queue and the `or`
makes it sufficient on its own. **`inbox` is `inbox --all` wearing the name of
a filter**, and because the widening is not the "cannot identify this agent"
path, it prints no warning.

**Measured 2026-09-15**, from a cloud container on
`claude/thicket-founding`:

| | |
|---|---|
| answered cards `inbox` returned | **14**, none of them this lane's |
| cards in the queue carrying `worktree=/home/user/Pixel_Physics` | **472 of 472** |
| this session's own `origin_info()` worktree | `/home/user/Pixel_Physics` |
| warning printed | **none** |

`--mark-seen` then marked all fourteen. The cards belonged to the druid
screen, bubble-aura, lab and pond lanes.

**Why it cost nothing this time, and why that is luck rather than design.**
`sync_now` exchanges `("cards", "media", "responses")` and **not `seen/`**, so
the markers stayed in a container that was about to be destroyed. Two things
turn this live: adding `seen/` to that list (it is the obvious "why does my
inbox keep showing me answered cards" fix, and it is one line), or running on
a machine where worktree paths actually differ — which is the owner's own
machine, where the queue is served.

**The consequence if either happens.** A lane runs `inbox --mark-seen`,
reads its own answers, and silently consumes every other lane's. The next
session to ask gets an empty inbox, which the skill's own documentation warns
"reads exactly like *nothing was posted*" — the failure the tool exists to
prevent, reached through the tool.

**What it is not.** Not the documented degraded path. That one is deliberate
and *does* announce itself (*"cannot identify this agent... showing every
unseen answer, not just yours"*). This is the opposite: the filter believes
it has identified you.

**Remedies, cheapest first, none measured.**

1. **Set `PIXEL_PHYSICS_REVIEW_AGENT` per session.** Already supported, needs
   no code, and `origin_info()` already treats an explicit agent name as the
   only real identity. Does not fix the default.
2. **Make worktree a tie-breaker rather than a sufficient clause** — require
   branch to match, or worktree *and* something else. The docstring's reason
   for the `or` ("an agent may post from a worktree and read back from the
   main checkout after merging") is a same-machine assumption that does not
   survive every session sharing one path.
3. **Scope `seen/` by reader** rather than one file per card. It is already
   written with a `by` field that nothing reads.

**Do not "fix" this by syncing `seen/`** until 2 is done; that converts a
contained bug into a shared one.

---

### Z27. Heat cannot cross a shallow gradient into ground already at ambient, and the fix that exists defends the other direction (engine) — **OPEN but UNREACHABLE today, found 2026-09-15 by survey, not by play**

`fire::diffuse_heat` pulls a cell toward its four-neighbour average and
stores the result in an `i16` of **whole degrees**. At a typical
`heat_conductivity` of 0.1 a 4 °C difference is a 0.4 °C pull, which rounds
straight back. The function already knows this — its own comment calls it
*"a genuine numerical fixed point"* — and forces one degree of movement
whenever the raw pull is real but rounds away.

**But that nudge is gated on `!already_settled`, and `already_settled` is
measured against `AMBIENT_TEMPERATURE` rather than against the cell's own
local equilibrium.** So it is switched off exactly where a cell sits at
ambient, which is the *receiving* end of every shallow gradient. It rescues
a hot cell cooling down; it cannot rescue an at-ambient cell warming up.

Measured in the sweep (`examples/quantgrad frames=500`), 24 ash cells at
exact ambient beside a +4 °C block of the same material: **0 of 24 warmed at
all**, over 500 frames of `parallel::step`. Exactly zero, at several sample
points, and it keeps reading it.

**Both arms of the representation bar are met.** The same rule and the same
gradient at a finer quantum, changing nothing else: at 24 °C and
conductivity 0.02 the shipped width reads **0.00 °C** and a tenth-degree
width reads the whole **4.00 °C**. The flatness is representational, not
physical.

**Why it is filed rather than fixed, and it is all three:**

1. **Unreachable today.** The gap needs a cell-level heat source under
   ~5 °C above ambient and there is not one: every `burn_temperature` is
   320–900 °C, the only `intrinsic_temperature` is lava at 1000 °C, a corpse
   inherits exact ambient, and beside a 500 °C fire the neighbour average is
   ~140 °C — rounding never kills that and the gate never binds.
2. **The gate is deliberate and load-bearing.** Its comment records what it
   prevents: without it *"a connected mass of many cooling cells"* nudges
   itself awake for ever and **no chunk ever sleeps** — caught originally by
   a test with 40 connected ash cells. Removing it trades a latent defect
   for a live frame-cost one.
3. **Re-scaling is not scoped by one constant.** Tenth-degrees touches
   `AMBIENT_TEMPERATURE`, every material's `burn_temperature` and
   phase-change points, `HEAT_GLOW_RANGE`, weather's cold values and
   explosion glow. Range is *not* the obstacle — lava at 1000 °C is 10,000
   quanta, well inside `i16`.

***Re-test when:*** **any cell-level heat source lands that sits within a
few degrees of ambient** — body heat, solar warming of surfaces, geothermal,
a warm spring. That single change makes this live with nothing else moving,
and the creature and weather lines are both plausible authors of it. Also
re-test if `THERMAL_SETTLE_EPSILON` is ever re-derived against local
equilibrium instead of ambient, which would close it as a side effect.

Full account, the survey that found it, and why every other narrow per-cell
scalar in the engine is a null:
[`decaying-gradient-quantization-2026-09-15.md`](decaying-gradient-quantization-2026-09-15.md).

---

### Z28. The moisture deposition preference was deleted rather than moved, and `DropSpoil` has no headroom to carry it back (engine/creatures) — **OPEN, found 2026-09-19 by CI**

**What a player loses.** `wiki/ants.md`: *"An ant carrying something is a
little more likely to put it down where the ground is drying out unevenly."*
That is `(MoistureGrad, Drop, 0.169)`, and as the ant ships today it reaches
**no verb at all**. The page describes a preference the animal does not have.

**How it went.** The commit that took the two terrain wires off `Drop` —
correctly, see below — recorded *"BOTH STAY ON `DropSpoil`, where the
construction argument they were written for actually holds."* Only one of
them did. `DropSpoil` on `main` carried `(AtNest, 0.9)`, `(Carrying, 0.2)`
and `(SurfaceCurvature, 0.169)`, and **no moisture term to stay on**, so
moving curvature was a no-op and moving moisture deleted it from the species.

Nothing could catch it. `examples/ascii.rs`'s deposition scene is the only
gate over this preference and it was reading `CreatureStats::drops`, the
*forager's* counter — so it went red for the right reason wearing the wrong
message (`no ant ever dropped anything`), and the obvious repair, putting the
wire back on `Drop`, is the one thing that must not happen.

**Why it must not go back on `Drop`.** `trailfollow mode=gap gaps=90
seeds=18 gate=shipped homebias=1`, deliveries on the `hand` arm:

| `Drop` wiring | deliveries |
|---|---|
| no terrain terms (shipped) | **511** |
| `(MoistureGrad, Drop, 0.169)` restored | 180 |
| moisture and curvature both restored | 113 |

Any away-from-nest drop probability is a laden ant putting its dinner down
short of home, and the longer the journey the likelier it is. The foraging
loop closes *because* `(AtNest, Drop, 1.0889)` is the only trigger left.

**Why putting it on `DropSpoil` is not the one-line fix either, measured.**
`DropSpoil` has **no `Bias` wire**, so a pellet in the mandibles alone puts
the urge at `squash(0.2 + terrain)`: the ant puts it down within a few ticks
of cutting it, and a site preference on a verb that fires at the face cannot
select a site. Adding `(MoistureGrad, DropSpoil, 0.169)`, same bed, same
seed, the wire the only difference:

| | matched ratio | sign test |
|---|---|---|
| without the wire | 1.30x | 7 of 22 frames (31.8%) |
| with the wire | 0.92x | 10 of 22 (45.5%) |

Both ratios sit either side of 1.0 and both sign tests either side of half,
in no consistent direction — a null, exactly as a verb with no headroom
predicts. And the wire is not free elsewhere: the same 18 seeds put
deliveries on the `hand` arm at **511 → 121**, because spoil decisions
perturb the bed the foraging result is measured on, and
`sim::creature::tests::a_share_is_booked_on_both_sides` **fails** with the
wire in and passes without it. One line into a species file, three separate
things move — which is the shape of an unpriced lever, not of a fix.

**Why it is filed rather than fixed.** The repair is the wire *and* a
`(Bias, DropSpoil, ~-0.2)` for it to be spent against, which is a genome
re-derivation, not a line: it changes where every colony in both games puts
its tailings, and `ascii`'s excavation scene calibrates standing-bank,
roofed-void and packed-wall counts against the present behaviour. Landing it
inside the foraging PR would have put an unmeasured constant under the
headline that PR exists to make.

***Re-test when:*** anyone gives `DropSpoil` a bias, or re-derives the spoil
economy for any other reason — at that point the wire is one line and the
scene below already measures it. The deposition scene keeps the bed, the
gradient and the matched-pair instrument alive for exactly that: it prints
the moisture columns unasserted, so the day the preference comes back the
readout is already there and the ablation (`PIXEL_PHYSICS_DROP_TERRAIN`) is
already wired.

---

### Z29. An ant stands on its own freshest deposit, so the homing gradient reads "home is behind me" whichever way it faces (engine/creatures) — **OPEN, found 2026-09-19**

**The one-line version.** `step` deposits channel A at the ant's own head cell
after a successful move; `sense` then reads that same cell as `here` in
`along = (ahead − here) / (ahead + here + guard)`. The cell underfoot is
therefore the freshest thing in the neighbourhood and `along` comes out
**negative in every direction**. The animal's own trail blinds its own homing
sensor.

**Measured, laden ants only** (the homing pair is gated `Bias -45,
CarryingFood +45.5`, so it is shut for an empty ant and only the laden
population can show this). Cohort traces, gap 90, `hand` arm:
...
Facing the nest yields a homeward reading on **1–8% of laden ticks**. Facing
away yields "away is downhill" most of the time. The reading is negative
regardless of heading, which is what a self-deposit predicts and what a real
gradient cannot produce.

**Reconstructing the two samples confirms it.** From `along` and
`PheroAFront`, `here = [ahead(1−along) − guard·along] / (1 + along)`. Median
`ahead` is **0.0** against a median `here` of **105 and 457** on the two arms
carrying signal, with `here > ahead` on **75–87%** of laden ticks.

**Why it survived this long.** It is invisible to every aggregate. `|along|`
looks healthy — median 0.23–0.53 against the 0.486 that `ant.ron`'s own design
note says delivers its intended ±3.75 swing on `Move` — so a magnitude check
passes. What is broken is the *correlation between heading and sign*, and no
census of the plane, the reading, or the gate can see that.

**It is not §Z7.** That was the gate parking the unit on the flat of `squash`,
and its homing half was fixed 2026-09-09 — the pair now sits at +0.5 when open,
on the slope. This is upstream of the gate: the *input* is systematically
negative before any gate sees it.

**What it explains.** §7.43 made the plane four times more readable (`READ`
13.7% → 54.6%) and produced **no extra round trips** (47 → 48). A bigger signal
that is negative whichever way the ant faces is a bigger useless signal. It
also explains why the measured ratchet is +0.04 to +0.11 on `P(move)` where the
species file describes 0.84 against 0.

***Re-test / fix candidates, none measured:*** deposit at the cell just
**vacated** rather than the head, which is also the physically natural reading
of laying a trail as you walk; or read `here` from the plane **before** this
tick's deposit; or compare two forward samples (`so` and `2·so`) so neither
term carries the animal's own mark. The first is a one-line change to the
deposit site and is the cheapest to try. Note P-11 (deposit only on a
...
`examples/filmstrip.rs`'s `advance()` calls `parallel::step` (or
`update::step`), `step_liquid_bodies`, `rigid::step_chunk_bodies` and the
gnome, and **never `world.step_pheromones()`**. It does not route through
`src/sim/frame.rs` either, which is where the real app makes that call
(`frame.rs:143`). So on every `filmstrip` scene, deposits accumulate into
the planes and nothing ever runs a pass over them: no decay, no diffusion,
no sleep, no `PheromoneStats::passes`.

**Reproduction, and it is the cheapest possible one.** `scene=colony` founds
21 ants and runs 28,000 frames; the whole of channel A's behaviour is a
constant the plane is built with. Vary it across its entire range and the
output does not move:

```
PIXEL_PHYSICS_A_RHO=0   filmstrip scene=colony seed=3 start=4000 every=8000 count=4 channel=pheromone_a
PIXEL_PHYSICS_A_RHO=0.9 filmstrip scene=colony seed=3 ...   # erases 90% of the plane per pass
```

Both write a **byte-identical PNG** (`md5 ce68886b...`) and print identical
creature counters (`moves 953 blocked 43 falls 37 | pickups 123 drops 2
deliveries 2 deaths 23`, `forage trips 1`, `impulses 1824`). The same switch
on `examples/ascii scene=ants` moves pheromone tiles per pass **17.4 → 13.5**
and live organisms 337 → 364, so the switch works and the scene does not
reach it. This is `CLAUDE.md`'s own tell — identical output across a change
that must have moved something — and what it found this time was not a stale
binary but a missing call.

**What it invalidates.** Anything judged by eye off a `filmstrip` scene
containing creatures, where the claim was about trails: recruitment, trail
following, homing, `channel=pheromone_a` / `pheromone_b` overlays. A plane
that only ever accumulates saturates where ants dwell and holds every mark a
colony has ever made, which is not a faint trail and not a strong one — it is
a different mechanism. `scene=colony`'s `forage trips 1 (bar 8)` has been read
as a foraging result and cannot be one.

It does **not** invalidate the measurements in
`pheromone-trail-direction-2026-09-16.md`: those come from
`examples/trailfollow.rs`, `ascii` and the probes, all of which call
`step_pheromones` in their own loops. It is why §7.46's change could not be
shown on a contact sheet.

**The fix is one line** — `world.step_pheromones()` in `advance`, which gates
itself on `PHEROMONE_INTERVAL` so calling it every frame is correct. It is
filed rather than pushed because `examples/filmstrip.rs` is the fifth most
contested file in the repo (99 landings) with **8 branches holding unlanded
commits in it**, and the fix changes the output of every scene that has a
creature in it — including the ones other lanes are mid-way through judging.
...
### Z31. `field::step` carries derived arrays forward over a settled chunk that still holds an un-taken stale mark, so a write reaches the field's occupancy a frame late (engine) — **OPEN, found 2026-09-19 by a switch that was not looking for it**

**What is wrong.** `field::step`'s carry decision asks
`Chunk::is_settled()`, on the premise its own comment states:

> Sound because `Chunk::is_settled` is exactly "nothing to sweep", and its own
> doc guarantees the direction that matters here: a chunk cannot go from settled
> to awake without a write, and every write marks it dirty through `set_world`.
> So a settled chunk's occupancy is unchanged since the tile above it was last
> solved — and it *was* solved, because an awake chunk always seeds its own tile
> into `awake` above.

**That premise measurably does not hold.** A settled chunk can hold a live
`stale_blocks` mark, and when it does, `inherit_derived` serves the five
CA-derived arrays (`blocked`, `transmission`, `moisture_source`, `glow`, `beam`)
from before the write, for one frame, until the next `end_sweep` promotion wakes
the chunk and the mask is finally taken.

**The reproduction, and it needs no creatures.** `FIELD_CARRY_STALE=1` adds
`&& !c.has_stale_blocks()` to that decision and changes nothing else. On a bed
with **zero** creatures in it:

```
RAYON_NUM_THREADS=4 ./target/release/examples/lab_cost colonies=0 frames=600 every=600 phases=1
```

| arm | field hash |
|---|---|
| unset (shipped) | `0x77761fab0c35bfce` |
| `FIELD_CARRY_STALE=1` | `0xd67e172d6d16b8a4` |

So the case arises routinely in an ordinary lab box, not in a corner. Rescanning
is the ground truth by construction — `rebuild_blocked` re-reads the CA cells —
so the shipped arm is the stale one.

**What has been ruled out, by ablation rather than by argument.** Both obvious
candidates for "what sets a mark on a chunk that then reads settled" are not it:

- **the soil-moisture phase's placement.** `PIXEL_PHYSICS_MOISTURE=sweep` in
  both arms — the pass folded back inside the CA sweep — still diverges.
- **`Chunk::mark_moist_dirty`'s own stale mark**, which is the one channel
  documented to mark a chunk the ordinary channel does not. Ablated in both arms
  (a throwaway edit, not landed), still diverges.

**The mechanism that survives is the ordering of `end_sweep` against a
cross-chunk write**, and it is consistent with everything above. A write into
chunk C from *another* chunk's sweep lands in C's `pending_dirty` and in its
`stale_blocks`, but `dirty` is only promoted from `pending_dirty` at C's **own**
`end_sweep` — which, for a C swept earlier in the pass, has already run. So at
`field::step` C reads settled while holding a live mark. Stated as a mechanism
rather than as a measurement: it has not been confirmed by instrumenting the
writer, and that is the next step for whoever takes this.

**Why this is filed and not fixed.** It was found inside
`FIELD_CREATURE_WAKE` (`Reports/ant-field-wake-2026-09-19.md` §3), where the
same term was needed for an unrelated reason, and it moved the field hash on a
bed with no ants in it — which is not a thing a creature switch may do. Landing
it there would have made every number in that report a measurement of two
changes. And on its own merits it wants a *decision* more than it wants code:
the field is a coarse ambient channel at one cell per sixteen, and a one-frame
lag in its occupancy map may be entirely immaterial. What is not immaterial is
that `step_pressure` carries a comment worrying about exactly this hazard from
the other end — *"checking `old` here would be reading last step's occupancy for
a decision this step is making, one frame stale exactly when a CA solid has just
moved. Caught by review, not by any test"* — so the engine has already decided
once that a one-frame-stale occupancy is worth avoiding in the pass next door.

**What is *not* established.** How large the error is in the field's own terms
(the reproduction is a hash, not a magnitude — `lab_cost fielddump=` would give
per-channel deltas the way `ant-field-wake` §2.3 does for the other switch);
how many chunks are affected per frame; and whether any consumer decision
(`fire::try_ignite`'s wetness gate, a brain's `LightHere`) ever changes because
of it. The world hash was not compared across this switch and should be — if it
moves, a decision is changing and the "immaterial" reading is dead.

***Re-test when:*** anyone touches the carry decision, `Chunk::end_sweep`'s
promotion, or `parallel.rs`'s remote-write replay. The switch stays in
