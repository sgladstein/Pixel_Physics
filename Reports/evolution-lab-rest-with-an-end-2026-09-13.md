...
## 4b. Per *animal*, because the pooled rate cannot settle what the owner is describing

**The coordinator relayed a correction mid-build**, from the owner on being
shown the 74-76% idle rate:

> ***"It seems to me though that 95% of the creatures on the screen are not
> resting and then a few decide to rest for a large portion of the entire
> gameplay. It doesn't read like a brief break that all creatures do. It reads
> some creatures got frozen."***

**He is right that a rate cannot tell those apart**, and the correction is
`CLAUDE.md`'s *a mean over events is not a mean over the thing you care about*
one level up: "every animal rests three-quarters of the time in short bouts"
and "most never rest while a quarter are frozen" both produce 75%. So the
census was rebuilt **per animal** (`labforage`'s `SUMMARY per_animal`).

**The answer is neither story, and his is much the closer of the two.**
120,000 frames, `sample=900`, animals with at least ten readings scored:

| `played_bed_longant` | seed 1 control | seed 3 control | seed 1 fix | seed 3 fix |
|---|---|---|---|---|
| animals scored | 275 | 557 | 190 | 602 |
| idle-fraction deciles | 70 40 25 20 15 26 24 21 20 **14** | 103 45 69 65 59 68 48 49 33 **18** | 177 10 3 0 0 0 0 0 0 **0** | 494 98 10 0 0 0 0 0 0 **0** |
| median animal's idle fraction | 0.302 | 0.394 | 0.026 | 0.043 |
| animals over 90% idle | 14 | 18 | **0** | **0** |
| animals with a run of 5+ stops (4,500 frames) | 157 | 338 | **0** | **1** |
| ...**of those, never seen moving again** | **51** | **122** | **0** | **0** |
| ...their median longest run | **15 stops = 13,500 frames** | 9 stops = 8,100 | — | — |
| ...their longest | **48 stops = 43,200 frames** | 35 = 31,500 | — | — |

- **Not "a brief break that all creatures do."** The idle fraction spreads
  across all ten deciles, p90 0.79–0.85, and 56–61% of animals have at least
  one 4,500-frame stand.
- **Not "95% never rest" either** — only 25–18% sit under 10% idle.
- **But the freezing he describes is real, and this is the measurement of
  it**: **18.5% and 21.9% of all scored animals go quiet and are never once seen
  elsewhere again**, having stood a median of 8,100–13,500 frames and up to
  **43,200**. That is a latch, and it is the mechanism §1 names — a rest with
  no exit the animal can reach.

**The obvious confound is closed.** "Never seen moving again" would also
describe an animal that went quiet on its way to dying, which is why the
longest-run rows are there: an animal whose *best* run was 15 stops was alive
and stationary for 13,500 frames, not briefly quiet before a death.

**And what distinguishes a frozen animal from a busy one at the moment it is
frozen** — the fourth thing the correction asked for, weighted by readings so
a state that lasts longer counts for more:

| | deep-idle | moving |
|---|---|---|
| mean energy | 434 / 455 J | 425 / 400 J |
| **carrying something** | **8.1% / 14.4%** | **34.6% / 41.5%** |

A frozen animal is **slightly richer and much less likely to be laden**, which
is the shipped `Move` row read back out of the world: `(Energy, Move, -1.75)`
and `(FoodAdjacent, Move, -1.16)` are what drive the sum negative, and a laden
ant has somewhere to be. The census arrived at the mechanism independently of
the arithmetic.

**On population.** The correction warned that a small bed might contain no
frozen animals and read as a clean null. It does not: the phenomenon is at
18.5–21.9% of 275–602 *distinct* animals per run, so the sample is far past what
an 18% effect needs. A wide bare bed (`width=1536 founders=24 colonies=8`,
three seeds) reproduces it — 32/8/7 animals with a long run and 9/1/2 never
returning, against **0** long runs in the fix arm on all three. Nobody here
can build the owner's thousand-ant bed (round 31), so **frame cost** is the
thing that had to be measured at population and was: `antcost` at ~650
standing ants, §5.

**A flag, not a finding, recorded because reporting only the favourable arm is
the trap `CLAUDE.md` names.** On that wide bare bed `alive` at 120,000 frames
went 141 → 18 on seed 1, and then 17 → 23 and 22 → 104 on seeds 2 and 3. Same
coin flip as the played bed; one alarming seed is not a result.
...
## 7. The owner's verdict

**He picked the fix.** Card `20260913T172957391Z-dcab18`, read with
`review.py get` rather than off `inbox`:

> ***"A looks way better"***

**Decoded, because the card was blind and the prose and the stored label are
in different namespaces** (`.claude/skills/review/SKILL.md`): the response
carries `blind_was: [1, 0]`, so **displayed pane 0 showed stored item 1**.
Stored item 0 is *"A — today's trunk"* and stored item 1 is *"B — rest with an
end"*. His **"A"** is therefore the **fix**. He left no click (`choice` is
null), so the prose is the whole verdict and the decode is the only thing that
resolves it.
