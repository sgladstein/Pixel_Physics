...
| [6. Found: a thrown rock lands claiming to be bedrock](#6-found-a-thrown-rock-lands-claiming-to-be-bedrock) | 488 | 11,102 |
| [7. The cheapest experiment that would falsify each](#7-the-cheapest-experiment-that-would-falsify-each) | 1316 | 700 |
| [8. Still unmeasured](#8-still-unmeasured) | 1359 | 367 |
| [9. Recommendation](#9-recommendation) | 1383 | 539 |
...
#### It does not fix the small-world residual, and that is a third source

§6.5c flagged the 2048x1280 row as "a warning, not noise" and guessed the
same thing might be behind it as behind the hammer. **Measured, and it is
not.** Same pick load, `RECONVERGE_AT=1599`:

| world | all three bugs | landing fix only | + footing root |
|---|---|---|---|
| 2048x1280 | 51,499 (`pending` 53,063) | 48,342 (43,067) | **49,113 (44,078)** |
| 4096x1600 | 26,624 (27,397) | — | **216 (2,662)** |
| 8192x2560 | 4,490 (10,033) | 3 | **3 (5,486)** |

At 4096 and above the two fixes together take the residual to essentially
nothing. At 2048 the footing root moves it by **1.6% in the wrong
direction** — one run each, so the honest reading is *not distinguishable
from unchanged* rather than a regression, but it is certainly not the 99%
collapse the larger worlds show.

So there is a **third source**, and neither of this report's fixes touches
it. Two things are worth carrying into whoever picks it up:

- **A confound is still in the way and has to be removed first.**
  `scale_probe`'s mine cuts a band at `w/2 ± 192` — a *fixed* 378 cells
  regardless of world width, so at 2048 it spans 18% of the world against
  4.6% at 8192. The small arm is proportionally four times more damaged, and
  the wrong-cell counts say so directly: **2.28% of body cells** at 2048
  against ~0.00% at 8192. Matching proportional damage rather than absolute
  is the first measurement, not a new trap.
- **It is not the false-anchor family.** Both fixes here remove writers of a
  too-low distance, and both are demonstrably working at 4096. Whatever is
  left at 2048 survives having those removed.

This is recorded rather than chased because §S's own claim — every
destructive verb leaving the scheduler pinned at its cap — is about the
shipped world size and is answered there. `pending` lands at idle for all
three verbs at 8192x2560. The 2048 residual is a different question wearing
the same numbers.
