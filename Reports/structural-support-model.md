...
**Contents** — 9 sections, ~12,471 tokens. Read a section, not the file: the line number is there so you can `sed -n 'START,ENDp'` it, and the token column is there so that is a priced decision rather than a guess. Regenerated by `scripts/reporttoc.py`; a merge conflict inside this block is never hand-merged — take either side whole and re-run.

| Section | Line | ~tokens |
|---|---|---|
| [1. What a replacement has to reproduce, stated exactly](#1-what-a-replacement-has-to-reproduce-stated-exactly) | 78 | 1,154 |
| [2. The instrument](#2-the-instrument) | 166 | 170 |
| [3. Candidate A — the short-horizon saturating gradient: **falsified**](#3-candidate-a--the-short-horizon-saturating-gradient-falsified) | 184 | 718 |
| [4. Candidate B — a hierarchical potential over the coarse layer](#4-candidate-b--a-hierarchical-potential-over-the-coarse-layer) | 246 | 1,702 |
| [5. Candidate C — take away the reactive path's authority to climb: **tried, and it is a trade, not a fix**](#5-candidate-c--take-away-the-reactive-paths-authority-to-climb-tried-and-it-is-a-trade-not-a-fix) | 378 | 1,346 |
| [6. Found: a thrown rock lands claiming to be bedrock](#6-found-a-thrown-rock-lands-claiming-to-be-bedrock) | 482 | 4,484 |
| [7. The cheapest experiment that would falsify each](#7-the-cheapest-experiment-that-would-falsify-each) | 818 | 700 |
| [8. Still unmeasured](#8-still-unmeasured) | 861 | 367 |
| [9. Recommendation](#9-recommendation) | 885 | 539 |
