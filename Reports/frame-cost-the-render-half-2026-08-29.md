...
3. **Rain is a second, separate ~8 ms** of a redraw *and* forces a full
   ...
   Worth re-sizing against §7's redraw, which is a sixth of the one this was
   measured beside.
...
## 7. The other half of the same cost: it did not need to run at all

*Added by the render lane, which reached §5's cause independently and from
the other end. The splat fix above is this report's; what follows is what a
second pass found beside it, and the two compose.*

§5 makes each rebuild ~6x cheaper. This makes it **not happen**.
...
repaint every pixel"* (`App::draw`), and it is true whenever **the cursor is
over the window**, because the brush outline follows it. That is the "on
every forced full redraw" in §5's own sentence, and it is not a property of
the world at all. The splat reads `Material::glow` off cells and nothing
else — its own doc comment says so — which is exactly what the condition's
other two terms ask.

**This is the second correction to this same condition.** `open-bugs-handoff.md`
§0c records keying it on `glow_unsettled` rebuilding on every draw, found and
fixed; `force_full` was doing the identical thing through a different term of
the same `if`.

`force_full` was covering one thing by accident: `App::reset` builds a new
`World` and keeps the `Renderer`, so a new world with the same emitter tile
coordinates kept the old splat. It covered that **only when the cursor
happened to be over the window**, so it was never a guard. That is now
`Renderer::forget_world`, called from `reset` — a correctness gain rather
than a performance one.

With both halves, on the shipped world:

| | redraw |
|---|---|
| before either fix | 39.6 ms |
| §5's splat fix alone | ~7.5 ms |
| **both** | **2.4 ms** |

and the rebuild happens on a frame where a crystal was exposed or mined
instead of on every frame of play. The viewport scaling is the independent
check that the fixed cost is gone: 1.00x / 1.96x / 3.08x at 1x / 2.25x / 4x
the pixels, where before the fix it read 1.00x / 1.06x / 1.13x — the
fingerprint of a cost that does not depend on pixels at all.

### The instrument: `PIXEL_PHYSICS_DRAW_TIMING=1`

§2's hole had a second edge to it. Nothing counted the renderer *as a whole*,
and nothing could attribute it *internally* either, so the cause in §5 had to
be found by bisecting worldgen parameters. `Renderer::draw` now splits itself
by phase behind an env var, and names it in one run:

```
draw rebuild_horizon:          0.71 ms
draw sky_light:                1.69 ms
draw glow scan + near_glow:   37.33 ms      <- of which the field-tile scan: 0.04 ms
draw pixels:                   1.91 ms
draw overlays + particles:     0.27 ms
```

The pixel loop everyone reasoned about is 1.9 ms. Everything anyone had
...
`a_settled_glow_does_not_rebuild_its_halo_every_frame` existed, passed
throughout, and could not have caught this: it drew its eight frames with
`force_full: **false**` — a state the game is almost never in. It now runs
the same loop at `true`, and checks `forget_world` actually invalidates.
Both were watched going red against the old code.

That is the same finding as *"every image-level check of this is blind"*
above, arriving through the guard rather than through the render: the
existing check exercised a case the shipped code does not run.

**One note on that section, from re-running its fault against this
implementation.** The off-by-one (dropping the last column of every chunk)
makes `a_glow_halo_is_symmetric_across_a_chunk_seam` go red here as it does
there — and on `viewshot vault=1 seed=6` it *also* moved the rendered PNG.
So the image check is not blind on every scene and every fault; it was blind
on the scenes that were tried. The conclusion stands unchanged and for the
stated reason — whether a lining lands on a chunk seam is luck of the world,
not something a render is choosing to test — but the rule is "an image cannot
be relied on here", not "an image can never move".

## 8. What is left, with both halves in

- The redraw is 2.4 ms and **73% of it is the per-pixel chunk lookup**
  (1.4 ms), which `render_cost` has always measured and which was 3% of the
  old number. That is the next thing in this function worth touching.
- `rebuild_horizon` copies the whole per-cell genesis map every frame
  (0.5-0.7 ms at this world size) for exactly the `App::reset` reason
  `forget_world` now handles properly. A tenth of what was just removed, so
  not chased — but it is the same fix if it ever matters.
