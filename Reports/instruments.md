...
| `plantlook` | **Four ways of drawing the same stand, as a growth timelapse** — the shipped 1:1 render against three approaches that differ in *what* they change: the size of the shape primitive (`masses`), authored sprite art in place of derived shapes (`stamps`), and value/depth with **no shape change at all** (`tone`) | Built because `subpixel`'s single approach came back "not clearly better" three times and `CLAUDE.md`'s rule is that repeated failure the same way condemns the approach rather than the tuning. **Its real contribution is `tone`**, the arm that changes no pixel's shape: it is the control that says whether an appearance complaint is about *form* or about *value*, and nothing else here could distinguish those. Generalises past plants — the structure is "one simulation, N drawings of it, scrubbed together", so any "how should this be drawn" question can be asked by adding an arm. Re-simulates from zero per capture (`World` is not `Clone`, and the background pass has to empty the cells under test), which is seconds and cannot be wrong. Echoes its own parameters and prints the plant-cell count per frame |
