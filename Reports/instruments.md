...
| `labshot` | **What the evolution lab's box actually looks like, and whether anything is living in it** — builds `lab::scene::LabBox` and renders it through the shipped `Renderer` at chosen frames, as a single-column contact sheet, with the plant census printed under each stop | Built because the lab bed is the first scene in this repo to hold **plants and creatures together** — `filmstrip scene=colony` grows its plants from worldgen, `PlantScene` builds beds with no creatures, `creature_probe` builds floors with no plants — so nothing here could look at one. It generalises to **any hand-built scene whose defect would be a scene error rather than a code one**: the counts (`cells`/`orgs`/`seeds`) sit under the picture precisely because a founder that never germinated and one too small to see are the same picture and opposite findings, and `CLAUDE.md` records that mistake costing real time twice. Calls `LabBox::build`, never a private copy, so what it shows is the game's bed and its numbers transfer. **Three knobs added 2026-08-30 by the scene lane**, all of which exist to make a comparison one binary rather than two: `interior=0` draws the same box through the sky path instead of the lab interior, so the before/after is a branch and not a rebuild; `light=0..1` moves the held grow light anywhere on the day's curve, which is the only way to *see* the schedule the guide calls the game's largest lever; and the census gained `still a seed` / `biggest` / `roots reach`, which between them separate "the founder never germinated" from "the founder is one cell tall" and answer §2a's standing question about whether anything reaches the soil depth being paid for |
...
| `wg_ceilings` | **What worldgen cannot express, as opposed to what one world came out like.** `mode=step` is the largest single-column skyline step in the *finished* world, ablated per pass to attribute it; `mode=region` is where region boundaries actually land as a fraction of world width; `mode=strata` counts how many distinct rock hardnesses a whole world has | Read-only, changes no default. **`mode=step` generalises: it is the check that catches a terrain claim measured against the plan heightfield rather than the world** — the two differ by an order of magnitude, because `residuals` paints vertical faces after the plan is fixed. Prints one tagged line per measurement, because the passes `println!` during generation and a `print!`-built table interleaves with them |
