...
| `labsoil` | **Does a deeper bed cost the stand?** The lab bed's soil depth as a seed sweep rather than a single run | Built because a single paired run said deepening the bed 40 -> 96 rows cost 20% of the stand, and `CLAUDE.md` is explicit that outcomes here are chaotic in the seed and that a per-seed baseline gets rubber-stamped. Reads an order statistic. **The trap it exists inside generalises to any bed change**: light reaches the bench as `SKY_TRANSMISSION^(depth / FIELD_SCALE)`, counted in *field blocks*, so a change to the bed's geometry is silently a change to the crop's light -- thickening the ceiling 4 -> 7 rows once cost 45% of the bench light and half the stand with no gate going red. Suspect light before biology whenever a scene change reads as a biology result |
