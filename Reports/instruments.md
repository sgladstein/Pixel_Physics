...
| `labsoil` | **What does deepening the lab's soil cost the crop?** The real `lab::scene::LabBox` bed at several soil depths over many world seeds, censused at equal *ticks*, reporting the paired per-seed direction statistic rather than a pooled mean | Built for the lab's bed-depth question, and reach for it over `labshot soil=` for one reason: `labshot` takes no **world seed**, and one seed is not an answer to anything about procedural content -- `CLAUDE.md` records six seeds reading 1.64x and the next twelve 1.08x on one change, pooling to a per-seed median of zero. **Two things in it generalise past soil.** It equalises *ticks* rather than seconds, which is the fix for a confound that reads as biology and is performance: deeper soil is 1.9x the frame, so an equal-wall-clock arm gets fewer ticks and then reports a smaller stand. And it prints frame 0 beside the final frame for every column, which separates *the scene was built differently* from *the run went differently* -- the two are indistinguishable from an end-state table, and `CLAUDE.md`'s scene-error trap is exactly that confusion. Both controls are shipped and named for which half they hold: `control=empty` is specificity (no founders, so every crop column must read 0), `control=dark` is sensitivity (the sky held low, which must crater the stand). Renders nothing and reads no clock, deliberately |
| `labui` | **What the lab's control bar looks like in every state it has**, including the two a screenshot cannot capture -- hover and pressed -- driven with synthetic cursor positions and synthetic clicks and laid out as a contact sheet | The reusable trick is not the sheet, it is that **it aims every click through the retained layout** (`Ui::widget_rect`) rather than at coordinates the harness believes the button occupies, so a tile cannot be a picture of a click landing where the button is not. And it prints beside each tile whether the click actually *fired*: a bar that paints perfectly and does nothing produces an identical sheet, which is `CLAUDE.md`'s "did it fire at all" needs a counter, not a picture, in its purest form. Any pointer-driven interface with a retained layout can be photographed the same way |
