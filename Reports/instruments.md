...
| `cargo test --lib swing_probe -- --ignored --nocapture` | **What each successive hammer swing does**, as a table: prior damage, how far the crack front grew, cells released, cracked cells, reach, bodies, stone removed | Not a guard — a printed probe (`rigid::swing_probe`). It exists because two separate *fixed points* in the blow were invisible to every picture and every aggregate: the front reaching one ring and standing there for ever, and the cracked count pinned across five identical swings. Both read as "the mechanic is weak" on a contact sheet and as "the mechanic is dead" in this table. Two runs, and reading only one of them cost a wrong diagnosis. `swing_by_swing` aims at the **face** each swing, the way `player::face_toward` does; `worked_scene_swing_by_swing` holds the aim **still** on `acceptance.sh`'s `worked` shelf, which is what working a cantilever's root is. They disagree, and the disagreement is the finding: a moving face lands every blow on fresh rock and keeps a broken growth rule alive, while a fixed aim exposes it in three swings. Read the fixed-aim one when the question is whether the front advances |
