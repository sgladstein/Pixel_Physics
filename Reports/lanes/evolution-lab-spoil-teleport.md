# The spoil teleport, and §Z13 answered — round 32, lane C
...
- **Lane notes are named for the work, not the letter, and this one learned it
  the expensive way.** This round's note was first written to
  `evolution-lab-lane-c.md` with `cat >`, which **destroyed a 221-line note on
  the lab's biosphere stats page** — a different round's lane C, since letters
  are reused and that file was already taken. Caught by reading the PR's own
  diffstat (`338 +++---` on a file this round should only have *added* to), not
  by any gate. `docscheck` has no opinion about a lane note's contents and
  cannot get one. **Read the target before `cat >`, and prefer a name nobody
  else can already hold.**
