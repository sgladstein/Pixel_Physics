...
fn try_bud(world: &mut World, organism: u16, def: &CreatureDef) -> Option<ActiveSite> {
    ...
    if let Some(state) = world.organism_mut(child) {
        ...
        state.genome = genome;
        for (slot, (t, &width)) in state.traits.iter_mut().zip(def.trait_variance.iter()).enumerate() {
            if width > 0.0 {
...
fn gut_of(world: &World, organism: u16, def: &CreatureDef) -> Gut {
    Gut {
        ...
        bite: bite_force_of(def, &traits_of(world, organism, def), world.trait_reach),
    }
}
...
mod tests {
    ...
    fn a_maximally_armoured_ant_is_graded_only_when_the_reach_allows_it() {
        ...
        let median_breach = |reach: f32| -> (usize, usize) {
            ...
            for seed in 0..6u64 {
                ...
                w.seed = 1234 + seed * 7919;
                w.trait_reach = reach;
                // Ant against ant needs the two to be strangers; without this
                // every ant is every other ant's nestmate and the scene holds
                // no fight at all.
    ...
    fn being_bitten_calls_out_and_nothing_else_does() {
        ...
        // The same bed with a fight in it.
        let mut fight = test_world();
        fight.colony_rivalry = true;
