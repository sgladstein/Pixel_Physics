...
fn allocate_to_frontier(world: &mut World, organism_id: u16) {
    ...
    // money, not policy.
    let income = income_noon * night_income_factor(world.frame);
        ...
        for behavior in behavior_buf.into_iter().take(behavior_count).flatten() {
            match behavior {
                Behavior::Photosynthesize { rate, shade_death, transpiration, drought_death } => {
                    ...
                    // `NIGHT_INCOME_FLOOR`.
                    resource =
                        (resource + rate * leaf_rate_mult * light * status * night_income_factor(world.frame)).min(organism::RESOURCE_SCALE);
                }
