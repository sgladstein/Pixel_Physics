...
impl CellSurface for ChunkView<'_> {
    ...
    /// Answered from `group_of` alone, needing no extra state: `step` runs
    /// the four checkerboard groups in index order, so "swept after me" is
    /// just "belongs to a higher-numbered group than mine". A position
    /// inside this worker's own chunk compares equal and so answers `false`,
    /// which is correct — the caller's row-order reasoning is exactly right
    /// within a single chunk's sweep, and that is the overwhelmingly common
    /// case.
    #[inline]
    fn swept_after_me(&self, x: i32, y: i32) -> bool {
        group_of(ChunkCoord::containing(x, y)) > group_of(self.coord)
    }
...
mod tests {
    ...
    fn a_full_multi_chunk_world_of_sand_and_water_settles_under_the_parallel_sweep() {
        ...
        // 3000, raised from 2000 when `CellSurface::swept_after_me` landed.
        // That fix stops a cell crossing into a not-yet-swept chunk moving
        // twice in one frame, so material genuinely does take longer to
        // arrive: measured, this scene settles at frame 2136 where it used
        // to settle at 1399. The old number was not "faster", it was
        // material moving at two cells per frame across half the chunk
        // seams. Budgeted well past the measured value rather than just
        // over it, so this does not sit on its own boundary.
        for _ in 0..3000 {
            step(&mut w);
        }
